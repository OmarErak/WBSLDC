...
// For the following CMSIS includes, we want to use the C fallback, so include
// whether or not we set the CMSIS flag
...
class numpy {
 public:
   ...
   * @returns EIDSP_OK if OK
   */
  static inline int dot_by_row(int i, float *row, uint32_t matrix1_cols,
    ...
    const arm_matrix_instance_f32 m1 = {1, static_cast<uint16_t>(matrix1_cols),
                                        row};
    const arm_matrix_instance_f32 m2 = {static_cast<uint16_t>(matrix2->rows),
                                        static_cast<uint16_t>(matrix2->cols),
                                        matrix2->buffer};
    arm_matrix_instance_f32 mo = {1, static_cast<uint16_t>(out_matrix->cols),
  ...
  static int transpose(float *matrix, int rows, int columns) {
    ...
    const arm_matrix_instance_f32 i_m = {static_cast<uint16_t>(columns),
                                         static_cast<uint16_t>(rows), matrix};
    arm_matrix_instance_f32 o_m = {static_cast<uint16_t>(rows),
   ...
   * @returns EIDSP_OK if OK
   */
  static int dct2(matrix_t *matrix, DCT_NORMALIZATION_MODE normalization =
...
#if EIDSP_USE_CMSIS_DSP
      float rms_result;
      arm_rms_f32(matrix->buffer + (row * matrix->cols), matrix->cols,
                  &rms_result);
...
#if EIDSP_USE_CMSIS_DSP
      float mean;
      arm_mean_f32(input_matrix->buffer + (row * input_matrix->cols),
  ...
  static int min(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      float min;
      uint32_t ix;
      arm_min_f32(input_matrix->buffer + (row * input_matrix->cols),
                  input_matrix->cols, &min, &ix);
  ...
  static int max(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      float max;
      uint32_t ix;
      arm_max_f32(input_matrix->buffer + (row * input_matrix->cols),
                  input_matrix->cols, &max, &ix);
  ...
  static int stdev(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      float std;
      float var;
      cmsis_arm_variance(&input_matrix->buffer[(row * input_matrix->cols)],
                         input_matrix->cols, &var);
  ...
  static int skew(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      // Calculate m_3
      float m_3;
      cmsis_arm_third_moment(&input_matrix->buffer[(row * input_matrix->cols)],
                             input_matrix->cols, mean, &m_3);
  ...
  static int kurtosis(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      // Calculate m_4
      float m_4;
      cmsis_arm_fourth_moment(&input_matrix->buffer[(row * input_matrix->cols)],
                              input_matrix->cols, mean, &m_4);
  ...
  static int rfft(const float *src, size_t src_size, float *output,
    ...
    memset(fft_input.buffer + src_size, 0,
           (n_fft - src_size) * sizeof(kiss_fft_scalar));

#if EIDSP_USE_CMSIS_DSP
    if (n_fft != 32 && n_fft != 64 && n_fft != 128 && n_fft != 256 &&
        n_fft != 512 && n_fft != 1024 && n_fft != 2048 && n_fft != 4096) {
      int ret =
          software_rfft(fft_input.buffer, output, n_fft, n_fft_out_features);
      if (ret != EIDSP_OK) {
        EIDSP_ERR(ret);
      }
    } else {
   ...
   * @returns 0 if OK
   */
  static int int32_to_float(const EIDSP_i32 *input, float *output,
                            size_t length) {
#if EIDSP_USE_CMSIS_DSP
   ...
   * @returns 0 if OK
   */
  static int float_to_int32(const float *input, EIDSP_i32 *output,
                            size_t length) {
#if EIDSP_USE_CMSIS_DSP
   ...
   * @returns 0 if OK
   */
  static int float_to_int16(const float *input, EIDSP_i16 *output,
                            size_t length) {
#if EIDSP_USE_CMSIS_DSP
...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   ...
   * From:
   * https://stackoverflow.com/questions/39821367/very-fast-approximate-logarithm-natural-log-function-in-c/39822314#39822314
...
#pragma GCC diagnostic pop
  ...
  static int software_rfft(float *fft_input, float *output, size_t n_fft,
                           size_t n_fft_out_features) {
    kiss_fft_cpx *fft_output = (kiss_fft_cpx *)ei_dsp_malloc(
        n_fft_out_features * sizeof(kiss_fft_cpx));
    if (!fft_output) {
      EIDSP_ERR(EIDSP_OUT_OF_MEM);
    }
  ...
  static int signal_get_data_i16(int16_t *in_buffer, size_t offset,
                                 size_t length, int16_t *out_ptr) {
...
#if EIDSP_USE_CMSIS_DSP
  /**
   * @brief      The CMSIS std variance function with the same behaviour as the
   * NumPy implementation
   * @details    Variance in CMSIS version is calculated using fSum /
   ...
   */
  static void cmsis_arm_variance(const float32_t *pSrc, uint32_t blockSize,
                                 float32_t *pResult) {
    uint32_t blkCnt;
    float32_t sum = 0.0f;
    float32_t fSum = 0.0f;
  ...
  static int std_axis0_CMSIS(matrix_t *input_matrix, matrix_t *output_matrix) {
    ...
    arm_transposed_matrix.pData = (float *)ei_calloc(
        input_matrix->cols * input_matrix->rows * sizeof(float), 1);
  ...
  /**
   * @brief      A copy of the CMSIS power function, adapted to calculate the
   * third central moment
   * @details    Calculates the sum of cubes of a block with the mean value
   * subtracted.
   * @param[in]  pSrc       Pointer to float block
   * @param[in]  blockSize  Number of floats in block
   ...
   */
  static void cmsis_arm_third_moment(const float32_t *pSrc, uint32_t blockSize,
                                     float32_t mean, float32_t *pResult) {
    uint32_t blkCnt;
    float32_t sum = 0.0f;
    float32_t in;
   ...
   * @details    Calculates the sum of fourth powers of a block with the mean
   * value subtracted.
   * @param[in]  pSrc       Pointer to float block
   * @param[in]  blockSize  Number of floats in block
   * @param[in]  mean       The mean to subtract from each value before
   * calculating fourth power
   * @param      pResult    The fourth central moment of the input
   */
  static void cmsis_arm_fourth_moment(const float32_t *pSrc, uint32_t blockSize,
                                      float32_t mean, float32_t *pResult) {
    uint32_t blkCnt;
    float32_t sum = 0.0f;
    float32_t in;
  ...
  static void sqrt_q15(int16_t in, int16_t *pOut) {
    ...
    if (number > 0) {
      ...
      /* 1st iteration */
      var1 = ((int16_t)((int32_t)var1 *
                            (0x3000 -
                             ((int16_t)((((int16_t)(((int32_t)var1 * var1) >>
                                                    15)) *
                                         (int32_t)half) >>
                                        15))) >>
                        15))
   ...
   * We do it this way as this means we can compile out fast_init calls which
   * hints the compiler to which tables can be removed
   */
  static int cmsis_rfft_init_f32(arm_rfft_fast_instance_f32 *rfft_instance,
                                 const size_t n_fft) {
// ARM cores (ex M55) with Helium extensions (MVEF) need special treatment
// (Issue 2843)
