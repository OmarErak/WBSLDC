...
enum YUV_OPTIONS {
  BIG_ENDIAN_ORDER = 1,  // RGB reading from low to high memory.  Otherwise,
 ...
 */
int yuv422_to_rgb888(unsigned char *rgb_out, unsigned const char *yuv_in,
                     unsigned int in_size_B, YUV_OPTIONS opts) {
...
#define EI_GET_B_FROM_YUV(y, u, v) ((298 * y + 516 * u + 128) >> 8)
  ...
  int rgb_end =
      TEST_BIT_MASK(opts, PAD_4B) ? 2 * in_size_B : (6 * in_size_B) / 4;
  rgb_out += rgb_end - 1;
 ...
 * @param dstWidth Desired X dimension in pixels (should be smaller than
 * srcWidth)
 * @param dstHeight Desired Y dimension in pixels (should be smaller than
 * srcHeight)
 * @param dstImage Output buffer, can be the same as srcImage
 * @param iBpp 8 or 16 for bits per pixel
 */
int cropImage(const uint8_t *srcImage, int srcWidth, int srcHeight, int startX,
              int startY, uint8_t *dstImage, int dstWidth, int dstHeight,
              int iBpp) {
  uint32_t *s32, *d32;
  int x, y;
  ...
  if (iBpp == 8) {
    ...
    for (y = 0; y < dstHeight; y++) {
      ...
      x = 0;
      if ((intptr_t)s & 3 ||
          (intptr_t)d & 3) {  // either src or dst pointer is not aligned
        for (; x < dstWidth; x++) {
    ...
    for (y = 0; y < dstHeight; y++) {
      ...
      x = 0;
      if ((intptr_t)s & 2 ||
          (intptr_t)d & 2) {  // either src or dst pointer is not aligned
        for (; x < dstWidth; x++) {
 ...
 * @copydoc cropImage(
    ...
    int iBpp)
 */
int crop_image_rgb888_packed(const uint8_t *srcImage, int srcWidth,
                             int srcHeight, int startX, int startY,
                             uint8_t *dstImage, int dstWidth, int dstHeight) {
  // use 8 bpp mode, but do everything *3 for RGB
  return cropImage(srcImage, srcWidth * 3, srcHeight, startX * 3, startY,
                   dstImage, dstWidth * 3, dstHeight, 8);
}

/**
 * @brief Resize an image using interpolation
 * Can be used to resize the image smaller or larger
 * If resizing much smaller than 1/3 size, then a more rubust algorithm should
 * average all of the pixels This algorithm uses bilinear interpolation -
 * averages a 2x2 region to generate each new pixel
 *
...
int resize_image(const uint8_t *srcImage, int srcWidth, int srcHeight,
  ...
  // Modified for RGB888
  // This needs to be < 16 or it won't fit. Cortex-M4 only has SIMD for signed
  // multiplies
  constexpr int FRAC_BITS = 14;
  constexpr int FRAC_VAL = (1 << FRAC_BITS);
  constexpr int FRAC_MASK = (FRAC_VAL - 1);

  uint32_t src_x_accum,
      src_y_accum;  // accumulators and fractions for scaling the image
  ...
  // start at 1/2 pixel in to account for integer downsampling which might miss
  // pixels
  ...
  for (y = 0; y < dstHeight; y++) {
    ...
    // start at 1/2 pixel in to account for integer downsampling which might
    // miss pixels
 ...
 */
void calculate_crop_dims(int srcWidth, int srcHeight, int dstWidth,
                         int dstHeight, int &cropWidth, int &cropHeight) {
  // first, trim the largest axis to match destination aspect ratio
  // calculate by fixing the smaller axis
  if (srcWidth > srcHeight) {
    cropWidth = (uint32_t)(dstWidth * srcHeight) /
                dstHeight;  // cast in case int is small
...
int crop_and_interpolate_rgb888(const uint8_t *srcImage, int srcWidth,
                                int srcHeight, uint8_t *dstImage, int dstWidth,
                                int dstHeight) {
  int cropWidth, cropHeight;
  // What are dimensions that maintain aspect ratio?
  calculate_crop_dims(srcWidth, srcHeight, dstWidth, dstHeight, cropWidth,
                      cropHeight);
  // Now crop to that dimension
  int res = crop_image_rgb888_packed(
      srcImage, srcWidth, srcHeight, (srcWidth - cropWidth) / 2,
      (srcHeight - cropHeight) / 2, dstImage, cropWidth, cropHeight);

  if (res != EIDSP_OK) {
    return res;
  }
  // Finally, interpolate down to desired dimensions, in place
  return resize_image(dstImage, cropWidth, cropHeight, dstImage, dstWidth,
                      dstHeight, 3);
}
