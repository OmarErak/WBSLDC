...
class fir_filter {
 private:
   ...
   *
   * @param cutoff_normalized Should be in the range 0..0.5 (0.5 being the
   * nyquist)
   */
 ...
 public:
   ...
   * @param lowpass_cutoff Lowpass cutoff freqency.  If 0, will be a high pass
   * filter
   * @param highpass_cutoff Highpass cutoff.  If 0, will just be a lowpass.  If
   * both lowpass and higpass, bandpass
   * @param decimation_ratio To downsample, ratio of samples to get rid of.
   * For example, 4 to go from sample rate of 40k to 10k.  LOWPASS CUTOFF MUST
   * MATCH THIS If you don't filter the high frequencies, they WILL alias into
   * the passband So in the above example, you would want to cutoff at 5K (so
   * you have some buffer)
   */
  fir_filter(float sampling_frequency, uint8_t filter_size,
             float lowpass_cutoff, float highpass_cutoff = 0,
             int decimation_ratio = 1)
      : taps(filter_size), history(filter_size, 0) {
    ...
    if (lowpass_cutoff == 0) {
      // for highpass, we'll just design a lowpass filter, then invert its
      // spectrum
  ...
  /**
   * @brief Apply the filter to the input data.  You can do this blockwise, as
   * the object preserves memory of old samples Call reset if there's a gap in
   * the data
   *
   ...
   */
  void apply_filter(const input_t *src, input_t *dest, size_t size) {
   ...
   *
   */
  void reset() { std::fill(history.begin(), history.end(), 0); }
