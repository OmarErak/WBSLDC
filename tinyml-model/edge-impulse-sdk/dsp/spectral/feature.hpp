...
class feature {
 public:
   ...
   * @param out_features Output matrix. Use `calculate_spectral_buffer_size` to
   * calculate the size required. Needs as many rows as `raw_data`.
   ...
   * @returns 0 if OK
   */
  static int spectral_analysis(matrix_t *out_features, matrix_t *input_matrix,
                               float sampling_freq, filter_t filter_type,
                               float filter_cutoff, uint8_t filter_order,
                               uint16_t fft_length, uint8_t fft_peaks,
    ...
    if (out_features->cols != calculate_spectral_buffer_size(
                                  true, fft_peaks, edges_matrix_in->rows)) {
      EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
    }
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      EI_DSP_MATRIX_B(axis_matrix, 1, input_matrix->cols,
                      input_matrix->buffer + (row * input_matrix->cols));

      // calculate FFT
      EI_DSP_MATRIX(fft_matrix, 1, fft_length / 2 + 1);
      ret = numpy::rfft(axis_matrix.buffer, axis_matrix.cols, fft_matrix.buffer,
                        fft_matrix.cols, fft_length);
      if (ret != EIDSP_OK) {
        EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
      }
      ...
      ret = spectral::processing::find_fft_peaks(
          &fft_matrix, &peaks_matrix, sampling_freq, fft_peaks_threshold,
          fft_length);
      if (ret != EIDSP_OK) {
        EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
      }
      ...
      ret = spectral::processing::periodogram(&axis_matrix, &period_fft_matrix,
                                              &period_freq_matrix,
                                              sampling_freq, fft_length);
      if (ret != EIDSP_OK) {
        EIDSP_ERR(ret);
      }
      ...
      ret = spectral::processing::spectral_power_edges(
          &period_fft_matrix, &period_freq_matrix, edges_matrix_in,
          &edges_matrix_out, sampling_freq);
      if (ret != EIDSP_OK) {
        EIDSP_ERR(ret);
      }
      ...
      for (size_t peak_row = 0; peak_row < peaks_matrix.rows; peak_row++) {
        features_row[fx++] =
            peaks_matrix.buffer[peak_row * peaks_matrix.cols + 0];
        features_row[fx++] =
            peaks_matrix.buffer[peak_row * peaks_matrix.cols + 1];
      }
      for (size_t edge_row = 0; edge_row < edges_matrix_out.rows; edge_row++) {
        features_row[fx++] =
            edges_matrix_out.buffer[edge_row * edges_matrix_out.cols] / 10.0f;
      }
    }
   ...
   */
  static size_t calculate_spectral_buffer_size(bool rms, size_t peaks_count,
                                               size_t spectral_edges_count) {
    size_t count = 0;
    if (rms) count++;
    count += (peaks_count * 2);
  ...
  static int extract_spectral_analysis_features_v1(
      matrix_t *input_matrix, matrix_t *output_matrix,
    ...
    memcpy(spectral_str, config_ptr->spectral_power_edges,
    ...
    size_t output_matrix_cols =
        spectral::feature::calculate_spectral_buffer_size(
            true, config_ptr->spectral_peaks_count, edges_matrix_in.rows);
    ...
    ret = spectral::feature::spectral_analysis(
        output_matrix, input_matrix, sampling_freq, filter_type,
        config_ptr->filter_cutoff, config_ptr->filter_order,
        config_ptr->fft_length, config_ptr->spectral_peaks_count,
        config_ptr->spectral_peaks_threshold, &edges_matrix_in);
    if (ret != EIDSP_OK) {
  ...
  static void get_start_stop_bin(float sampling_freq, size_t fft_length,
                                 float filter_cutoff, size_t *start_bin,
                                 size_t *stop_bin, bool is_high_pass) {
    // we want to find n such that fcutoff < sample_f / fft * n ( or > for high
    // pass ) also, + - half bin width (sample_f/(fft*2)) for high / low pass
    float bin = filter_cutoff * fft_length / sampling_freq;
    if (is_high_pass) {
      *start_bin = static_cast<size_t>(bin - 0.5) +
                   1;  // add one b/c we want to always round up
  ...
  static size_t extract_spec_features(
      matrix_t *input_matrix, matrix_t *output_matrix,
      ei_dsp_config_spectral_analysis_t *config, const float sampling_freq,
    ...
    if (strcmp(config->filter_type, "low") == 0) {
      if (config->filter_order) {
        EI_TRY(spectral::processing::butterworth_lowpass_filter(
            input_matrix, sampling_freq, config->filter_cutoff,
            config->filter_order));
      }
      do_filter = true;
      is_high_pass = false;
    ...
    if (do_filter) {
      get_start_stop_bin(sampling_freq, config->fft_length,
                         config->filter_cutoff, &start_bin, &stop_bin,
                         is_high_pass);
    } else {
      start_bin = 1;
    ...
    for (size_t row = 0; row < input_matrix->rows; row++) {
      ...
      // Standard Deviation
      float stddev = *(feature_out -
                       1);  //= sqrt(numpy::variance(data_window, data_size));
      if (stddev == 0.0f) {
        stddev = 1e-10f;
      }
      ...
      *feature_out++ = ((k_sum / data_size) / (temp * stddev)) - 3;
      ...
      if (config->implementation_version == 4) {
        ...
        EI_TRY(numpy::welch_max_hold(data_window, data_size, fft_out.data(), 0,
                                     fft_out_size, config->fft_length,
                                     config->do_fft_overlap));

        matrix_t x(1, fft_out.size(), const_cast<float *>(fft_out.data()));
        matrix_t out(1, 1);

        *feature_out++ = (numpy::skew(&x, &out) == EIDSP_OK)
                             ? (out.get_row_ptr(0)[0])
                             : 0.0f;
        *feature_out++ = (numpy::kurtosis(&x, &out) == EIDSP_OK)
                             ? (out.get_row_ptr(0)[0])
                             : 0.0f;
        ...
        EI_TRY(numpy::welch_max_hold(data_window, data_size, feature_out,
                                     start_bin, stop_bin, config->fft_length,
                                     config->do_fft_overlap));
      }
      if (config->do_log) {
  ...
  static int extract_spectral_analysis_features_v2(
      matrix_t *input_matrix, matrix_t *output_matrix,
      ei_dsp_config_spectral_analysis_t *config, const float sampling_freq) {
    size_t n_features = extract_spec_features(input_matrix, output_matrix,
                                              config, sampling_freq);
    return n_features == output_matrix->cols ? EIDSP_OK
                                             : EIDSP_MATRIX_SIZE_MISMATCH;
  }
  ...
  static int extract_spectral_analysis_features_v3(
    ...
    if (strcmp(config->analysis_type, "Wavelet") == 0) {
      return wavelet::extract_wavelet_features(input_matrix, output_matrix,
                                               config, sampling_freq);
    } else {
      return extract_spectral_analysis_features_v2(input_matrix, output_matrix,
                                                   config, sampling_freq);
    }
  }
  ...
  static size_t _decimate(matrix_t *input_matrix, matrix_t *output_matrix,
                          size_t ratio) {
    // generated by build_sav4_header in prepare.py
    static float sos_deci_3[] = {3.4799547399084973e-05f,
                                 6.959909479816995e-05f,
                                 3.4799547399084973e-05f,
                                 1.0f,
                                 -1.416907422639627f,
                                 0.5204552955670066f,
                                 1.0f,
                                 ...
                                 -1.2180861262950025f,
                                 0.8987833581253264};
    static float sos_zi_deci_3[] = {
        0.0013094887094341828f, -0.0006648423946383296f, 0.0193087012128479f,
        -0.010936639208493802f, 0.1485445305451165f,     -0.10217301649013415f,
    ...
    static float sos_deci_10[] = {3.5863243209995215e-09f,
                                  ...
                                  -1.8965395961864169f,
                                  0.9644245584642932};
    static float sos_zi_deci_10[] = {
        1.38071060429997e-06f,   -1.146570262401316e-06f,
  ...
  static int extract_spectral_analysis_features_v4(
    ...
    if (strcmp(config->analysis_type, "Wavelet") == 0) {
      return wavelet::extract_wavelet_features(input_matrix, output_matrix,
                                               config, sampling_freq);
    } else if (config->extra_low_freq == false &&
               config->input_decimation_ratio == 1) {
      size_t n_features = extract_spec_features(input_matrix, output_matrix,
                                                config, sampling_freq);
      return n_features == output_matrix->cols ? EIDSP_OK
                                               : EIDSP_MATRIX_SIZE_MISMATCH;
      ...
      if (config->input_decimation_ratio > 1) {
        ei_vector<int> ratio_combo =
            get_ratio_combo(config->input_decimation_ratio);
      ...
      // do this before extract_spec_features because extract_spec_features
      // modifies the matrix
      ...
      size_t n_features = extract_spec_features(
          input_matrix, output_matrix, config, new_sampling_freq, true, false);

      if (n_features > 0 && config->extra_low_freq) {
        matrix_t lf_features(
            1, output_matrix->rows * output_matrix->cols - n_features,
            output_matrix->buffer + n_features);

        n_features +=
            extract_spec_features(&lf_signal, &lf_features, config,
                                  new_sampling_freq / decimation, true, false);
      }
      return n_features == output_matrix->cols ? EIDSP_OK
                                               : EIDSP_MATRIX_SIZE_MISMATCH;
    }
