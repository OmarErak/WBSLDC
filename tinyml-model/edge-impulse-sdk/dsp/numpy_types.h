 ...
 * Freeing happens by calling `delete` on the object or letting the object go
 * out of scope.
 */
typedef struct ei_matrix {
   ...
   */
  ei_matrix(uint32_t n_rows, uint32_t n_cols, float *a_buffer = NULL
#if EIDSP_TRACK_ALLOCATIONS
            ,
            const char *fn = NULL, const char *file = NULL, int line = 0
#endif
  ) {
    if (a_buffer) {
      buffer = a_buffer;
    ...
    if (!a_buffer) {
      ...
      if (_fn) {
        ei_dsp_register_matrix_alloc_internal(fn, file, line, rows, cols,
                                              sizeof(float), buffer);
      } else {
...
#if EIDSP_TRACK_ALLOCATIONS
      if (_fn) {
        ei_dsp_register_matrix_free_internal(
            _fn, _file, _line, _originally_allocated_rows,
            _originally_allocated_cols, sizeof(float), buffer);
      } else {
        ei_dsp_register_matrix_free(_originally_allocated_rows,
                                    _originally_allocated_cols, sizeof(float),
                                    buffer);
      }
#endif
    }
   ...
   */
  float *get_row_ptr(size_t row) { return buffer + row * cols; }
...
typedef struct ei_matrix_i8 {
   ...
   * @param a_buffer Buffer, if not provided we'll alloc on the heap
   */
  ei_matrix_i8(uint32_t n_rows, uint32_t n_cols, int8_t *a_buffer = NULL
#if EIDSP_TRACK_ALLOCATIONS
               ,
               const char *fn = NULL, const char *file = NULL, int line = 0
#endif
  ...
  ~ei_matrix_i8() {
    if (buffer && buffer_managed_by_me) {
      ...
      if (_fn) {
        ...
        ei_dsp_register_matrix_free(_originally_allocated_rows,
                                    _originally_allocated_cols, sizeof(int8_t),
                                    buffer);
      }
#endif
    }
   ...
   * @return float* Pointer to the buffer at the start of row n
   */
  int8_t *get_row_ptr(size_t row) { return buffer + row * cols; }
...
typedef struct ei_matrix_i32 {
   ...
   * @param a_buffer Buffer, if not provided we'll alloc on the heap
   */
  ei_matrix_i32(uint32_t n_rows, uint32_t n_cols, int32_t *a_buffer = NULL
#if EIDSP_TRACK_ALLOCATIONS
                ,
                const char *fn = NULL, const char *file = NULL, int line = 0
#endif
  ...
  ~ei_matrix_i32() {
    if (buffer && buffer_managed_by_me) {
      ...
      if (_fn) {
        ...
        ei_dsp_register_matrix_free(_originally_allocated_rows,
                                    _originally_allocated_cols, sizeof(int32_t),
                                    buffer);
      }
#endif
    }
   ...
   * @return float* Pointer to the buffer at the start of row n
   */
  int32_t *get_row_ptr(size_t row) { return buffer + row * cols; }
 ...
 * Freeing happens by calling `delete` on the object or letting the object go
 * out of scope. We use this for the filterbanks, as we quantize these
...
typedef struct ei_quantized_matrix {
   ...
   * @param a_buffer Optional: a buffer, if set we won't allocate memory
   * ourselves
   */
  ei_quantized_matrix(uint32_t n_rows, uint32_t n_cols,
#ifdef __MBED__
...
#if EIDSP_TRACK_ALLOCATIONS
                      ,
                      const char *fn = NULL, const char *file = NULL,
                      int line = 0
#endif
  ) {
    if (a_buffer) {
  ...
  ~ei_quantized_matrix() {
    if (buffer && buffer_managed_by_me) {
      ...
      if (_fn) {
        ...
        ei_dsp_register_matrix_free(_originally_allocated_rows,
                                    _originally_allocated_cols, sizeof(uint8_t),
                                    buffer);
      }
#endif
    }
   ...
   * @return float* Pointer to the buffer at the start of row n
   */
  uint8_t *get_row_ptr(size_t row) { return buffer + row * cols; }
...
typedef struct ei_matrix_u8 {
   ...
   * @param a_buffer Buffer, if not provided we'll alloc on the heap
   */
  ei_matrix_u8(uint32_t n_rows, uint32_t n_cols, uint8_t *a_buffer = NULL
#if EIDSP_TRACK_ALLOCATIONS
               ,
               const char *fn = NULL, const char *file = NULL, int line = 0
#endif
...
// required on Adafruit nRF52, it seems not to matter too much on other
// targets...
#ifdef __cplusplus
namespace std {
