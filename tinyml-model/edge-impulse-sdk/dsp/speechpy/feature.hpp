...
class feature {
 public:
   ...
   * @param output_transposed If set to true this will transpose the matrix
   * (memory efficient). This is more efficient than calling this function and
   * then transposing as the latter requires the filterbank to be allocated
   * twice (for a short while).
  ...
  static int filterbanks(
      ...
      uint32_t low_freq, uint32_t high_freq, bool output_transposed = false) {
    ...
    if (filterbanks->rows != num_filter ||
        filterbanks->cols != static_cast<uint32_t>(coefficients)) {
      EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
    }

#if EIDSP_QUANTIZE_FILTERBANK
    memset(filterbanks->buffer, 0,
           filterbanks->rows * filterbanks->cols * sizeof(uint8_t));
#else
    memset(filterbanks->buffer, 0,
           filterbanks->rows * filterbanks->cols * sizeof(float));
#endif
    ...
    // num_filter+2 point.
    numpy::linspace(functions::frequency_to_mel(static_cast<float>(low_freq)),
    ...
    for (uint16_t ix = 0; ix < num_filter + 2; ix++) {
      ...
      // here is a really annoying bug in Speechpy which calculates the
      // frequency index wrong for the last bucket the last 'hertz' value is not
      // 8,000 (with sampling rate 16,000) but 7,999.999999 thus calculating the
      // bucket to 64, not 65. we're adjusting this here a tiny bit to ensure we
    ...
    for (uint16_t ix = 0; ix < num_filter + 2; ix++) {
      freq_index[ix] = static_cast<int>(
          floor((coefficients + 1) * hertz[ix] / sampling_freq));
    }
    ei_dsp_free(hertz, hertz_mem_size);
    ...
    for (size_t i = 0; i < num_filter; i++) {
      ...
      // so... z now contains some values that we need to overwrite in the
      // filterbank
   ...
   */
  static int get_fft_bin_from_hertz(uint16_t fft_size, float hertz,
   ...
   * @param out_features Use `calculate_mfe_buffer_size` to allocate the right
   * matrix.
   * @param out_energies A matrix in the form of Mx1 where M is the rows from
   * `calculate_mfe_buffer_size`
   * @param signal: audio signal structure with functions to retrieve data from
   * a signal
  ...
  static int mfe(matrix_t *out_features, matrix_t *out_energies,
                 ...
                 uint16_t fft_length, uint32_t low_frequency,
                 uint32_t high_frequency, uint16_t version) {
    int ret = 0;
    ...
    ret = processing::stack_frames(&stack_frame_info, sampling_frequency,
                                   frame_length, frame_stride, false, version);
    if (ret != 0) {
      EIDSP_ERR(ret);
    }
    ...
    if (out_energies) {
      if (stack_frame_info.frame_ixs.size() != out_energies->rows ||
          out_energies->cols != 1) {
        EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
      }
    }
    ...
    uint16_t *bins = reinterpret_cast<uint16_t *>(
        mels);  // alias the mels array so we can reuse the space
    ...
    uint16_t max_bin =
        version >= 4 ? fft_length
                     : power_spectrum_frame_size;  // preserve a bug in v<4
    ...
    // here is a really annoying bug in Speechpy which calculates the frequency
    // index wrong for the last bucket the last 'hertz' value is not 8,000 (with
    // sampling rate 16,000) but 7,999.999999 thus calculating the bucket to 64,
    // not 65. we're adjusting this here a tiny bit to ensure we have the same
    // result
    ...
    bins[MELS_SIZE - 1] = get_fft_bin_from_hertz(max_bin, mels[MELS_SIZE - 1],
                                                 sampling_frequency);
    ...
    for (size_t ix = 0; ix < stack_frame_info.frame_ixs.size(); ix++) {
      // don't read outside of the audio buffer... we'll automatically zero pad
      // then
      size_t signal_offset = stack_frame_info.frame_ixs.at(ix);
      size_t signal_length = stack_frame_info.frame_length;
      if (signal_offset + signal_length >
          stack_frame_info.signal->total_length) {
        signal_length = signal_length - (stack_frame_info.signal->total_length -
                                         (signal_offset + signal_length));
      }

      ret = stack_frame_info.signal->get_data(signal_offset, signal_length,
                                              signal_frame.buffer);
      if (ret != 0) {
        EIDSP_ERR(ret);
      }
      ...
      ret = numpy::power_spectrum(
          signal_frame.buffer, stack_frame_info.frame_length,
          power_spectrum_frame.buffer, power_spectrum_frame_size, fft_length);
      ...
      float energy =
          numpy::sum(power_spectrum_frame.buffer, power_spectrum_frame_size);
      if (energy == 0) {
        energy = 1e-10;
      }
      ...
      for (size_t i = 0; i < num_filters; i++) {
        ...
        for (size_t bin = left + 1; bin < right; bin++) {
          if (bin < middle) {
            row_ptr[i] += ((static_cast<float>(bin) - left) /
                           (middle - left)) *  // weight *
                          power_spectrum_frame.buffer[bin];
          }
          // intentionally skip middle, handled above
          if (bin > middle) {
            row_ptr[i] += ((right - static_cast<float>(bin)) /
                           (right - middle)) *  // weight *
                          power_spectrum_frame.buffer[bin];
          }
        }
  ...
  static int mfe_v3(matrix_t *out_features, matrix_t *out_energies,
                    signal_t *signal, uint32_t sampling_frequency,
                    float frame_length, float frame_stride,
                    uint16_t num_filters, uint16_t fft_length,
                    uint32_t low_frequency, uint32_t high_frequency,
                    uint16_t version) {
    int ret = 0;
    ...
    // calculate the filterbanks first... preferably I would want to do the
    // matrix multiplications whenever they happen, but OK...
#if EIDSP_QUANTIZE_FILTERBANK
    EI_DSP_QUANTIZED_MATRIX(filterbanks, num_filters, coefficients,
                            &numpy::dequantize_zero_one);
#else
    ...
    ret = feature::filterbanks(&filterbanks, num_filters, coefficients,
                               sampling_frequency, low_frequency,
                               high_frequency, true);
    if (ret != 0) {
      EIDSP_ERR(ret);
    }
    for (size_t ix = 0; ix < stack_frame_info.frame_ixs.size(); ix++) {
      ...
      ret = numpy::dot_by_row(ix, power_spectrum_frame.buffer,
                              power_spectrum_frame_size, &filterbanks,
                              out_features);
   ...
   * @EIDSP_OK if OK
   */
  static int spectrogram(matrix_t *out_features, signal_t *signal,
                         float sampling_frequency, float frame_length,
                         float frame_stride, uint16_t fft_length,
                         uint16_t version) {
    int ret = 0;
    ...
    for (size_t ix = 0; ix < stack_frame_info.frame_ixs.size(); ix++) {
      ...
      if (version >= 3) {
        ...
        for (size_t ix = 0; ix < signal_frame.rows * signal_frame.cols; ix++) {
          if (signal_frame.buffer[ix] < -1.0f ||
              signal_frame.buffer[ix] > 1.0f) {
      ...
      ret = numpy::power_spectrum(
          signal_frame.buffer, stack_frame_info.frame_length,
          out_features->buffer + (ix * coefficients), coefficients, fft_length);
  ...
  static matrix_size_t calculate_mfe_buffer_size(
      size_t signal_length, uint32_t sampling_frequency, float frame_length,
      float frame_stride, uint16_t num_filters, uint16_t version) {
    int32_t rows = processing::calculate_no_of_stack_frames(
        signal_length, sampling_frequency, frame_length, frame_stride, false,
        version);
   ...
   * @param out_features Use `calculate_mfcc_buffer_size` to allocate the right
   * matrix.
  ...
  static int mfcc(matrix_t *out_features, signal_t *signal,
                  uint32_t sampling_frequency, float frame_length,
                  float frame_stride, uint8_t num_cepstral,
                  uint16_t num_filters, uint16_t fft_length,
                  uint32_t low_frequency, uint32_t high_frequency,
                  bool dc_elimination, uint16_t version) {
    if (out_features->cols != num_cepstral) {
      EIDSP_ERR(EIDSP_MATRIX_SIZE_MISMATCH);
    }

    matrix_size_t mfe_matrix_size = calculate_mfe_buffer_size(
        signal->total_length, sampling_frequency, frame_length, frame_stride,
        num_filters, version);
    ...
    ret = mfe(&features_matrix, &energy_matrix, signal, sampling_frequency,
    ...
    if (dc_elimination) {
      for (size_t row = 0; row < features_matrix.rows; row++) {
        features_matrix.buffer[row * features_matrix.cols] =
            numpy::log(energy_matrix.buffer[row]);
      }
    }
    ...
    for (size_t row = 0; row < features_matrix.rows; row++) {
      for (int i = 0; i < num_cepstral; i++) {
        *(out_features->buffer + (num_cepstral * row) + i) =
            *(features_matrix.buffer + (features_matrix.cols * row) + i);
      }
    }
  ...
  static matrix_size_t calculate_mfcc_buffer_size(
      size_t signal_length, uint32_t sampling_frequency, float frame_length,
      float frame_stride, uint16_t num_cepstral, uint16_t version) {
    int32_t rows = processing::calculate_no_of_stack_frames(
        signal_length, sampling_frequency, frame_length, frame_stride, false,
        version);
