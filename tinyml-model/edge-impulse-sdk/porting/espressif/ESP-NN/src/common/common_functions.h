...
__NN_FORCE_INLINE__ int32_t esp_nn_sat_round_doubling_high_mul(int32_t in0,
                                                               int32_t in1) {
  int32_t result;
...
/**
 * fast version
 * this will fail for values closer to INT32_MAX and INT32_MIN by `1 <<
 * (exponent - 1)`. We can afford to do this because we are at the very last
 * stage of filter. Also it is pretty rare condition as our output is going to
 * be 8 bit.
 */
__NN_FORCE_INLINE__ int32_t esp_nn_div_by_power_of_two_fast(int32_t val,
                                                            int32_t exponent) {
  int32_t to_add = (1 << (exponent - 1)) - (val < 0);
  return (int32_t)((val + to_add) >> exponent);
}

__NN_FORCE_INLINE__ int32_t esp_nn_div_by_power_of_two(int32_t val,
                                                       int32_t exponent) {
  int32_t result;
...
__NN_FORCE_INLINE__ int32_t esp_nn_multiply_by_quantized_mult(int32_t x,
                                                              int32_t mult,
                                                              int32_t shift) {
  int32_t left_shift = shift > 0 ? shift : 0;
  int32_t right_shift = shift > 0 ? 0 : -shift;
  int32_t result =
      esp_nn_sat_round_doubling_high_mul(x * (1 << left_shift), mult);
  return esp_nn_div_by_power_of_two(result, right_shift);
}

__NN_FORCE_INLINE__ int32_t
esp_nn_multiply_by_quantized_mult_fast(int32_t x, int32_t mult, int32_t shift) {
...
static void esp_nn_aligned_s8_pad_with_value(
    const int8_t *src, int8_t *dst, const uint16_t input_wd,
    const uint16_t input_ht, const uint16_t channels, const int32_t pad_val,
    const uint16_t pad_wd, const uint16_t pad_ht) {
  /* memset with pad_val */
  memset(dst, pad_val,
         ((input_wd + 2 * pad_wd) * (input_ht + 2 * pad_ht)) * channels);
...
static void esp_nn_aligned_s8_pad_end_with_value(
    const int8_t *src, int8_t *dst, const uint16_t input_wd,
    const uint16_t input_ht, const uint16_t channels, const int32_t pad_val,
    const uint16_t pad_wd, const uint16_t pad_ht) {
  for (int i = 0; i < input_ht; i++) {
 ...
 */
__NN_FORCE_INLINE__ void esp_nn_s8_to_s16_with_offset(const int8_t *src,
                                                      int16_t *dst,
                                                      const int size,
 ...
 * @param       size length of data
 */
__NN_FORCE_INLINE__ void esp_nn_s8_to_s16(const int8_t *src, int16_t *dst,
                                          const int size) {
  int i = 0;
  for (; i < size; i += 2) {
    dst[i + 0] = src[i + 0];
