...
#define DIFF_BITS 5
  ...
  for (int row_idx = 0; row_idx < height; row_idx++) {
    ...
    for (col = 0; col < width; col++) {
      ...
      if (input_diff >= diff_min) {
        const int32_t input_diff_rescaled =
            SAT_HIGH_MUL(input_diff * mask, mult);
        const int32_t exp_raw =
            esp_nn_exp_on_negative_values(input_diff_rescaled);
        scratch_buf[col] =
            exp_raw;  // store to avoid duplicate calculation later
    ...
    const int32_t shifted_scale =
        ONE_OVER_ONE_X((sum_of_exps << headroom_plus1) - (1 << 31));
    const int32_t bits_over_unit =
        ACCUM_BITS - headroom_plus1 + 31 - sizeof(int8_t) * 8;
