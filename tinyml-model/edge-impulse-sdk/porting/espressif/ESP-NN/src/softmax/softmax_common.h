...
__NN_FORCE_INLINE__ int32_t mul_power_of_2(int val, int exp) {
  ...
  result =
      SELECT_USING_MASK(MASK_IF_NON_ZERO(val < -thresh), INT32_MIN, result);
  return result;
}
 ...
 *          Refer to that page for the logic behind the 48/17 and 32/17
 * constants. Pseudocode:
 * https://en.wikipedia.org/wiki/Division_algorithm#Pseudocode
 */
__NN_FORCE_INLINE__ int32_t
esp_nn_one_over_one_plus_x_for_x_in_0_1(int32_t val) {
  ...
  int32_t x = constant_48_over_17 +
              SAT_HIGH_MUL(half_denominator, constant_neg_32_over_17);
  const int32_t fixed_2_one = (1 << 29);

  x += mul_power_of_2(
      SAT_HIGH_MUL(x, fixed_2_one - SAT_HIGH_MUL(half_denominator, x)), 2);
  x += mul_power_of_2(
      SAT_HIGH_MUL(x, fixed_2_one - SAT_HIGH_MUL(half_denominator, x)), 2);
  x += mul_power_of_2(
      SAT_HIGH_MUL(x, fixed_2_one - SAT_HIGH_MUL(half_denominator, x)), 2);
...
__NN_FORCE_INLINE__ int32_t esp_nn_exp_on_negative_values(int32_t val) {
  ...
  const int32_t x4_over_4_plus_x3_over_6_plus_x2_over_2 =
      DIV_POW2(SAT_HIGH_MUL(x4_over_4 + x3, one_over_3) + x2, 1);
  int32_t result =
      one_over_8 +
      SAT_HIGH_MUL(one_over_8, x + x4_over_4_plus_x3_over_6_plus_x2_over_2);
