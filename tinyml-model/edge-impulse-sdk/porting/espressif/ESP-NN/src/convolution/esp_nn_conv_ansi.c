 ...
 */
void esp_nn_conv_u8_ansi(const uint8_t *input_data, const uint16_t input_wd,
                         const uint16_t input_ht, const uint16_t in_channels,
                         const int32_t input_offset, const uint16_t pad_wd,
                         const uint16_t pad_ht, const uint16_t stride_wd,
                         const uint16_t stride_ht, const uint8_t *filter_data,
                         const uint16_t filter_wd, const uint16_t filter_ht,
                         const int32_t filter_offset, const int32_t *bias,
                         uint8_t *out_data, const uint16_t out_wd,
                         const uint16_t out_ht, const uint16_t out_channels,
  ...
  for (int out_y = 0; out_y < out_ht; out_y++) {  // height loop
    ...
    for (int out_x = 0; out_x < out_wd; out_x++) {  // width_loop
      ...
      for (int out_ch_idx = 0; out_ch_idx < out_channels;
           out_ch_idx++) {  // channel_loop
        int32_t result = 0;
        ...
        for (int filter_y_idx = filter_y_start; filter_y_idx < filter_y_end;
          ...
          for (int filter_x_idx = filter_x_start; filter_x_idx < filter_x_end;
            ...
            for (int in_ch_idx = 0; in_ch_idx < in_channels; in_ch_idx++) {
              int32_t input_index =
                  (idx_y * input_wd + idx_x) * in_channels + in_ch_idx;
              int32_t filter_index =
                  ((out_ch_idx * filter_ht + filter_y_idx) * filter_wd +
                   filter_x_idx) *
                      in_channels +
                  in_ch_idx;
  ...
  for (out_y = 0; out_y < out_ht; out_y++) {
    for (out_x = 0; out_x < out_wd; out_x++) {
      for (out_ch_idx = 0; out_ch_idx < out_channels; out_ch_idx++) {
        ...
        conv_out = esp_nn_multiply_by_quantized_mult(
