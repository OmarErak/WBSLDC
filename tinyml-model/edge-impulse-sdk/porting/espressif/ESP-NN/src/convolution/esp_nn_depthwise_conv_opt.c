  ...
  for (int out_y = 0; out_y < out_ht; out_y++) {  // height loop
    ...
    for (int out_x = 0; out_x < out_wd; out_x++) {  // width_loop
      ...
      for (; ch_idx < channels - 3; ch_idx += 4) {  // channel_loop
        ...
        result0 = esp_nn_multiply_by_quantized_mult_fast(result0, *out_mult++,
                                                         *out_shift++);
        result1 = esp_nn_multiply_by_quantized_mult_fast(result1, *out_mult++,
                                                         *out_shift++);
        result2 = esp_nn_multiply_by_quantized_mult_fast(result2, *out_mult++,
                                                         *out_shift++);
        result3 = esp_nn_multiply_by_quantized_mult_fast(result3, *out_mult++,
                                                         *out_shift++);
      ...
      for (; ch_idx < channels; ch_idx++) {  // channel_loop
        ...
        result = esp_nn_multiply_by_quantized_mult_fast(result, *out_mult++,
                                                        *out_shift++);
        result += out_offset;
...
void esp_nn_depthwise_conv_s8_opt(
  ...
  if (ch_mult == 1) {
    esp_nn_depthwise_conv_s8_ch_mult_1(input_dims, input_data, filter_dims,
