...
__attribute__((noinline)) static void esp_nn_conv_s8_1x1(
  ...
  for (int32_t in_row = 0; in_row < out_ht * stride_ht; in_row += stride_ht) {
    for (int32_t in_col = 0; in_col < out_wd * stride_wd; in_col += stride_wd) {
      ...
      for (; out_ch_idx < out_channels; out_ch_idx++) {
        ...
        conv_out = esp_nn_multiply_by_quantized_mult_fast(conv_out, *out_mult++,
                                                          *out_shift++);
        conv_out += out_offset;
...
void esp_nn_conv_s8_opt(const data_dims_t *input_dims, const int8_t *input_data,
  ...
  if (filter_wd == 1 && filter_ht == 1) {
    esp_nn_conv_s8_1x1(input_dims, input_data, filter_data, bias, output_dims,
        ...
        for (filter_y_idx = filter_y_start; filter_y_idx < filter_y_end;
          ...
          for (filter_x_idx = filter_x_start; filter_x_idx < filter_x_end;
            ...
            const int8_t *filter_ptr =
                filter_data + out_ch_idx * in_channels * filter_ht * filter_wd +
