...
uint64_t ei_read_timer_ms() { return xTaskGetTickCount(); }

uint64_t ei_read_timer_us() { return xTaskGetTickCount() * 1000; }
 ...
 * @param c
 */
void ei_putchar(char c) { ei_printf("%c", c); }
...
__attribute__((weak)) void ei_free(void *ptr) { vPortFree(ptr); }
...
static void *pvPortCalloc(size_t sNb, size_t sSize) {
  ...
  if (sSize > 0) {
    ...
    if (vPtr) memset(vPtr, 0, (sSize * sNb));  // Must required
  }
  return vPtr;
}
