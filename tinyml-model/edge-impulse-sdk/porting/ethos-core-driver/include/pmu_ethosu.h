 ...
 *
 * Note: These values are symbolic. Actual HW-values may change. I.e. always use
 * API to set/get actual event-type value.
 * */
 ...
 * \return                 Cycle count
 * \note   Two HW 32-bit registers that can increment independently in-between
 * reads. To work-around raciness yet still avoid turning off the event both are
 * read as one value twice. If the latter read is not greater than the former,
 * it means overflow of LSW without incrementing MSW has occurred, in which case
 * the former value is used.
 */
 ...
 * \note   Two HW 32-bit registers that can increment independently in-between
 * reads. To work-around raciness, counter is temporary disabled if enabled.
 * \note   ETHOSU specific. Usage breaks CMSIS complience
 */
