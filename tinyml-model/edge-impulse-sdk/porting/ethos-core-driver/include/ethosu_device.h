...
enum ethosu_axi_limit_beats {
  ETHOSU_AXI_LIMIT_64_BYTES =
      0,  ///< NPU AXI limit 64 byte burst split alignment.
  ETHOSU_AXI_LIMIT_128_BYTES =
      1,  ///< NPU AXI limit 128 byte burst split alignment.
  ETHOSU_AXI_LIMIT_256_BYTES =
      2  ///< NPU AXI limit 256 byte burst split alignment.
};
...
enum ethosu_clock_q_request {
  ...
  ETHOSU_CLOCK_Q_ENABLE = 1    ///< Enable NPU signal ready for clock off when
                               ///< stop+idle state reached.
};

enum ethosu_power_q_request {
  ETHOSU_POWER_Q_DISABLE = 0,  ///< Disble NPU signal ready for power off.
  ETHOSU_POWER_Q_ENABLE = 1    ///< Enable NPU signal ready for power off when
                               ///< stop+idle state reached.
};
 ...
 * Get device id.
 */
enum ethosu_error_codes ethosu_get_id(struct ethosu_device *dev,
                                      struct ethosu_id *id);

/**
 * Get device configuration.
 */
enum ethosu_error_codes ethosu_get_config(struct ethosu_device *dev,
                                          struct ethosu_config *config);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_is_irq_raised(struct ethosu_device *dev,
                                             uint8_t *irq_status);
 ...
 * \return                         \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_get_status_mask(struct ethosu_device *dev,
 ...
 * \return                         \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_get_irq_history_mask(struct ethosu_device *dev,
 ...
 * \return                             \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_clear_irq_history_mask(
    struct ethosu_device *dev, uint16_t irq_history_clear_mask);
...
enum ethosu_error_codes ethosu_read_apb_reg(struct ethosu_device *dev,
                                            ...
                                            uint16_t num_reg, uint32_t *reg_p);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_qconfig(struct ethosu_device *dev,
                                           enum ethosu_memory_type memory_type);

/**
 * Set register REGIONCFG.
 *                             Base pointer configuration.
 *                             Bits[2*k+1:2*k] give the memory type for
 * BASEP[k]. \param[in] region           Region field to set: 0 - 7. \param[in]
 * memory_type      Memory_type to use for region: enum ethosu_memory_type.
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_regioncfg(
    struct ethosu_device *dev, uint8_t region,
    enum ethosu_memory_type memory_type);

/**
 * Set AXI limit parameters for port 0 counter 0.
 * \param[in] max_beats        Burst split alignment, \ref
 * ethosu_axi_limit_beats. \param[in] memtype          Cache policy \ref
 * ethosu_axi_limit_mem_type \param[in] max_reads        Maximum number of
 * outstanding reads. \param[in] max_writes       Maximum number of outstanding
 * writes. \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_axi_limit0(
    struct ethosu_device *dev, enum ethosu_axi_limit_beats max_beats,
    enum ethosu_axi_limit_mem_type memtype, uint8_t max_reads,
    uint8_t max_writes);
/**
 ...
 * writes. \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_axi_limit1(
    struct ethosu_device *dev, enum ethosu_axi_limit_beats max_beats,
    enum ethosu_axi_limit_mem_type memtype, uint8_t max_reads,
    uint8_t max_writes);
/**
 ...
 * writes. \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_axi_limit2(
    struct ethosu_device *dev, enum ethosu_axi_limit_beats max_beats,
    enum ethosu_axi_limit_mem_type memtype, uint8_t max_reads,
    uint8_t max_writes);
/**
 ...
 * writes. \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_axi_limit3(
    struct ethosu_device *dev, enum ethosu_axi_limit_beats max_beats,
    enum ethosu_axi_limit_mem_type memtype, uint8_t max_reads,
    uint8_t max_writes);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_get_qread(struct ethosu_device *dev,
                                         uint32_t *qread);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_get_revision(struct ethosu_device *dev,
                                            uint32_t *revision);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_get_shram_data(struct ethosu_device *dev,
                                              int section, uint32_t *shram_p);
 ...
 * \return                     \ref ethosu_error_codes
 */
enum ethosu_error_codes ethosu_set_clock_and_power(
    struct ethosu_device *dev, enum ethosu_clock_q_request clock_q,
 ...
 */
void ethosu_write_reg(struct ethosu_device *dev, uint32_t address,
                      uint32_t value);
 ...
 * \param[in] value            Value to be written.
 */
void ethosu_write_reg_shadow(struct ethosu_device *dev, uint32_t address,
                             uint32_t value, uint32_t *shadow);
