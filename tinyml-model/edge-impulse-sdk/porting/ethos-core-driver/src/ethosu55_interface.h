...
struct id_r {
 ...
 private:
  ...
  union {
    struct {
      ...
      uint32_t version_minor : 4;   // This is the n for the P part of an RnPn
                                    // release number
      uint32_t version_major : 4;   // This is the n for the R part of an RnPn
                                    // release number
      uint32_t
          product_major : 4;  // This is the X part of the ML00X product number
      uint32_t arch_patch_rev : 4;  // This is the patch number of the
                                    // architecture version a.b
      uint32_t arch_minor_rev : 8;  // This is the minor architecture version
                                    // number, b in the architecture version a.b
      uint32_t arch_major_rev : 4;  // This is the major architecture version
                                    // number, a in the architecture version a.b
    };
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  CONSTEXPR id_r()
      : version_status(static_cast<uint32_t>(1)),
        version_minor(static_cast<uint32_t>(0x0)),
        version_major(static_cast<uint32_t>(0x1)),
        product_major(static_cast<uint32_t>(4)),
        arch_patch_rev(static_cast<uint32_t>(6)),
        arch_minor_rev(static_cast<uint32_t>(0)),
        arch_major_rev(static_cast<uint32_t>(1)) {}
  CONSTEXPR id_r(uint32_t init) : word(init) {}
  CONSTEXPR void operator=(uint32_t value) { word = value; }
  void operator=(uint32_t value) volatile { word = value; }
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  id_r copy() volatile { return *this; }
...
struct status_r {
 ...
 private:
  ...
  union {
    struct {
      ...
      uint32_t irq_raised : 1;  // Raw IRQ status, 0 = IRQ not raised, 1 = IRQ
                                // raised. IRQ is cleared using command register
                                // bit 1
      uint32_t bus_status : 1;  // 0=OK, 1=Bus abort detected and processing
                                // halted (NPU will reach IDLE state and not to
                                // start process any more commands/AXI
                                // transactions). Can only be cleared by a reset
      uint32_t
          reset_status : 1;  // Reset is ongoing and only this register can be
                             // read (other registers read as 0 and writes are
                             // ignored.) A value of 0 means NPU is not being
                             // reset and can be accessed as normal
      uint32_t
          cmd_parse_error : 1;  // 0=No error 1=Command stream parsing error
                                // detected. Can only be cleared by reset
      uint32_t
          cmd_end_reached : 1;  // 0=Not reached, 1=Reached. Cleared by writing
                                // QBASE or QSIZE when NPU is in stopped state
      uint32_t pmu_irq_raised : 1;  // 0=No PMU IRQ, 1=PMU IRQ raised. Cleared
                                    // by using command register bit 1
      uint32_t
          wd_fault : 1;  // Weight decoder state: 0=no fault 1=weight decoder
                         // decompression fault. Can only be cleared by reset
      uint32_t ecc_fault : 1;  // ECC state for internal RAMs: 0=no fault 1=ECC
                               // fault signalled. Can only be cleared by reset
      uint32_t reserved0 : 2;
      uint32_t faulting_interface : 1;  // Faulting interface on bus abort.
                                        // 0=AXI-M0 1=AXI-M1
      uint32_t faulting_channel : 4;   // Faulting channel on a bus abort. Read:
                                       // 0=Cmd 1=IFM 2=Weights 3=Scale+Bias
 ...
 public:
  CONSTEXPR status_r()
      : state(static_cast<uint32_t>(::state::STOPPED)),
        irq_raised(static_cast<uint32_t>(0x0)),
        bus_status(static_cast<uint32_t>(0x0)),
        reset_status(static_cast<uint32_t>(0x1)),
        cmd_parse_error(static_cast<uint32_t>(0x0)),
        cmd_end_reached(static_cast<uint32_t>(0x0)),
        pmu_irq_raised(static_cast<uint32_t>(0x0)),
        wd_fault(static_cast<uint32_t>(0x0)),
        ecc_fault(static_cast<uint32_t>(0x0)),
        reserved0(static_cast<uint32_t>(0)),
        faulting_interface(static_cast<uint32_t>(0x0)),
        faulting_channel(static_cast<uint32_t>(0x0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  status_r copy() volatile { return *this; }
...
struct cmd_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t transition_to_running_state : 1;  // Write 1 to transition the
                                                 // NPU to running state.
                                                 // Writing 0 has no effect
      uint32_t clear_irq : 1;  // Write 1 to clear the IRQ status in the STATUS
                               // register. Writing 0 has no effect
      uint32_t clock_q_enable : 1;  // Write 1 to this bit to enable clock off
                                    // using clock q-interface and enable the
                                    // master clock gate
      uint32_t power_q_enable : 1;  // Write 1 to this bit to enable power off
                                    // using power q-interface
      uint32_t stop_request : 1;    // Write 1 to this bit to request STOP after
                                    // completing any already-started commands
 ...
 public:
  CONSTEXPR cmd_r()
      : transition_to_running_state(static_cast<uint32_t>(0x0)),
        clear_irq(static_cast<uint32_t>(0x0)),
        clock_q_enable(static_cast<uint32_t>(0x1)),
        power_q_enable(static_cast<uint32_t>(0x1)),
        stop_request(static_cast<uint32_t>(0x0)),
        reserved0(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  cmd_r copy() volatile { return *this; }
  ...
  CONSTEXPR cmd_r &set_transition_to_running_state(uint32_t value) {
    transition_to_running_state =
        ((1u << 1) - 1) & static_cast<uint32_t>(value);
    return *this;
  }
...
struct reset_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  reset_r copy() volatile { return *this; }
...
// qbase0_r - Base address of command queue bits [31:0]. The address is 4 byte
// aligned
struct qbase0_r {
#ifdef __cplusplus
 private:
#endif  //__cplusplus
  union {
    uint32_t QBASE0;  // The 4 byte aligned lower bytes of the base address
                      // value for the command stream
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  qbase0_r copy() volatile { return *this; }
...
struct qbase1_r {
...
#endif  //__cplusplus
  union {
    uint32_t QBASE1;  // The 4 byte aligned upper bytes of the base address
                      // value for the command stream
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  qbase1_r copy() volatile { return *this; }
...
// qread_r - Read offset in the command stream in bytes. Multiple of 4 in the
// range 0 to 16 MB
struct qread_r {
#ifdef __cplusplus
 private:
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  qread_r copy() volatile { return *this; }
...
// qconfig_r - AXI configuration for the command stream in the range 0-3. Same
...
struct qconfig_r {
 ...
 private:
  ...
  union {
    uint32_t
        QCONFIG;  // AXI configuration for the command stream in the range 0-3
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  qconfig_r copy() volatile { return *this; }
...
// qsize_r - Size of the command stream in bytes. Multiple of 4 in the range 0
// to 16 MB
struct qsize_r {
#ifdef __cplusplus
 private:
  ...
  union {
    uint32_t
        QSIZE;  // Size of the next command stream to be executed by the NPU
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  qsize_r copy() volatile { return *this; }
...
struct prot_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  prot_r copy() volatile { return *this; }
...
struct config_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t
          macs_per_cc : 4;  // The log2(macs/clock cycle). Valid encoding range
                            // is 5 to 8 for 32 to 256 MACs/clock cycle.
      uint32_t cmd_stream_version : 4;  // command stream version accepted by
                                        // this NPU.
 ...
 public:
  CONSTEXPR config_r()
      : macs_per_cc(static_cast<uint32_t>(0)),
        cmd_stream_version(static_cast<uint32_t>(0x0)),
        shram_size(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
        product(static_cast<uint32_t>(0)) {}
  CONSTEXPR config_r(uint32_t init) : word(init) {}
  CONSTEXPR void operator=(uint32_t value) { word = value; }
  void operator=(uint32_t value) volatile { word = value; }
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  config_r copy() volatile { return *this; }
...
struct lock_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  lock_r copy() volatile { return *this; }
...
struct regioncfg_r {
 ...
 public:
  CONSTEXPR regioncfg_r()
      : region0(
            static_cast<uint32_t>(::memory_type::AXI0_OUTSTANDING_COUNTER0)),
        region1(
            static_cast<uint32_t>(::memory_type::AXI0_OUTSTANDING_COUNTER0)),
        region2(
            static_cast<uint32_t>(::memory_type::AXI0_OUTSTANDING_COUNTER0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  regioncfg_r copy() volatile { return *this; }
...
struct axi_limit0_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t max_beats : 2;  // Burst split alignment: 0=64 bytes, 1=128
                               // bytes, 2=256 bytes, 3=reserved
      uint32_t reserved0 : 2;
      uint32_t memtype : 4;  // Memtype to be used to encode AxCACHE signals
      uint32_t reserved1 : 8;
      uint32_t max_outstanding_read_m1 : 8;   // Maximum number of outstanding
                                              // AXI read transactions - 1 in
                                              // range 0 to 31
      uint32_t max_outstanding_write_m1 : 8;  // Maximum number of outstanding
                                              // AXI write transactions - 1 in
                                              // range 0 to 15
    };
    uint32_t word;
  };
 ...
 public:
  CONSTEXPR axi_limit0_r()
      : max_beats(static_cast<uint32_t>(0x0)),
        reserved0(static_cast<uint32_t>(0)),
        memtype(static_cast<uint32_t>(
            ::axi_mem_encoding_type::DEVICE_NON_BUFFERABLE)),
        reserved1(static_cast<uint32_t>(0)),
        max_outstanding_read_m1(static_cast<uint32_t>(0x00)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  axi_limit0_r copy() volatile { return *this; }
  ...
  CONSTEXPR ::axi_mem_encoding_type get_memtype() const {
    ::axi_mem_encoding_type value =
        static_cast<::axi_mem_encoding_type>(memtype);
    return value;
  }
...
struct axi_limit1_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  axi_limit1_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_max_beats() const {
    uint32_t value = static_cast<uint32_t>(max_beats);
    return value;
  }
...
struct axi_limit2_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  axi_limit2_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_max_beats() const {
    uint32_t value = static_cast<uint32_t>(max_beats);
    return value;
  }
...
struct axi_limit3_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  axi_limit3_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_max_beats() const {
    uint32_t value = static_cast<uint32_t>(max_beats);
    return value;
  }
...
struct basep0_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep0_r copy() volatile { return *this; }
...
struct basep1_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep1_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep2_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep2_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep3_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep3_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep4_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep4_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep5_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep5_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep6_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep6_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep7_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep7_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep8_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep8_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep9_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep9_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep10_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep10_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep11_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep11_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep12_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep12_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep13_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep13_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep14_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep14_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct basep15_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  basep15_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_addr_word() const {
    uint32_t value = static_cast<uint32_t>(addr_word);
    return value;
  }
...
struct wd_status_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t core_slice_state : 2;  // STATE_HEADER=0, STATE_PALETTE=1,
                                      // STATE_WEIGHTS=2
      ...
      uint32_t
          write_buf_index0 : 3;  // current write index for next data from core
      uint32_t write_buf_valid0 : 1;  // write buf valid (full)
      uint32_t write_buf_idle0 : 1;   // write buf idle (empty)
      uint32_t
          write_buf_index1 : 3;  // current write index for next data from core
 ...
 public:
  CONSTEXPR wd_status_r()
      : core_slice_state(static_cast<uint32_t>(0)),
        core_idle(static_cast<uint32_t>(0)),
        ctrl_state(static_cast<uint32_t>(0)),
        ctrl_idle(static_cast<uint32_t>(0)),
        write_buf_index0(static_cast<uint32_t>(0)),
        write_buf_valid0(static_cast<uint32_t>(0)),
        write_buf_idle0(static_cast<uint32_t>(0)),
        write_buf_index1(static_cast<uint32_t>(0)),
        write_buf_valid1(static_cast<uint32_t>(0)),
        write_buf_idle1(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  wd_status_r copy() volatile { return *this; }
...
struct mac_status_r {
 ...
 private:
  ...
  union {
    struct {
      ...
      uint32_t wait_for_ib : 1;  // MAC is waiting for an Input Buffer to become
                                 // available
      uint32_t wait_for_acc_buf : 1;  // MAC is waiting for an Accumulator
                                      // Buffer to become available
      uint32_t wait_for_weights : 1;  // MAC is waiting for a Weight Block to
                                      // become available
      uint32_t stall_stripe : 1;      // MAC is stalling between two stripes
      uint32_t dw_sel : 1;  // Currently used weight interface in MAC AI
      uint32_t wait_for_dw0_ready : 1;  // MAC AI is waiting for MAC DPU to send
                                        // dw0_ready to WD
      uint32_t wait_for_dw1_ready : 1;  // MAC AI is waiting for MAC DPU to send
                                        // dw1_ready to WD
      uint32_t acc_buf_sel_ai : 1;  // Currently used AccBuf interface in MAC AI
      uint32_t
          wait_for_acc0_ready : 1;  // MAC AI is waiting for acc0_ready from AO
      uint32_t
          wait_for_acc1_ready : 1;  // MAC AI is waiting for acc1_ready from AO
      uint32_t acc_buf_sel_aa : 1;  // Currently used AccBuf interface in MAC
                                    // ADDER_ARRAY
 ...
 public:
  CONSTEXPR mac_status_r()
      : block_cfg_valid(static_cast<uint32_t>(0)),
        trav_en(static_cast<uint32_t>(0)),
        wait_for_ib(static_cast<uint32_t>(0)),
        wait_for_acc_buf(static_cast<uint32_t>(0)),
        wait_for_weights(static_cast<uint32_t>(0)),
        stall_stripe(static_cast<uint32_t>(0)),
        dw_sel(static_cast<uint32_t>(0)),
        wait_for_dw0_ready(static_cast<uint32_t>(0)),
        wait_for_dw1_ready(static_cast<uint32_t>(0)),
        acc_buf_sel_ai(static_cast<uint32_t>(0)),
        wait_for_acc0_ready(static_cast<uint32_t>(0)),
        wait_for_acc1_ready(static_cast<uint32_t>(0)),
        acc_buf_sel_aa(static_cast<uint32_t>(0)),
        acc0_valid(static_cast<uint32_t>(0)),
        acc1_valid(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
        events(static_cast<uint32_t>(0)),
        reserved1(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  mac_status_r copy() volatile { return *this; }
...
struct ao_status_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t cmd_sbw_valid : 1;  // Block command to shared buffer write
                                   // module is valid.
      uint32_t cmd_act_valid : 1;  // Block command to activation function
                                   // module is valid.
      ...
      uint32_t cmd_sbr_valid : 1;  // Block command to shared buffer read module
                                   // is valid.
      uint32_t
          cmd_ofm_valid : 1;  // Block command to ofm parameter module is valid.
 ...
 public:
  CONSTEXPR ao_status_r()
      : cmd_sbw_valid(static_cast<uint32_t>(0)),
        cmd_act_valid(static_cast<uint32_t>(0)),
        cmd_ctl_valid(static_cast<uint32_t>(0)),
        cmd_scl_valid(static_cast<uint32_t>(0)),
        cmd_sbr_valid(static_cast<uint32_t>(0)),
        cmd_ofm_valid(static_cast<uint32_t>(0)),
        blk_cmd_ready(static_cast<uint32_t>(0)),
        blk_cmd_valid(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
        events(static_cast<uint32_t>(0)),
        reserved1(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  ao_status_r copy() volatile { return *this; }
...
struct dma_status0_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t
          CMD_IDLE : 1;  // When this bit is high means that the CMD block is
                         // not busy in generating addresses for a CMD job.
      uint32_t IFM_IDLE : 1;  // When this bit is high means that there are no
                              // ongoing IFM jobs
      uint32_t
          WGT_IDLE_C0 : 1;  // When this bit is high means that the WGT block is
                            // not busy in generating addresses for a WGT job
      uint32_t
          BAS_IDLE_C0 : 1;  // When this bit is high means that the BAS block is
                            // not busy in generating addresses for a BAS job
      uint32_t M2M_IDLE : 1;  // When this bit is high means that there are no
                              // ongoing M2M jobs
      uint32_t OFM_IDLE : 1;  // When this bit is high means that there are no
                              // ongoing OFM jobs
      uint32_t
          HALT_REQ : 1;  // CPM has requested to HALT AXI bus before soft reset
      uint32_t HALT_ACK : 1;   // DMA is in condition to halt the AXI bus since
                               // there are no pending transactions
      uint32_t PAUSE_REQ : 1;  // CC has requested to pause the AXI
      uint32_t PAUSE_ACK : 1;  // DMA is in condition to pause the AXI bus since
                               // there are no pending transactions
      uint32_t IB0_AI_VALID_C0 : 1;  // Data for AI to be read in IFM input
                                     // buffer 0 - Core 0
      uint32_t IB0_AI_READY_C0 : 1;  // Data consumed from AI in IFM input
                                     // buffer 0 - Core 0
      uint32_t IB1_AI_VALID_C0 : 1;  // Data for AI to be read in IFM input
                                     // buffer 1 - Core 0
      uint32_t IB1_AI_READY_C0 : 1;  // Data consumed from AI in IFM input
                                     // buffer 1 - Core 0
      uint32_t IB0_AO_VALID_C0 : 1;  // Data for AO to be read in IFM input
                                     // buffer 0 - Core 0
      uint32_t IB0_AO_READY_C0 : 1;  // Data consumed from AO in IFM input
                                     // buffer 0 - Core 0
      uint32_t IB1_AO_VALID_C0 : 1;  // Data for AO to be read in IFM input
                                     // buffer 0 - Core 0
      uint32_t IB1_AO_READY_C0 : 1;  // Data consumed from AO in IFM input
                                     // buffer 1 - Core 0
      uint32_t OB0_VALID_C0 : 1;     // Data for DMA ready to be consumed in OFM
                                     // output buffer 0 -  Core 0
      uint32_t OB0_READY_C0 : 1;  // Data consumed from DMA in OFM output buffer
                                  // 0 - Core 0
      uint32_t OB1_VALID_C0 : 1;  // Data for DMA ready to be consumed in OFM
                                  // output buffer 1 -  Core 0
      uint32_t OB1_READY_C0 : 1;  // Data consumed from DMA in OFM output buffer
                                  // 1 - Core 0
      uint32_t CMD_VALID : 1;     // New command word for CC to be consumed
      uint32_t CMD_READY : 1;     // command word consumed by CC
      uint32_t WD_BITSTREAM_VALID_C0 : 1;  // New weight word for WD to be
                                           // consumed - Core 0
      uint32_t
          WD_BITSTREAM_READY_C0 : 1;  // Weight word consumed by WD - Core 0
      uint32_t BS_BITSTREAM_VALID_C0 : 1;  // New BaS word for AO to be consumed
                                           // - Core 0
      uint32_t BS_BITSTREAM_READY_C0 : 1;  // BaS word consumed by AO - Core 0
      uint32_t AXI0_AR_STALLED : 1;  // Read transfer request stalled on arready
                                     // low AXI0 (due to memory system)
      uint32_t AXI0_RD_LIMIT_STALL : 1;  // Read stalled due to one AXI0 limit
                                         // counter being reached
      uint32_t AXI0_AW_STALLED : 1;      // Write transfer request stalled on
                                     // awready low AXI0 (due to memory system)
      uint32_t AXI0_W_STALLED : 1;  // Write transfer stalled on awready low
                                    // AXI0 (due to memory system)
    };
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  CONSTEXPR dma_status0_r()
      : CMD_IDLE(static_cast<uint32_t>(0)),
        IFM_IDLE(static_cast<uint32_t>(0)),
        WGT_IDLE_C0(static_cast<uint32_t>(0)),
        BAS_IDLE_C0(static_cast<uint32_t>(0)),
        M2M_IDLE(static_cast<uint32_t>(0)),
        OFM_IDLE(static_cast<uint32_t>(0)),
        HALT_REQ(static_cast<uint32_t>(0)),
        HALT_ACK(static_cast<uint32_t>(0)),
        PAUSE_REQ(static_cast<uint32_t>(0)),
        PAUSE_ACK(static_cast<uint32_t>(0)),
        IB0_AI_VALID_C0(static_cast<uint32_t>(0)),
        IB0_AI_READY_C0(static_cast<uint32_t>(0)),
        IB1_AI_VALID_C0(static_cast<uint32_t>(0)),
        IB1_AI_READY_C0(static_cast<uint32_t>(0)),
        IB0_AO_VALID_C0(static_cast<uint32_t>(0)),
        IB0_AO_READY_C0(static_cast<uint32_t>(0)),
        IB1_AO_VALID_C0(static_cast<uint32_t>(0)),
        IB1_AO_READY_C0(static_cast<uint32_t>(0)),
        OB0_VALID_C0(static_cast<uint32_t>(0)),
        OB0_READY_C0(static_cast<uint32_t>(0)),
        OB1_VALID_C0(static_cast<uint32_t>(0)),
        OB1_READY_C0(static_cast<uint32_t>(0)),
        CMD_VALID(static_cast<uint32_t>(0)),
        CMD_READY(static_cast<uint32_t>(0)),
        WD_BITSTREAM_VALID_C0(static_cast<uint32_t>(0)),
        WD_BITSTREAM_READY_C0(static_cast<uint32_t>(0)),
        BS_BITSTREAM_VALID_C0(static_cast<uint32_t>(0)),
        BS_BITSTREAM_READY_C0(static_cast<uint32_t>(0)),
        AXI0_AR_STALLED(static_cast<uint32_t>(0)),
        AXI0_RD_LIMIT_STALL(static_cast<uint32_t>(0)),
        AXI0_AW_STALLED(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  dma_status0_r copy() volatile { return *this; }
...
struct dma_status1_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t AXI0_WR_LIMIT_STALL : 1;  // Write stalled due to one AXI0 limit
                                         // counter being reached
      uint32_t AXI1_AR_STALLED : 1;  // Read transfer request stalled on arready
                                     // low AXI1 (due to memory system)
      uint32_t AXI1_RD_LIMIT_STALL : 1;  // Read stalled due to one AXI1 limit
                                         // counter being reached
      uint32_t AXI1_WR_STALLED : 1;      // Write transfer request stalled on
                                     // awready low AXI1 (due to memory system)
      uint32_t AXI1_W_STALLED : 1;  // Write transfer stalled on wready low AXI1
                                    // (due to memory system)
      uint32_t AXI1_WR_LIMIT_STALL : 1;  // Write stalled due to one AXI1 limit
                                         // counter being reached
      uint32_t
          WGT_IDLE_C1 : 1;  // When this bit is high means that the WGT block is
                            // not busy in generating addresses for a WGT job
      uint32_t
          BAS_IDLE_C1 : 1;  // When this bit is high means that the BAS block is
                            // not busy in generating addresses for a BAS job.
      uint32_t IB0_AI_VALID_C1 : 1;  // Data for AI to be read in IFM input
                                     // buffer 0 - Core 1
      uint32_t IB0_AI_READY_C1 : 1;  // Data consumed from AI in IFM input
                                     // buffer 0 - Core 1
      uint32_t IB1_AI_VALID_C1 : 1;  // Data for AI to be read in IFM input
                                     // buffer 1 - Core 1
      uint32_t IB1_AI_READY_C1 : 1;  // Data consumed from AI in IFM input
                                     // buffer 1 - Core 1
      uint32_t IB0_AO_VALID_C1 : 1;  // Data for AO to be read in IFM input
                                     // buffer 0 - Core 1
      uint32_t IB0_AO_READY_C1 : 1;  // Data consumed from AO in IFM input
                                     // buffer 0 - Core 1
      uint32_t IB1_AO_VALID_C1 : 1;  // Data for AO to be read in IFM input
                                     // buffer 0 - Core 1
      uint32_t IB1_AO_READY_C1 : 1;  // Data consumed from AO in IFM input
                                     // buffer 1 - Core 1
      uint32_t OB0_VALID_C1 : 1;     // Data for DMA ready to be consumed in OFM
                                     // output buffer 0 - Core 1
      uint32_t OB0_READY_C1 : 1;  // Data consumed from DMA in OFM output buffer
                                  // 0 - Core 1
      uint32_t OB1_VALID_C1 : 1;  // Data for DMA ready to be consumed in OFM
                                  // output buffer 1 - Core 1
      uint32_t OB1_READY_C1 : 1;  // Data consumed from DMA in OFM output buffer
                                  // 1 - Core 1
      uint32_t WD_BITSTREAM_VALID_C1 : 1;  // New weight word for WD to be
                                           // consumed - Core 1
      uint32_t
          WD_BITSTREAM_READY_C1 : 1;  // Weight word consumed by WD - Core 1
      uint32_t BS_BITSTREAM_VALID_C1 : 1;  // New BaS word for AO to be consumed
                                           // - Core 1
 ...
 public:
  CONSTEXPR dma_status1_r()
      : AXI0_WR_LIMIT_STALL(static_cast<uint32_t>(0)),
        AXI1_AR_STALLED(static_cast<uint32_t>(0)),
        AXI1_RD_LIMIT_STALL(static_cast<uint32_t>(0)),
        AXI1_WR_STALLED(static_cast<uint32_t>(0)),
        AXI1_W_STALLED(static_cast<uint32_t>(0)),
        AXI1_WR_LIMIT_STALL(static_cast<uint32_t>(0)),
        WGT_IDLE_C1(static_cast<uint32_t>(0)),
        BAS_IDLE_C1(static_cast<uint32_t>(0)),
        IB0_AI_VALID_C1(static_cast<uint32_t>(0)),
        IB0_AI_READY_C1(static_cast<uint32_t>(0)),
        IB1_AI_VALID_C1(static_cast<uint32_t>(0)),
        IB1_AI_READY_C1(static_cast<uint32_t>(0)),
        IB0_AO_VALID_C1(static_cast<uint32_t>(0)),
        IB0_AO_READY_C1(static_cast<uint32_t>(0)),
        IB1_AO_VALID_C1(static_cast<uint32_t>(0)),
        IB1_AO_READY_C1(static_cast<uint32_t>(0)),
        OB0_VALID_C1(static_cast<uint32_t>(0)),
        OB0_READY_C1(static_cast<uint32_t>(0)),
        OB1_VALID_C1(static_cast<uint32_t>(0)),
        OB1_READY_C1(static_cast<uint32_t>(0)),
        WD_BITSTREAM_VALID_C1(static_cast<uint32_t>(0)),
        WD_BITSTREAM_READY_C1(static_cast<uint32_t>(0)),
        BS_BITSTREAM_VALID_C1(static_cast<uint32_t>(0)),
        BS_BITSTREAM_READY_C1(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  dma_status1_r copy() volatile { return *this; }
...
struct clkforce_r {
 ...
 public:
  CONSTEXPR clkforce_r()
      : top_level_clk(static_cast<uint32_t>(0)),
        cc_clk(static_cast<uint32_t>(0)),
        dma_clk(static_cast<uint32_t>(0)),
        mac_clk(static_cast<uint32_t>(0)),
        ao_clk(static_cast<uint32_t>(0)),
        wd_clk(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  clkforce_r copy() volatile { return *this; }
...
struct pid4_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid4_r copy() volatile { return *this; }
...
struct pid5_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid5_r copy() volatile { return *this; }
...
struct pid6_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid6_r copy() volatile { return *this; }
...
struct pid7_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid7_r copy() volatile { return *this; }
...
struct pid0_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid0_r copy() volatile { return *this; }
...
struct pid1_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid1_r copy() volatile { return *this; }
...
struct pid2_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid2_r copy() volatile { return *this; }
...
struct pid3_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pid3_r copy() volatile { return *this; }
...
struct cid0_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  cid0_r copy() volatile { return *this; }
...
struct cid1_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  cid1_r copy() volatile { return *this; }
...
struct cid2_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  cid2_r copy() volatile { return *this; }
...
struct cid3_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  cid3_r copy() volatile { return *this; }
...
struct pmcr_r {
 ...
 private:
  ...
  union {
    struct {
      ...
      uint32_t mask_en : 1;  // PMU can be enabled/disabled by command stream
                             // operation NPU_OP_PMU_MASK
 ...
 public:
  CONSTEXPR pmcr_r()
      : cnt_en(static_cast<uint32_t>(0x0)),
        event_cnt_rst(static_cast<uint32_t>(0)),
        cycle_cnt_rst(static_cast<uint32_t>(0)),
        mask_en(static_cast<uint32_t>(0x0)),
        reserved0(static_cast<uint32_t>(0)),
        num_event_cnt(static_cast<uint32_t>(0x04)),
        reserved1(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmcr_r copy() volatile { return *this; }
...
struct pmcntenset_r {
 ...
 public:
  CONSTEXPR pmcntenset_r()
      : EVENT_CNT_0(static_cast<uint32_t>(0)),
        EVENT_CNT_1(static_cast<uint32_t>(0)),
        EVENT_CNT_2(static_cast<uint32_t>(0)),
        EVENT_CNT_3(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmcntenset_r copy() volatile { return *this; }
...
struct pmcntenclr_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmcntenclr_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_EVENT_CNT_0() const {
    uint32_t value = static_cast<uint32_t>(EVENT_CNT_0);
    return value;
  }
...
struct pmovsset_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t
          EVENT_CNT_0_OVF : 1;  // Event counter overflow set bit for PMEVCNTR0
      uint32_t
          EVENT_CNT_1_OVF : 1;  // Event counter overflow set bit for PMEVCNTR1
      uint32_t
          EVENT_CNT_2_OVF : 1;  // Event counter overflow set bit for PMEVCNTR2
      uint32_t
          EVENT_CNT_3_OVF : 1;  // Event counter overflow set bit for PMEVCNTR3
      uint32_t reserved0 : 27;
 ...
 public:
  CONSTEXPR pmovsset_r()
      : EVENT_CNT_0_OVF(static_cast<uint32_t>(0)),
        EVENT_CNT_1_OVF(static_cast<uint32_t>(0)),
        EVENT_CNT_2_OVF(static_cast<uint32_t>(0)),
        EVENT_CNT_3_OVF(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmovsset_r copy() volatile { return *this; }
...
struct pmovsclr_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t EVENT_CNT_0_OVF : 1;  // Event counter overflow clear bit for
                                     // PMEVCNTR0
      uint32_t EVENT_CNT_1_OVF : 1;  // Event counter overflow clear bit for
                                     // PMEVCNTR1
      uint32_t EVENT_CNT_2_OVF : 1;  // Event counter overflow clear bit for
                                     // PMEVCNTR2
      uint32_t EVENT_CNT_3_OVF : 1;  // Event counter overflow clear bit for
                                     // PMEVCNTR3
      uint32_t reserved0 : 27;
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmovsclr_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_EVENT_CNT_0_OVF() const {
    uint32_t value = static_cast<uint32_t>(EVENT_CNT_0_OVF);
    return value;
  }
...
struct pmintset_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t EVENT_CNT_0_INT : 1;  // Event counter overflow interrupt request
                                     // enable bit for PMEVCNTR0
      uint32_t EVENT_CNT_1_INT : 1;  // Event counter overflow interrupt request
                                     // enable bit for PMEVCNTR1
      uint32_t EVENT_CNT_2_INT : 1;  // Event counter overflow interrupt request
                                     // enable bit for PMEVCNTR2
      uint32_t EVENT_CNT_3_INT : 1;  // Event counter overflow interrupt request
                                     // enable bit for PMEVCNTR3
      uint32_t reserved0 : 27;
      uint32_t
          CYCLE_CNT_INT : 1;  // PMCCNTR overflow interrupt request enable bit
    };
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  CONSTEXPR pmintset_r()
      : EVENT_CNT_0_INT(static_cast<uint32_t>(0)),
        EVENT_CNT_1_INT(static_cast<uint32_t>(0)),
        EVENT_CNT_2_INT(static_cast<uint32_t>(0)),
        EVENT_CNT_3_INT(static_cast<uint32_t>(0)),
        reserved0(static_cast<uint32_t>(0)),
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmintset_r copy() volatile { return *this; }
...
struct pmintclr_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t EVENT_CNT_0_INT : 1;  // Event counter overflow interrupt request
                                     // disable bit for PMEVCNTR0
      uint32_t EVENT_CNT_1_INT : 1;  // Event counter overflow interrupt request
                                     // disable bit for PMEVCNTR1
      uint32_t EVENT_CNT_2_INT : 1;  // Event counter overflow interrupt request
                                     // disable bit for PMEVCNTR2
      uint32_t EVENT_CNT_3_INT : 1;  // Event counter overflow interrupt request
                                     // disable bit for PMEVCNTR3
      uint32_t reserved0 : 27;
      uint32_t
          CYCLE_CNT_INT : 1;  // PMCCNTR overflow interrupt request disable bit
    };
    uint32_t word;
  };
#ifdef __cplusplus
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmintclr_r copy() volatile { return *this; }
  CONSTEXPR uint32_t get_EVENT_CNT_0_INT() const {
    uint32_t value = static_cast<uint32_t>(EVENT_CNT_0_INT);
    return value;
  }
...
struct pmccntr_lo_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmccntr_lo_r copy() volatile { return *this; }
...
struct pmccntr_hi_r {
 ...
 public:
  CONSTEXPR pmccntr_hi_r()
      : CYCLE_CNT_HI(static_cast<uint32_t>(0x0000)),
        reserved0(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmccntr_hi_r copy() volatile { return *this; }
...
struct pmccntr_cfg_r {
 ...
 public:
  CONSTEXPR pmccntr_cfg_r()
      : CYCLE_CNT_CFG_START(static_cast<uint32_t>(0x00)),
        reserved0(static_cast<uint32_t>(0)),
        CYCLE_CNT_CFG_STOP(static_cast<uint32_t>(0x00)),
        reserved1(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmccntr_cfg_r copy() volatile { return *this; }
...
struct pmcaxi_chan_r {
 ...
 private:
  ...
  union {
    struct {
      uint32_t CH_SEL : 4;  // Channel number to monitor for latency
                            // measurements (Read: 0=Cmd 1=IFM 2=Weights
                            // 3=Scale+Bias 4=Mem2Mem; Write: 8=OFM 9=Mem2Mem)
      uint32_t reserved0 : 4;
      uint32_t AXI_CNT_SEL : 2;  // AXI counter to monitor for latency
                                 // measurements (0=AXI0 counter0, 1=AXI0
                                 // counter1, 2=AXI1 counter 2, 3=AXI counter3)
      uint32_t
          BW_CH_SEL_EN : 1;  // Bandwidth channel selector enable: {0=AXI bw
                             // events measured for all channels, 1=AXI bw
                             // events measured for channel specified by CH_SEL
 ...
 public:
  CONSTEXPR pmcaxi_chan_r()
      : CH_SEL(static_cast<uint32_t>(0x0)),
        reserved0(static_cast<uint32_t>(0)),
        AXI_CNT_SEL(static_cast<uint32_t>(0x000000)),
        BW_CH_SEL_EN(static_cast<uint32_t>(0x000000)),
        reserved1(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmcaxi_chan_r copy() volatile { return *this; }
...
struct pmevtyper0_r {
 ...
 public:
  CONSTEXPR pmevtyper0_r()
      : EV_TYPE(static_cast<uint32_t>(::pmu_event_type::NO_EVENT)),
        reserved0(static_cast<uint32_t>(0)) {}
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmevtyper0_r copy() volatile { return *this; }
...
struct pmevtyper1_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmevtyper1_r copy() volatile { return *this; }
  CONSTEXPR ::pmu_event_type get_EV_TYPE() const {
    ::pmu_event_type value = static_cast<::pmu_event_type>(EV_TYPE);
    return value;
  }
...
struct pmevtyper2_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmevtyper2_r copy() volatile { return *this; }
  CONSTEXPR ::pmu_event_type get_EV_TYPE() const {
    ::pmu_event_type value = static_cast<::pmu_event_type>(EV_TYPE);
    return value;
  }
...
struct pmevtyper3_r {
 ...
 public:
  ...
  CONSTEXPR operator uint32_t() { return word; }
  operator uint32_t() volatile { return word; }
  pmevtyper3_r copy() volatile { return *this; }
  CONSTEXPR ::pmu_event_type get_EV_TYPE() const {
    ::pmu_event_type value = static_cast<::pmu_event_type>(EV_TYPE);
    return value;
  }
...
struct NPU_REG {
  ...
  void reset() {
    ...
    for (size_t i = 0; i < (sizeof(PMEVTYPER) / sizeof(PMEVTYPER[0])); ++i)
      PMEVTYPER[i] = 0;
    for (size_t i = 0; i < (sizeof(SHARED_BUFFER) / sizeof(SHARED_BUFFER[0]));
         ++i)
      SHARED_BUFFER[i] = 0;
  }
  ...
  enum class access_type_t : bool { RO, RW };
...
struct command_no_payload_t {
  ...
  uint32_t param : 16;
#ifdef __cplusplus
  CONSTEXPR bool valid() const { return must_be_zero0 == 0; }
  CONSTEXPR void init() { must_be_zero0 = 0; }
  ...
  CONSTEXPR uint32_t get_param() const { return static_cast<uint32_t>(param); }
...
struct command_with_payload_t {
  ...
  CONSTEXPR uint32_t get_data() const { return static_cast<uint32_t>(data); }
...
// Move to stopped state once all commands to this point are done. Raise IRQ to
// the host and logically OR the mask into the status register upper 16 bits
...
struct npu_op_stop_t {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_STOP) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
  ...
  CONSTEXPR uint32_t get_mask() const { return static_cast<uint32_t>(mask); }
...
// Raise IRQ to the host and logically OR the mask into the status register
// upper 16 bits (see the status register)
struct npu_op_irq_t {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_IRQ) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_CONV) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_DEPTHWISE) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_POOL) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_ELEMENTWISE) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// Queue new DMA for the given channel with the given mode. Mode bit 0 specifies
// the source address type 0=external, 1=internal Mode bit 1 specifies the
// destination address type 0=external, 1=internal In Ethos-U55 there is only
// one user channel so channel=0. If the channel is fully in use then the
// command blocks until a new DMA can start
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_DMA_START) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// In Ethos-U55 there is only one user channel so channel=0. In Ethos-U55 there
// is only one descriptor per channel so k=0 and the command waits for the
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_DMA_WAIT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// Wait for n or fewer kernel operations to be remaining (not complete) before
// starting the next command. A kernel operation is Conv, Depthwise, Pool,
// VectorProd Elementwise. This command is typically placed before an
// NPU_OP_DMA_START command to prevent the DMA from starting until a previous
// kernel operation reading the memory has completed.
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_KERNEL_WAIT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_OP_PMU_MASK) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_PAD_TOP) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_PAD_LEFT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_PAD_RIGHT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_PAD_BOTTOM) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_DEPTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_PRECISION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_UPSCALE) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_ZERO_POINT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_WIDTH0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_HEIGHT0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_HEIGHT1_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_IB_END) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_WIDTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_HEIGHT_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_DEPTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
struct npu_set_ofm_precision_t {
  ...
  uint32_t format : 2;
  uint32_t scaling : 1;  // 0=Per channel scale/bias 1=Global scale
                         // (SET_OFM_SCALE), no bias
  uint32_t reserved1 : 5;
  uint32_t rounding : 2;  // 0=TFL rounding 1=truncate towards zero 2=natural
                          // rounding
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_PRECISION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_BLK_WIDTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_BLK_HEIGHT_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_BLK_DEPTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_ZERO_POINT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_WIDTH0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_HEIGHT0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_HEIGHT1_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_OFM_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_KERNEL_WIDTH_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_KERNEL_HEIGHT_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// Kernel stride b0=(X stride-1)&1, b1=(Y stride-1)&1, b2=weight order (0=depth,
// 1=kernel) b3 = kernel_x_dilation - 1 (0=no x dilation, 1=x dilation of x2) b4
// = kernel_y_dilation -1 (0=no y dilation, 1=y dilation of x2) b5 = kernel
// decomposition size (0 for kernel_split_size=8, 1 for kernel_split_size=4)
// b[8:6] = (X stride-1)>>1 b[11:9] = (Y stride-1)>>1
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_KERNEL_STRIDE) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// 0=1-core, 1=2-core depth (this command is Ethos-U65 only and UNPREDICTABLE
// for Ethos-U55)
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_PARALLEL_MODE) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_ACC_FORMAT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_ACTIVATION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_ACTIVATION_MIN) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_ACTIVATION_MAX) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// Index n for weight access: BasePointer[n] is added to all Weight stream
// offsets
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_WEIGHT_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
// Index n for weight access: BasePointer[n] is added to all scale stream
// offsets
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_SCALE_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_AB_START) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_BLOCKDEP) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
struct npu_set_dma0_src_region_t {
  ...
  uint32_t must_be_zero0 : 6;  // 0
  uint32_t region : 8;  // If Bit[8]=0, Bit[7:0]=Region number in the range [0,
                        // 8) of SRC offset. If Bit[8]=1, Bit[7:0]=Core number
                        // (0 or 1) to read.
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_DMA0_SRC_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
struct npu_set_dma0_dst_region_t {
  ...
  uint32_t must_be_zero0 : 6;  // 0
  uint32_t region : 8;  // If Bit[8]=0, Bit[7:0]=Region number in the range [0,
                        // 8) of DST offset. If Bit[8]=1, Bit[7:0]=Core mask to
                        // write to (bit k set for core k=0,1).
  uint32_t internal : 1;     // Select external/internal=0/1
  uint32_t stride_mode : 2;  // stride mode 0/1/2=1D/2D/3D
  uint32_t reserved0 : 5;
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_DMA0_DST_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_DMA0_SIZE0) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_DMA0_SIZE1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_BROADCAST) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_SCALAR) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_PRECISION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_ZERO_POINT) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_WIDTH0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_HEIGHT0_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_HEIGHT1_M1) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_IB_START) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd0::NPU_SET_IFM2_REGION) &&
           must_be_zero0 == 0;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_BASE0) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_BASE1) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_BASE2) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_BASE3) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_STRIDE_X) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_STRIDE_Y) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm_stride_c_t {
  ...
  uint32_t payload_size : 2;  // Min:1 Max:2
  uint32_t reserved0 : 16;
  uint32_t data : 32;  // IFM byte stride between channel blocks (of 16 bytes
                       // each block)
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM_STRIDE_C) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_BASE0) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_BASE1) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_BASE2) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_BASE3) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_STRIDE_X) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_STRIDE_Y) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ofm_stride_c_t {
  ...
  uint32_t payload_size : 2;  // Min:1 Max:2
  uint32_t reserved0 : 16;
  uint32_t data : 32;  // OFM byte stride between channel blocks (of 16 bytes
                       // each block)
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_STRIDE_C) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_WEIGHT_BASE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_WEIGHT_LENGTH) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_SCALE_BASE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_SCALE_LENGTH) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
// Set scale (32-bit). Used by average pool with pad=0, elementwise MUL, ADD,
// SUB
struct npu_set_ofm_scale_t {
  ...
  uint32_t payload_size : 2;  // Min:1 Max:2
  uint32_t shift : 16;
  uint32_t data : 32;  // scale (32-bit). Used by average pool with pad=0,
                       // elementwise MUL, ADD, SUB
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OFM_SCALE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
  ...
  CONSTEXPR uint32_t get_shift() const { return static_cast<uint32_t>(shift); }
...
// Set scale (32-bit) used for elementwise ADD/SUB OPA prescale. If IFM scale
// mode is 0 then shift is ignored and scale is 16-bit. If IFM scale mode is 1
// or 2 then shift is 6-bit and scale is 32-bit
struct npu_set_opa_scale_t {
  uint32_t cmd_code : 10;     // NPU_SET_OPA_SCALE
  uint32_t must_be_zero : 4;  // 0
  uint32_t payload_size : 2;  // Min:1 Max:2
  uint32_t shift : 16;
  uint32_t data : 32;  // scale (32-bit) used for elementwise ADD/SUB OPA
                       // prescale. If IFM scale mode is 0 then shift is ignored
                       // and scale is 16-bit. If IFM scale mode is 1 or 2 then
                       // shift is 6-bit and scale is 32-bit
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OPA_SCALE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
// Set scale (16-bit) used for elementwise ADD/SUB OPB prescale. If IFM scale
// mode is 0 then scale is 16-bit. If IFM scale mode is 1 or 2 then this
...
struct npu_set_opb_scale_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // scale (16-bit) used for elementwise ADD/SUB OPB prescale.
                  // If IFM scale mode is 0 then scale is 16-bit. If IFM scale
                  // mode is 1 or 2 then this register is not used
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_OPB_SCALE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_DMA0_SRC) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_DMA0_DST) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_DMA0_LEN) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_DMA0_SKIP0) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_DMA0_SKIP1) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm2_base0_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // IFM2 tile0 offset (top left tile) from IFM_REGION start
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_BASE0) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm2_base1_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // IFM2 tile1 offset (top right tile) from IFM_REGION start
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_BASE1) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm2_base2_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // IFM2 tile2 offset (bottom left tile) from IFM_REGION start
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_BASE2) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm2_base3_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // IFM2 tile3 offset (bottom right tile) from IFM_REGION start
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_BASE3) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_STRIDE_X) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_STRIDE_Y) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_ifm2_stride_c_t {
  ...
  uint32_t payload_size : 2;  // Min:1 Max:2
  uint32_t reserved0 : 16;
  uint32_t data : 32;  // IFM2 byte stride between channel blocks (of 16 bytes
                       // each block)
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_IFM2_STRIDE_C) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_WEIGHT1_BASE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_WEIGHT1_LENGTH) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_scale1_base_t {
  ...
  uint32_t param : 16;
  uint32_t
      data : 32;  // Scale and bias stream input byte offset from SCALE_REGION
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_SCALE1_BASE) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
struct npu_set_scale1_length_t {
  ...
  uint32_t reserved0 : 16;
  uint32_t
      data : 32;  // Scale and bias stream input byte length (unsigned 20 bits)
#ifdef __cplusplus
  CONSTEXPR bool valid() const {
    return cmd_code == static_cast<uint32_t>(cmd1::NPU_SET_SCALE1_LENGTH) &&
           must_be_zero == 0 && payload_size >= 1 && payload_size <= 2;
  }
  CONSTEXPR void init() {
...
#define EXPAND_ACC_FORMAT(FUNC, SEP)                          \
  FUNC(acc_format, INT_32BIT) SEP FUNC(acc_format, INT_40BIT) \
  SEP FUNC(acc_format, FP_S5_10)

#define EXPAND_ACTIVATION(FUNC, SEP) \
  FUNC(activation, NONE)             \
  SEP FUNC(activation, TANH)         \
  SEP FUNC(activation, SIGMOID)      \
  SEP FUNC(activation, LUT_START)    \
...
#define EXPAND_CLIP_RANGE(FUNC, SEP) \
  ...
  SEP FUNC(clip_range, FORCE_UINT8)  \
  SEP FUNC(clip_range, FORCE_INT8)   \
  SEP FUNC(clip_range, FORCE_INT16)

#define EXPAND_CMD0(FUNC, SEP)              \
  FUNC(cmd0, NPU_OP_STOP)                   \
  SEP FUNC(cmd0, NPU_OP_IRQ)                \
  SEP FUNC(cmd0, NPU_OP_CONV)               \
  SEP FUNC(cmd0, NPU_OP_DEPTHWISE)          \
  SEP FUNC(cmd0, NPU_OP_POOL)               \
  SEP FUNC(cmd0, NPU_OP_ELEMENTWISE)        \
  SEP FUNC(cmd0, NPU_OP_DMA_START)          \
  SEP FUNC(cmd0, NPU_OP_DMA_WAIT)           \
  SEP FUNC(cmd0, NPU_OP_KERNEL_WAIT)        \
  SEP FUNC(cmd0, NPU_OP_PMU_MASK)           \
  SEP FUNC(cmd0, NPU_SET_IFM_PAD_TOP)       \
  SEP FUNC(cmd0, NPU_SET_IFM_PAD_LEFT)      \
  SEP FUNC(cmd0, NPU_SET_IFM_PAD_RIGHT)     \
  SEP FUNC(cmd0, NPU_SET_IFM_PAD_BOTTOM)    \
  SEP FUNC(cmd0, NPU_SET_IFM_DEPTH_M1)      \
  SEP FUNC(cmd0, NPU_SET_IFM_PRECISION)     \
  SEP FUNC(cmd0, NPU_SET_IFM_UPSCALE)       \
  SEP FUNC(cmd0, NPU_SET_IFM_ZERO_POINT)    \
  SEP FUNC(cmd0, NPU_SET_IFM_WIDTH0_M1)     \
  SEP FUNC(cmd0, NPU_SET_IFM_HEIGHT0_M1)    \
  SEP FUNC(cmd0, NPU_SET_IFM_HEIGHT1_M1)    \
  SEP FUNC(cmd0, NPU_SET_IFM_IB_END)        \
  SEP FUNC(cmd0, NPU_SET_IFM_REGION)        \
  SEP FUNC(cmd0, NPU_SET_OFM_WIDTH_M1)      \
  SEP FUNC(cmd0, NPU_SET_OFM_HEIGHT_M1)     \
  SEP FUNC(cmd0, NPU_SET_OFM_DEPTH_M1)      \
  SEP FUNC(cmd0, NPU_SET_OFM_PRECISION)     \
  SEP FUNC(cmd0, NPU_SET_OFM_BLK_WIDTH_M1)  \
  SEP FUNC(cmd0, NPU_SET_OFM_BLK_HEIGHT_M1) \
  SEP FUNC(cmd0, NPU_SET_OFM_BLK_DEPTH_M1)  \
  SEP FUNC(cmd0, NPU_SET_OFM_ZERO_POINT)    \
  SEP FUNC(cmd0, NPU_SET_OFM_WIDTH0_M1)     \
  SEP FUNC(cmd0, NPU_SET_OFM_HEIGHT0_M1)    \
  SEP FUNC(cmd0, NPU_SET_OFM_HEIGHT1_M1)    \
  SEP FUNC(cmd0, NPU_SET_OFM_REGION)        \
  SEP FUNC(cmd0, NPU_SET_KERNEL_WIDTH_M1)   \
  SEP FUNC(cmd0, NPU_SET_KERNEL_HEIGHT_M1)  \
  SEP FUNC(cmd0, NPU_SET_KERNEL_STRIDE)     \
  SEP FUNC(cmd0, NPU_SET_PARALLEL_MODE)     \
  SEP FUNC(cmd0, NPU_SET_ACC_FORMAT)        \
  SEP FUNC(cmd0, NPU_SET_ACTIVATION)        \
  SEP FUNC(cmd0, NPU_SET_ACTIVATION_MIN)    \
  SEP FUNC(cmd0, NPU_SET_ACTIVATION_MAX)    \
  SEP FUNC(cmd0, NPU_SET_WEIGHT_REGION)     \
  SEP FUNC(cmd0, NPU_SET_SCALE_REGION)      \
  SEP FUNC(cmd0, NPU_SET_AB_START)          \
  SEP FUNC(cmd0, NPU_SET_BLOCKDEP)          \
  SEP FUNC(cmd0, NPU_SET_DMA0_SRC_REGION)   \
  SEP FUNC(cmd0, NPU_SET_DMA0_DST_REGION)   \
  SEP FUNC(cmd0, NPU_SET_DMA0_SIZE0)        \
  SEP FUNC(cmd0, NPU_SET_DMA0_SIZE1)        \
  SEP FUNC(cmd0, NPU_SET_IFM2_BROADCAST)    \
  SEP FUNC(cmd0, NPU_SET_IFM2_SCALAR)       \
  SEP FUNC(cmd0, NPU_SET_IFM2_PRECISION)    \
  SEP FUNC(cmd0, NPU_SET_IFM2_ZERO_POINT)   \
  SEP FUNC(cmd0, NPU_SET_IFM2_WIDTH0_M1)    \
  SEP FUNC(cmd0, NPU_SET_IFM2_HEIGHT0_M1)   \
...
#define EXPAND_CMD1(FUNC, SEP)           \
  ...
  SEP FUNC(cmd1, NPU_SET_IFM_BASE1)      \
  SEP FUNC(cmd1, NPU_SET_IFM_BASE2)      \
  SEP FUNC(cmd1, NPU_SET_IFM_BASE3)      \
  SEP FUNC(cmd1, NPU_SET_IFM_STRIDE_X)   \
  SEP FUNC(cmd1, NPU_SET_IFM_STRIDE_Y)   \
  SEP FUNC(cmd1, NPU_SET_IFM_STRIDE_C)   \
  SEP FUNC(cmd1, NPU_SET_OFM_BASE0)      \
  SEP FUNC(cmd1, NPU_SET_OFM_BASE1)      \
  SEP FUNC(cmd1, NPU_SET_OFM_BASE2)      \
  SEP FUNC(cmd1, NPU_SET_OFM_BASE3)      \
  SEP FUNC(cmd1, NPU_SET_OFM_STRIDE_X)   \
  SEP FUNC(cmd1, NPU_SET_OFM_STRIDE_Y)   \
  SEP FUNC(cmd1, NPU_SET_OFM_STRIDE_C)   \
  SEP FUNC(cmd1, NPU_SET_WEIGHT_BASE)    \
  SEP FUNC(cmd1, NPU_SET_WEIGHT_LENGTH)  \
  SEP FUNC(cmd1, NPU_SET_SCALE_BASE)     \
  SEP FUNC(cmd1, NPU_SET_SCALE_LENGTH)   \
  SEP FUNC(cmd1, NPU_SET_OFM_SCALE)      \
  SEP FUNC(cmd1, NPU_SET_OPA_SCALE)      \
  SEP FUNC(cmd1, NPU_SET_OPB_SCALE)      \
  SEP FUNC(cmd1, NPU_SET_DMA0_SRC)       \
  SEP FUNC(cmd1, NPU_SET_DMA0_DST)       \
  SEP FUNC(cmd1, NPU_SET_DMA0_LEN)       \
  SEP FUNC(cmd1, NPU_SET_DMA0_SKIP0)     \
  SEP FUNC(cmd1, NPU_SET_DMA0_SKIP1)     \
  SEP FUNC(cmd1, NPU_SET_IFM2_BASE0)     \
  SEP FUNC(cmd1, NPU_SET_IFM2_BASE1)     \
  SEP FUNC(cmd1, NPU_SET_IFM2_BASE2)     \
  SEP FUNC(cmd1, NPU_SET_IFM2_BASE3)     \
...
#define EXPAND_DATA_FORMAT(FUNC, SEP) \
  FUNC(data_format, NHWC) SEP FUNC(data_format, NHCWB16)

#define EXPAND_ELEMENTWISE_MODE(FUNC, SEP) \
  FUNC(elementwise_mode, MUL)              \
  SEP FUNC(elementwise_mode, ADD)          \
  SEP FUNC(elementwise_mode, SUB)          \
  SEP FUNC(elementwise_mode, MIN)          \
  SEP FUNC(elementwise_mode, MAX)          \
  SEP FUNC(elementwise_mode, LRELU)        \
  SEP FUNC(elementwise_mode, ABS)          \
  SEP FUNC(elementwise_mode, CLZ)          \
  SEP FUNC(elementwise_mode, SHR)          \
  SEP FUNC(elementwise_mode, SHL)

#define EXPAND_IFM_PRECISION(FUNC, SEP) \
  FUNC(ifm_precision, U8)               \
  SEP FUNC(ifm_precision, S8)           \
  SEP FUNC(ifm_precision, U16)          \
  SEP FUNC(ifm_precision, S16)          \
  SEP FUNC(ifm_precision, S32)

#define EXPAND_IFM_SCALE_MODE(FUNC, SEP)    \
  FUNC(ifm_scale_mode, SCALE_16BIT)         \
  SEP FUNC(ifm_scale_mode, SCALE_OPA_32BIT) \
  SEP FUNC(ifm_scale_mode, SCALE_OPB_32BIT)

#define EXPAND_MACS_PER_CC(FUNC, SEP)     \
  FUNC(macs_per_cc, MACS_PER_CC_IS_5)     \
  SEP FUNC(macs_per_cc, MACS_PER_CC_IS_6) \
  SEP FUNC(macs_per_cc, MACS_PER_CC_IS_7) \
  SEP FUNC(macs_per_cc, MACS_PER_CC_IS_8)

#define EXPAND_MEMORY_TYPE(FUNC, SEP)              \
  FUNC(memory_type, AXI0_OUTSTANDING_COUNTER0)     \
  SEP FUNC(memory_type, AXI0_OUTSTANDING_COUNTER1) \
  SEP FUNC(memory_type, AXI1_OUTSTANDING_COUNTER2) \
  SEP FUNC(memory_type, AXI1_OUTSTANDING_COUNTER3)

#define EXPAND_OFM_PRECISION(FUNC, SEP) \
  FUNC(ofm_precision, U8)               \
  SEP FUNC(ofm_precision, S8)           \
  SEP FUNC(ofm_precision, U16)          \
  SEP FUNC(ofm_precision, S16)          \
  SEP FUNC(ofm_precision, S32)

#define EXPAND_PMU_EVENT_TYPE(FUNC, SEP)                 \
  FUNC(pmu_event_type, NO_EVENT)                         \
  SEP FUNC(pmu_event_type, CYCLE)                        \
  SEP FUNC(pmu_event_type, NPU_IDLE)                     \
  SEP FUNC(pmu_event_type, CC_STALLED_ON_BLOCKDEP)       \
  SEP FUNC(pmu_event_type, CC_STALLED_ON_SHRAM_RECONFIG) \
  SEP FUNC(pmu_event_type, NPU_ACTIVE)                   \
  SEP FUNC(pmu_event_type, MAC_ACTIVE)                   \
  SEP FUNC(pmu_event_type, MAC_ACTIVE_8BIT)              \
  SEP FUNC(pmu_event_type, MAC_ACTIVE_16BIT)             \
  SEP FUNC(pmu_event_type, MAC_DPU_ACTIVE)               \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_WD_ACC)        \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_WD)            \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_ACC)           \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_IB)            \
  SEP FUNC(pmu_event_type, MAC_ACTIVE_32BIT)             \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_INT_W)         \
  SEP FUNC(pmu_event_type, MAC_STALLED_BY_INT_ACC)       \
  SEP FUNC(pmu_event_type, AO_ACTIVE)                    \
  SEP FUNC(pmu_event_type, AO_ACTIVE_8BIT)               \
  SEP FUNC(pmu_event_type, AO_ACTIVE_16BIT)              \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_OFMP_OB)        \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_OFMP)           \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_OB)             \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_ACC_IB)         \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_ACC)            \
  SEP FUNC(pmu_event_type, AO_STALLED_BY_IB)             \
  SEP FUNC(pmu_event_type, WD_ACTIVE)                    \
  SEP FUNC(pmu_event_type, WD_STALLED)                   \
  SEP FUNC(pmu_event_type, WD_STALLED_BY_WS)             \
  SEP FUNC(pmu_event_type, WD_STALLED_BY_WD_BUF)         \
  SEP FUNC(pmu_event_type, WD_PARSE_ACTIVE)              \
  SEP FUNC(pmu_event_type, WD_PARSE_STALLED)             \
  SEP FUNC(pmu_event_type, WD_PARSE_STALLED_IN)          \
  SEP FUNC(pmu_event_type, WD_PARSE_STALLED_OUT)         \
  SEP FUNC(pmu_event_type, WD_TRANS_WS)                  \
  SEP FUNC(pmu_event_type, WD_TRANS_WB)                  \
  SEP FUNC(pmu_event_type, WD_TRANS_DW0)                 \
  SEP FUNC(pmu_event_type, WD_TRANS_DW1)                 \
  SEP FUNC(pmu_event_type, AXI0_RD_TRANS_ACCEPTED)       \
  SEP FUNC(pmu_event_type, AXI0_RD_TRANS_COMPLETED)      \
  SEP FUNC(pmu_event_type, AXI0_RD_DATA_BEAT_RECEIVED)   \
  SEP FUNC(pmu_event_type, AXI0_RD_TRAN_REQ_STALLED)     \
  SEP FUNC(pmu_event_type, AXI0_WR_TRANS_ACCEPTED)       \
  SEP FUNC(pmu_event_type, AXI0_WR_TRANS_COMPLETED_M)    \
  SEP FUNC(pmu_event_type, AXI0_WR_TRANS_COMPLETED_S)    \
  SEP FUNC(pmu_event_type, AXI0_WR_DATA_BEAT_WRITTEN)    \
  SEP FUNC(pmu_event_type, AXI0_WR_TRAN_REQ_STALLED)     \
  SEP FUNC(pmu_event_type, AXI0_WR_DATA_BEAT_STALLED)    \
  SEP FUNC(pmu_event_type, AXI0_ENABLED_CYCLES)          \
  SEP FUNC(pmu_event_type, AXI0_RD_STALL_LIMIT)          \
  SEP FUNC(pmu_event_type, AXI0_WR_STALL_LIMIT)          \
  SEP FUNC(pmu_event_type, AXI1_RD_TRANS_ACCEPTED)       \
  SEP FUNC(pmu_event_type, AXI1_RD_TRANS_COMPLETED)      \
  SEP FUNC(pmu_event_type, AXI1_RD_DATA_BEAT_RECEIVED)   \
  SEP FUNC(pmu_event_type, AXI1_RD_TRAN_REQ_STALLED)     \
  SEP FUNC(pmu_event_type, AXI1_WR_TRANS_ACCEPTED)       \
  SEP FUNC(pmu_event_type, AXI1_WR_TRANS_COMPLETED_M)    \
  SEP FUNC(pmu_event_type, AXI1_WR_TRANS_COMPLETED_S)    \
  SEP FUNC(pmu_event_type, AXI1_WR_DATA_BEAT_WRITTEN)    \
  SEP FUNC(pmu_event_type, AXI1_WR_TRAN_REQ_STALLED)     \
  SEP FUNC(pmu_event_type, AXI1_WR_DATA_BEAT_STALLED)    \
  SEP FUNC(pmu_event_type, AXI1_ENABLED_CYCLES)          \
  SEP FUNC(pmu_event_type, AXI1_RD_STALL_LIMIT)          \
  SEP FUNC(pmu_event_type, AXI1_WR_STALL_LIMIT)          \
  SEP FUNC(pmu_event_type, AXI_LATENCY_ANY)              \
  SEP FUNC(pmu_event_type, AXI_LATENCY_32)               \
  SEP FUNC(pmu_event_type, AXI_LATENCY_64)               \
  SEP FUNC(pmu_event_type, AXI_LATENCY_128)              \
  SEP FUNC(pmu_event_type, AXI_LATENCY_256)              \
  SEP FUNC(pmu_event_type, AXI_LATENCY_512)              \
  SEP FUNC(pmu_event_type, AXI_LATENCY_1024)             \
  SEP FUNC(pmu_event_type, ECC_DMA)                      \
  SEP FUNC(pmu_event_type, ECC_SB0)                      \
  SEP FUNC(pmu_event_type, ECC_SB1)

#define EXPAND_POOLING_MODE(FUNC, SEP)                    \
  FUNC(pooling_mode, MAX) SEP FUNC(pooling_mode, AVERAGE) \
  SEP FUNC(pooling_mode, REDUCE_SUM)

#define EXPAND_PRIVILEGE_LEVEL(FUNC, SEP) \
  FUNC(privilege_level, USER) SEP FUNC(privilege_level, PRIVILEGED)

#define EXPAND_RESAMPLING_MODE(FUNC, SEP)                        \
  FUNC(resampling_mode, NONE) SEP FUNC(resampling_mode, NEAREST) \
  SEP FUNC(resampling_mode, TRANSPOSE)

#define EXPAND_ROUNDING(FUNC, SEP)                 \
  FUNC(rounding, TFL) SEP FUNC(rounding, TRUNCATE) \
  SEP FUNC(rounding, NATURAL)

#define EXPAND_SECURITY_LEVEL(FUNC, SEP) \
  FUNC(security_level, SECURE) SEP FUNC(security_level, NON_SECURE)

#define EXPAND_SHRAM_SIZE(FUNC, SEP) \
  FUNC(shram_size, SHRAM_96KB)       \
  SEP FUNC(shram_size, SHRAM_48KB)   \
  SEP FUNC(shram_size, SHRAM_24KB)   \
  SEP FUNC(shram_size, SHRAM_16KB)

#define EXPAND_STATE(FUNC, SEP) FUNC(state, STOPPED) SEP FUNC(state, RUNNING)

#define EXPAND_STRIDE_MODE(FUNC, SEP)                                     \
  FUNC(stride_mode, STRIDE_MODE_1D) SEP FUNC(stride_mode, STRIDE_MODE_2D) \
  SEP FUNC(stride_mode, STRIDE_MODE_3D)
