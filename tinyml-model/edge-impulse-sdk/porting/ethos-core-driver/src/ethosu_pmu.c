...
static const enum pmu_event_type eventbyid[] = {
    EXPAND_PMU_EVENT_TYPE(EVID, COMMA)};
...
void ETHOSU_PMU_Enable(struct ethosu_driver *drv) {
  ...
  pmcr.cnt_en = 1;
  set_clock_and_power_request(drv, ETHOSU_PMU_REQUEST, ETHOSU_CLOCK_Q_DISABLE,
                              ETHOSU_POWER_Q_DISABLE);
...
void ETHOSU_PMU_Disable(struct ethosu_driver *drv) {
  ...
  pmcr.word = drv->dev.pmcr;
  pmcr.cnt_en = 0;
  set_clock_and_power_request(drv, ETHOSU_PMU_REQUEST, ETHOSU_CLOCK_Q_ENABLE,
                              ETHOSU_POWER_Q_ENABLE);
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCR, pmcr.word, &drv->dev.pmcr);
}

void ETHOSU_PMU_Set_EVTYPER(struct ethosu_driver *drv, uint32_t num,
  ...
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMEVTYPER(num), val,
                          &drv->dev.pmu_evtypr[num]);
}
...
void ETHOSU_PMU_CNTR_Enable(struct ethosu_driver *drv, uint32_t mask) {
  ...
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCNTENSET, mask,
                          &drv->dev.pmcnten);
}

void ETHOSU_PMU_CNTR_Disable(struct ethosu_driver *drv, uint32_t mask) {
  LOG_DEBUG("%s: mask=0x%08x\n", __FUNCTION__, mask);
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCNTENCLR, mask,
                          &drv->dev.pmcnten);
}
...
uint64_t ETHOSU_PMU_Get_CCNTR(struct ethosu_driver *drv) {
  ...
  // Return the shadow variable in case the NPU was powered off and lost the
  // cycle count
  if (shadow > val) {
    return shadow;
  }
...
void ETHOSU_PMU_Set_CCNTR(struct ethosu_driver *drv, uint64_t val) {
  ...
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCCNTR_LO, val & MASK_0_31_BITS,
                          &drv->dev.pmccntr[0]);
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCCNTR_HI,
                          (val & MASK_32_47_BITS) >> 32, &drv->dev.pmccntr[1]);
...
void ETHOSU_PMU_Set_EVCNTR(struct ethosu_driver *drv, uint32_t num,
                           uint32_t val) {
  assert(num < ETHOSU_PMU_NCOUNTERS);
...
void ETHOSU_PMU_CNTR_Increment(struct ethosu_driver *drv, uint32_t mask) {
  ...
  for (int i = 0; i < ETHOSU_PMU_NCOUNTERS; i++) {
    if (mask & (1 << i)) {
      ...
      ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMEVCNTR(i), val + 1,
                              &drv->dev.pmu_evcntr[i]);
    }
  }
...
void ETHOSU_PMU_PMCCNTR_CFG_Set_Start_Event(
    struct ethosu_driver *drv, enum ethosu_pmu_event_type start_event) {
  ...
  ethosu_write_reg_shadow(&drv->dev, NPU_REG_PMCCNTR_CFG, cfg.word,
                          &drv->dev.pmccntr_cfg);
}

void ETHOSU_PMU_PMCCNTR_CFG_Set_Stop_Event(
    struct ethosu_driver *drv, enum ethosu_pmu_event_type stop_event) {
