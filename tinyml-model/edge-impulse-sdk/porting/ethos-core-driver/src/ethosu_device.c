...
enum ethosu_error_codes ethosu_run_command_stream(struct ethosu_device *dev,
  ...
  LOG_DEBUG("QBASE=0x%016llx, QSIZE=%u, base_pointer_offset=0x%08x\n", qbase,
            cms_length, BASE_POINTER_OFFSET);
  ...
  for (int i = 0; i < num_base_addr; i++) {
    ...
    ethosu_write_reg(dev, NPU_REG_BASEP0 + (2 * i) * BASEP_OFFSET,
                     addr & 0xffffffff);
    ethosu_write_reg(dev, NPU_REG_BASEP0 + (2 * i + 1) * BASEP_OFFSET,
                     addr >> 32);
  }
...
enum ethosu_error_codes ethosu_soft_reset(struct ethosu_device *dev) {
  ...
  reset.word = 0;
  reset.pending_CPL =
      dev->privileged ? PRIVILEGE_LEVEL_PRIVILEGED : PRIVILEGE_LEVEL_USER;
  reset.pending_CSL =
      dev->secure ? SECURITY_LEVEL_SECURE : SECURITY_LEVEL_NON_SECURE;
  ...
  if (prot.active_CPL != reset.pending_CPL ||
      prot.active_CSL != reset.pending_CSL) {
  ...
  // Soft reset will clear the PMU configuration and counters. The shadow PMU
  // counters are cleared by saving the PMU counters to ram, which will read
  // back zeros. The PMU configuration will be restored in the invoke function
  // after power save has been disabled.
...
enum ethosu_error_codes ethosu_set_qconfig(
    struct ethosu_device *dev, enum ethosu_memory_type memory_type) {
...
enum ethosu_error_codes ethosu_save_pmu_config(struct ethosu_device *dev) {
  ...
  for (uint32_t i = 0; i < ETHOSU_PMU_NCOUNTERS; i++) {
    dev->pmu_evtypr[i] =
        ethosu_read_reg(dev, NPU_REG_PMEVTYPER0 + i * sizeof(uint32_t));
  }
#else
  UNUSED(dev);
#endif
...
enum ethosu_error_codes ethosu_restore_pmu_config(struct ethosu_device *dev) {
  ...
  // Save the event settings and counters
  for (uint32_t i = 0; i < ETHOSU_PMU_NCOUNTERS; i++) {
    ethosu_write_reg(dev, NPU_REG_PMEVTYPER0 + i * sizeof(uint32_t),
                     dev->pmu_evtypr[i]);
  }
#else
  UNUSED(dev);
...
enum ethosu_error_codes ethosu_save_pmu_counters(struct ethosu_device *dev) {
  ...
  for (uint32_t i = 0; i < ETHOSU_PMU_NCOUNTERS; i++) {
    dev->pmu_evcntr[i] =
        ethosu_read_reg(dev, NPU_REG_PMEVCNTR0 + i * sizeof(uint32_t));
  }
#else
  UNUSED(dev);
#endif
...
bool ethosu_status_has_error(struct ethosu_device *dev) {
  ...
  struct status_r status;
  status.word = ethosu_read_reg(dev, NPU_REG_STATUS);
  status_error = ((1 == status.bus_status) || (1 == status.cmd_parse_error) ||
                  (1 == status.wd_fault) || (1 == status.ecc_fault));
#else
  UNUSED(dev);
#endif
  return status_error;
