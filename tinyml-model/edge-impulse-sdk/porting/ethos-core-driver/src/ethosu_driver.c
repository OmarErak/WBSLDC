...
#define ETHOSU_FOURCC \
  ('1' << 24 | 'P' << 16 | 'O' << 8 | 'C')  // "Custom Operator Payload 1"
 ...
 * case of baremetal applications. Weak symbols will be overwritten by RTOS
 * definitions and implement true thread-safety. (Done in application layer)
 */
...
void *__attribute__((weak)) ethosu_mutex_create(void) { return NULL; }

void __attribute__((weak)) ethosu_mutex_lock(void *mutex) { UNUSED(mutex); }

void __attribute__((weak)) ethosu_mutex_unlock(void *mutex) { UNUSED(mutex); }
...
// Baremetal simulation of waiting/sleeping for and then taking a semaphore
// using intrisics
...
// Baremetal simulation of giving a semaphore and waking up processes using
// intrinsics
...
ethosu_inference_begin(struct ethosu_driver *drv, const void *inference_data) {
  (void)inference_data;
  (void)drv;
}

void __attribute__((weak))
ethosu_inference_end(struct ethosu_driver *drv, const void *inference_data) {
  (void)inference_data;
  (void)drv;
}

static int handle_optimizer_config(struct ethosu_driver *drv,
...
static void dump_npu_register(struct ethosu_driver *drv, int npu_reg,
                              int npu_reg_end);
static void dump_command_stream(const uint32_t *cmd_stream,
                                const int cms_length, int qread);
static void npu_axi_init(struct ethosu_driver *drv);
static struct ethosu_driver *ethosu_find_and_reserve_driver(void);

int ethosu_init(struct ethosu_driver *drv, const void *base_address,
                const void *fast_memory, const size_t fast_memory_size,
                uint32_t secure_enable, uint32_t privilege_enable) {
  int return_code = 0;

  LOG_INFO(
      "%s. base_address=%p, fast_memory=%p, fast_memory_size=%zu, "
      "secure=%" PRIu32 ", privileged=%" PRIu32 "\n",
      __FUNCTION__, base_address, fast_memory, fast_memory_size, secure_enable,
      privilege_enable);
  ...
  if (ETHOSU_SUCCESS != ethosu_dev_init(&drv->dev, base_address, secure_enable,
                                        privilege_enable)) {
    LOG_ERR("Failed in ethosu_dev_init");
    return -1;
  }

  if (ETHOSU_SUCCESS != set_clock_and_power_request(
                            drv, ETHOSU_INFERENCE_REQUEST,
                            ETHOSU_CLOCK_Q_DISABLE, ETHOSU_POWER_Q_DISABLE)) {
...
int ethosu_get_version(struct ethosu_driver *drv,
...
int ethosu_invoke(struct ethosu_driver *drv, const void *custom_data_ptr,
                  const int custom_data_size, const uint64_t *base_addr,
                  const size_t *base_addr_size, const int num_base_addr) {
  ...
  if (data_ptr->word != ETHOSU_FOURCC) {
    LOG_ERR("Custom Operator Payload: %" PRIu32
            " is not correct, expected %x\n",
  ...
  if (drv->fast_memory != 0 && num_base_addr >= FAST_MEMORY_BASE_ADDR_INDEX) {
    ...
    if (base_addr_size != NULL &&
        base_addr_size[FAST_MEMORY_BASE_ADDR_INDEX] > drv->fast_memory_size) {
      LOG_ERR(
          "Fast memory area too small. fast_memory_size=%u, "
          "base_addr_size=%u\n",
  ...
  if (!drv->dev_power_always_on) {
    ...
    drv->status_error = false;
    set_clock_and_power_request(drv, ETHOSU_INFERENCE_REQUEST,
                                ETHOSU_CLOCK_Q_ENABLE, ETHOSU_POWER_Q_DISABLE);
    ...
    switch (data_ptr->driver_action_command) {
      case OPTIMIZER_CONFIG:
        ...
        data_ptr += DRIVER_ACTION_LENGTH_32_BIT_WORD +
                    OPTIMIZER_CONFIG_LENGTH_32_BIT_WORD;
        break;
      case COMMAND_STREAM:
        LOG_INFO("ethosu_invoke COMMAND_STREAM\n");
        void *command_stream =
            (uint8_t *)(data_ptr) + sizeof(struct custom_data_s);
        int cms_length = (data_ptr->reserved << 16) | data_ptr->length;

        drv->abort_inference = false;
        // It is safe to clear this flag without atomic, because npu is not
        // running.
        drv->irq_triggered = false;

        ret = handle_command_stream(drv, command_stream, cms_length, base_addr,
  ...
  if (!drv->status_error && !drv->dev_power_always_on) {
    ...
    set_clock_and_power_request(drv, ETHOSU_INFERENCE_REQUEST,
                                ETHOSU_CLOCK_Q_ENABLE, ETHOSU_POWER_Q_ENABLE);
  }

  return return_code;
}

void ethosu_abort(struct ethosu_driver *drv) { drv->abort_inference = true; }
...
int ethosu_deregister_driver(struct ethosu_driver *drv) {
  ...
  LOG_ERR(
      "%s: NPU driver at address %p does not match a registered driver and "
      "therefore may not be deregistered.\n",
      __FUNCTION__, drv);
...
enum ethosu_error_codes set_clock_and_power_request(
    struct ethosu_driver *drv, enum ethosu_request_clients client,
  ...
  // Get current clock request (ENABLE if both PMU and INFERENCE asks for clock
  // request, else DISABLE)
  clock_request =
      drv->clock_request == 0 ? ETHOSU_CLOCK_Q_ENABLE : ETHOSU_CLOCK_Q_DISABLE;
  ...
  // Get current power request (ENABLE if both PMU and INFERENCE asks for power
  // request, else DISABLE)
  power_request =
      drv->power_request == 0 ? ETHOSU_POWER_Q_ENABLE : ETHOSU_POWER_Q_DISABLE;

  // Set clock and power
  enum ethosu_error_codes ret =
      ethosu_set_clock_and_power(&drv->dev, clock_request, power_request);
...
static int handle_optimizer_config(struct ethosu_driver *drv,
  ...
  LOG_INFO("Optimizer release nbr: %d patch: %d\n", opt_cfg_p->da_data.rel_nbr,
           opt_cfg_p->da_data.patch_nbr);
  LOG_INFO(
      "Optimizer config cmd_stream_version: %d macs_per_cc: %d shram_size: %d "
      "custom_dma: %d\n",
      opt_cfg_p->cmd_stream_version, opt_cfg_p->macs_per_cc,
      opt_cfg_p->shram_size, opt_cfg_p->custom_dma);
  LOG_INFO("Optimizer config Ethos-U version: %d.%d.%d\n",
           opt_cfg_p->arch_major_rev, opt_cfg_p->arch_minor_rev,
           opt_cfg_p->arch_patch_rev);

  (void)ethosu_get_config(&drv->dev, &cfg);
  (void)ethosu_get_id(&drv->dev, &id);
  LOG_INFO(
      "Ethos-U config cmd_stream_version: %" PRIu32 " macs_per_cc: %" PRIu32
      " shram_size: %" PRIu32 " custom_dma: %" PRIu32 "\n",
      cfg.cmd_stream_version, cfg.macs_per_cc, cfg.shram_size, cfg.custom_dma);
  ...
  if ((cfg.macs_per_cc != opt_cfg_p->macs_per_cc) ||
      (cfg.shram_size != opt_cfg_p->shram_size) ||
      (cfg.cmd_stream_version != opt_cfg_p->cmd_stream_version) ||
      (!cfg.custom_dma && opt_cfg_p->custom_dma)) {
    if (cfg.macs_per_cc != opt_cfg_p->macs_per_cc) {
      LOG_ERR("NPU config mismatch: npu.macs_per_cc=%" PRIu32
              " optimizer.macs_per_cc=%d\n",
              cfg.macs_per_cc, opt_cfg_p->macs_per_cc);
    }
    if (cfg.shram_size != opt_cfg_p->shram_size) {
      LOG_ERR("NPU config mismatch: npu.shram_size=%" PRIu32
              " optimizer.shram_size=%d\n",
              cfg.shram_size, opt_cfg_p->shram_size);
    }
    if (cfg.cmd_stream_version != opt_cfg_p->cmd_stream_version) {
      LOG_ERR("NPU config mismatch: npu.cmd_stream_version=%" PRIu32
              " optimizer.cmd_stream_version=%d\n",
              cfg.cmd_stream_version, opt_cfg_p->cmd_stream_version);
    }
    if (!cfg.custom_dma && opt_cfg_p->custom_dma) {
      LOG_ERR("NPU config mismatch: npu.custom_dma=%" PRIu32
              " optimize.custom_dma=%d\n",
              cfg.custom_dma, opt_cfg_p->custom_dma);
    }
    LOG_ERR(
        "Did you choose the correct target core? This model was compiled for a "
        "different Ethos configuration\n");
    return_code = -1;
  }

  if ((id.arch_major_rev != opt_cfg_p->arch_major_rev) ||
      (id.arch_minor_rev < opt_cfg_p->arch_minor_rev)) {
    LOG_ERR("NPU arch mismatch: npu.arch=%" PRIu32 ".%" PRIu32 ".%" PRIu32
            " optimizer.arch=%d.%d.%d\n",
            id.arch_major_rev, id.arch_minor_rev, id.arch_patch_rev,
            opt_cfg_p->arch_major_rev, opt_cfg_p->arch_minor_rev,
            opt_cfg_p->arch_patch_rev);
    return_code = -1;
...
static void npu_axi_init(struct ethosu_driver *drv) {
  ...
  (void)ethosu_set_axi_limit0(
      &drv->dev, AXI_LIMIT0_MAX_BEATS_BYTES, AXI_LIMIT0_MEM_TYPE,
      AXI_LIMIT0_MAX_OUTSTANDING_READS, AXI_LIMIT0_MAX_OUTSTANDING_WRITES);
  (void)ethosu_set_axi_limit1(
      &drv->dev, AXI_LIMIT1_MAX_BEATS_BYTES, AXI_LIMIT1_MEM_TYPE,
      AXI_LIMIT1_MAX_OUTSTANDING_READS, AXI_LIMIT1_MAX_OUTSTANDING_WRITES);
  (void)ethosu_set_axi_limit2(
      &drv->dev, AXI_LIMIT2_MAX_BEATS_BYTES, AXI_LIMIT2_MEM_TYPE,
      AXI_LIMIT2_MAX_OUTSTANDING_READS, AXI_LIMIT2_MAX_OUTSTANDING_WRITES);
  (void)ethosu_set_axi_limit3(
      &drv->dev, AXI_LIMIT3_MAX_BEATS_BYTES, AXI_LIMIT3_MEM_TYPE,
      AXI_LIMIT3_MAX_OUTSTANDING_READS, AXI_LIMIT3_MAX_OUTSTANDING_WRITES);
}

/* Default implementation to flush the data cache. Override if available on the
 * targeted device. Passing NULL as p argument expects the whole cache to be
 * flushed.
 */
...
/* Default implementation to invalidate the data cache. Override if available on
 * the targeted device. Passing NULL as p argument expects the whole cache to be
 * flushed.
 */
...
static int handle_command_stream(struct ethosu_driver *drv,
   ...
   * The base_addr_size is null if invoking from prior to invoke_V2, in that
   * case the whole cache is being flushed.
   */

  if (base_addr_size != NULL) {
    ethosu_flush_dcache((uint32_t *)cmd_stream_ptr, cms_bytes);
    for (int i = 0; i < num_base_addr; i++) {
      ethosu_flush_dcache((uint32_t *)(uintptr_t)base_addr[i],
                          base_addr_size[i]);
    }
  } else {
  ...
  if (ETHOSU_SUCCESS != ethosu_run_command_stream(&drv->dev, cmd_stream,
                                                  cms_bytes, base_addr,
                                                  num_base_addr)) {
    return -1;
  }
  ...
  if (base_addr_size != NULL) {
    for (int i = 0; i < num_base_addr; i++) {
      ethosu_invalidate_dcache((uint32_t *)(uintptr_t)base_addr[i],
                               base_addr_size[i]);
    }
  } else {
  ...
  if (qread != cms_bytes) {
    LOG_WARN("Failure: IRQ received but qread (%" PRIu32
             ") not at end of stream (%" PRIu32 ").\n",
             qread, cms_bytes);
    return -1;
  }
...
static int read_apb_reg(struct ethosu_driver *drv, uint16_t da_data) {
  ...
  if (ETHOSU_SUCCESS ==
      ethosu_read_apb_reg(&drv->dev, start_address, num_reg, reg_p)) {
    for (int i = 0; i < num_reg; i++) {
      LOG_INFO("NPU_REG ADDR 0x%04" PRIu32 " = 0x%08" PRIu32 "\n",
...
static const char *lookup_name(const name_lookup_t *lookup_table,
                               int lookup_table_count, int find) {
  int n;
...
static void dump_npu_register(struct ethosu_driver *drv, int npu_reg,
  ...
  const char *reg_name;
  int npu_reg_name_tbl_count =
      sizeof(npu_reg_name_tbl) / sizeof(npu_reg_name_tbl[0]);
...
static const name_lookup_t cmd1_name_tbl[] = {
    {0x000, "NPU_SET_IFM_BASE0"},     {0x001, "NPU_SET_IFM_BASE1"},
    {0x002, "NPU_SET_IFM_BASE2"},     {0x003, "NPU_SET_IFM_BASE3"},
    {0x004, "NPU_SET_IFM_STRIDE_X"},  {0x005, "NPU_SET_IFM_STRIDE_Y"},
    {0x006, "NPU_SET_IFM_STRIDE_C"},  {0x007, "NPU_SET_IFM_STRIDE_N"},
    {0x010, "NPU_SET_OFM_BASE0"},     {0x011, "NPU_SET_OFM_BASE1"},
    {0x012, "NPU_SET_OFM_BASE2"},     {0x013, "NPU_SET_OFM_BASE3"},
    {0x014, "NPU_SET_OFM_STRIDE_X"},  {0x015, "NPU_SET_OFM_STRIDE_Y"},
    {0x016, "NPU_SET_OFM_STRIDE_C"},  {0x017, "NPU_SET_OFM_STRIDE_N"},
    {0x020, "NPU_SET_WEIGHT_BASE"},   {0x021, "NPU_SET_WEIGHT_LENGTH"},
    {0x022, "NPU_SET_SCALE_BASE"},    {0x023, "NPU_SET_SCALE_LENGTH"},
    {0x024, "NPU_SET_OFM_SCALE"},     {0x025, "NPU_SET_OPA_SCALE"},
    {0x026, "NPU_SET_OPB_SCALE"},     {0x030, "NPU_SET_DMA0_SRC"},
    {0x031, "NPU_SET_DMA0_DST"},      {0x032, "NPU_SET_DMA0_LEN"},
    {0x080, "NPU_SET_IFM2_BASE0"},    {0x081, "NPU_SET_IFM2_BASE1"},
    {0x082, "NPU_SET_IFM2_BASE2"},    {0x083, "NPU_SET_IFM2_BASE3"},
    {0x084, "NPU_SET_IFM2_STRIDE_X"}, {0x085, "NPU_SET_IFM2_STRIDE_Y"},
...
static void dump_command_stream(const uint32_t *cmd_stream,
  ...
  for (n = 0; n < cms_length; n++) {
    ...
    LOG_INFO("0x%.2X 0x%.2X 0x%.2X 0x%.2X ", cmd_ptr[0], cmd_ptr[1], cmd_ptr[2],
             cmd_ptr[3]);
    // Command name and payload
    if (cmd_stream[n] & 0x4000) {
      cmd_name = lookup_name(cmd1_name_tbl, cmd1_name_tbl_count,
                             cmd_stream[n] & 0x3FF);
      n++;
      cmd_val = cmd_stream[n];
      cmd_ptr = (const uint8_t *)&cmd_stream[n];
      LOG_INFO("0x%.2X 0x%.2X 0x%.2X 0x%.2X ", cmd_ptr[0], cmd_ptr[1],
               cmd_ptr[2], cmd_ptr[3]);
    } else {
      cmd_val = cmd_stream[n] >> 16;
      cmd_name = lookup_name(cmd0_name_tbl, cmd0_name_tbl_count,
                             cmd_stream[n] & 0x3FF);
    }
    if (cmd_name) {
