 ...
 * Library Debug mode
 */
#define DBG_MODE_RELEASE \
  (0) /*< No debug. Messages:OFF; Assertions:OFF; ReturnCodes: Always OK */
#define DBG_MODE_RET_CODES                                                    \
  (1) /*< Return codes mode. Messages:OFF; Assertions:OFF; ReturnCodes: Valid \
         Return*/
#define DBG_MODE_ASSERT                                              \
  (2) /*< Assert. Messages:OFF; Assertions:ON; Extra Assertions:OFF; \
         ReturnCodes: Valid Return */
#define DBG_MODE_DEBUG                                             \
  (3) /*< Debug. Messages:ON; Assertions:ON; Extra Assertions:OFF; \
         ReturnCodes: Valid Return */
#define DBG_MODE_FULL                                                  \
  (4) /*< Full Debug. Messages:ON; Assertions:ON; Extra Assertions:ON; \
         ReturnCodes: Valid Return */
...
/*
 * Define the platform (according to pre-defined macro or according to HW
 * config) 1 - ARCV2DSP ISA 2 - ARCV2DSP ISA with XY Memory 3 - ARCV2DSP ISA
 * with 64bit operands (HS Only)
 */

#if defined(V2DSP_XY) || \
    ((defined __Xxy) && !(defined(V2DSP) || defined(V2DSP_WIDE)))
/* Platform with XY memory (EM9D or EM11D) */
#undef V2DSP_XY
#define ARC_PLATFORM (2)
#define ARC_PLATFORM_STR "ARCv2DSP XY"

#elif defined(V2DSP_WIDE) || \
    ((defined __Xdsp_wide) && !(defined(V2DSP) || defined(V2DSP_XY)))
/* Platform with wide DSP ISA (HS45D or HS47D) */
#undef V2DSP_WIDE
#define ARC_PLATFORM (3)
#define ARC_PLATFORM_STR "ARCv2DSP Wide"

#elif defined(V2DSP) || ((defined(__Xdsp2) || defined(__Xdsp_complex)) && \
                         !(defined(V2DSP_XY) || defined(V2DSP_WIDE)))
 ...
 * this means all the kernels that perform a convolution like operation between
 * inputs and weights. MLI_OUT_PTR is used for the output of all other kernels.
 */
