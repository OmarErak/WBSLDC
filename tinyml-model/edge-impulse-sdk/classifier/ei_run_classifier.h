...
/* Function prototypes ----------------------------------------------------- */
extern "C" EI_IMPULSE_ERROR run_inference(const ei_impulse_t *impulse,
                                          ei::matrix_t *fmatrix,
                                          ei_impulse_result_t *result,
                                          bool debug);
extern "C" EI_IMPULSE_ERROR run_classifier_image_quantized(
    const ei_impulse_t *impulse, signal_t *signal, ei_impulse_result_t *result,
    bool debug);
...
extern "C" EI_IMPULSE_ERROR run_inference(const ei_impulse_t *impulse,
  ...
  if (impulse->has_anomaly) {
    EI_IMPULSE_ERROR anomaly_res =
        inference_anomaly_invoke(impulse, fmatrix, result, debug);
...
extern "C" EI_IMPULSE_ERROR process_impulse(const ei_impulse_t *impulse,
     ...
     (EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_TFLITE ||      \
      EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_TENSAIFLOW ||  \
      EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_ONNX_TIDL)) || \
    EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_DRPAI
  ...
  for (size_t ix = 0; ix < impulse->dsp_blocks_size; ix++) {
    ...
    if (block.axes_size != impulse->raw_samples_per_frame) {
      ei_printf(
          "ERR: EIDSP_SIGNAL_C_FN_POINTER can only be used when all axes are "
          "selected for DSP blocks\n");
      return EI_IMPULSE_DSP_ERROR;
    }
    ...
    int ret = block.extract_fn(swa.get_signal(), &fm, block.config,
                               impulse->frequency);
#endif
 ...
 * @return     The ei impulse error.
 */
extern "C" EI_IMPULSE_ERROR process_impulse_continuous(
    const ei_impulse_t *impulse, signal_t *signal, ei_impulse_result_t *result,
    bool debug, bool enable_maf) {
  ...
  for (size_t ix = 0; ix < impulse->dsp_blocks_size; ix++) {
    ...
    int (*extract_fn_slice)(ei::signal_t * signal, ei::matrix_t * output_matrix,
                            void *config, const float frequency,
    ...
    if (block.extract_fn == extract_mfcc_features) {
      ...
      ei_printf(
          "ERR: Unknown extract function, only MFCC, MFE and spectrogram "
          "supported\n");
      return EI_IMPULSE_DSP_ERROR;
    }
    ...
    int ret = extract_fn_slice(signal, &fm, block.config, impulse->frequency,
                               &features_written);
#else
    SignalWithAxes swa(signal, block.axes, block.axes_size, impulse);
    int ret = extract_fn_slice(swa.get_signal(), &fm, block.config,
                               impulse->frequency, &features_written);
#endif
    ...
    classifier_continuous_features_written +=
        (features_written.rows * features_written.cols);
  ...
  if (classifier_continuous_features_written >= impulse->nn_input_frame_size) {
    ...
    if (is_mfcc) {
      calc_cepstral_mean_and_var_normalization_mfcc(
          &classify_matrix, impulse->dsp_blocks[0].config);
    } else if (is_spectrogram) {
      calc_cepstral_mean_and_var_normalization_spectrogram(
          &classify_matrix, impulse->dsp_blocks[0].config);
    } else if (is_mfe) {
      calc_cepstral_mean_and_var_normalization_mfe(
          &classify_matrix, impulse->dsp_blocks[0].config);
    }
    result->timing.dsp_us += ei_read_timer_us() - dsp_start_us;
    result->timing.dsp = (int)(result->timing.dsp_us / 1000);
    ...
    if (impulse->sensor == EI_CLASSIFIER_SENSOR_MICROPHONE) {
      if ((void *)avg_scores != NULL && enable_maf == true) {
        if (enable_maf && !ei_calibration.is_configured) {
          ...
          if (!has_printed_msg) {
            ei_printf(
                "WARN: run_classifier_continuous, enable_maf is true, but "
                "performance calibration is not configured.\n");
            ei_printf(
                "       Previously we'd run a moving-average filter over your "
                "outputs in this case, but this is now disabled.\n");
            ei_printf(
                "       Go to 'Performance calibration' in your Edge Impulse "
                "project to configure post-processing parameters.\n");
            ei_printf(
                "       (You can enable this from 'Dashboard' if it's not "
          ...
          if (!has_printed_msg) {
            ei_printf(
                "\nPerformance calibration is configured for your project. If "
                "no event is detected, all values are 0.\r\n\n");
            has_printed_msg = true;
          }
    ...
    if (!impulse->object_detection) {
      for (int i = 0; i < impulse->label_count; i++) {
        // set label correctly in the result struct if we have no results
...
extern "C" const ei_impulse_t ei_construct_impulse() {
  const ei_impulse_t impulse = {
    ...
    .fomo_output_size = static_cast<uint32_t>(
        sqrt(EI_CLASSIFIER_NN_OUTPUT_COUNT /
             (EI_CLASSIFIER_LABEL_COUNT + 1 /* background */))),
#else
...
#if ((EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_TFLITE) ||       \
     (EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_TFLITE_FULL)) && \
    (EI_CLASSIFIER_COMPILED != 1)
    ...
    .slice_size = (EI_CLASSIFIER_RAW_SAMPLE_COUNT /
...
can_run_classifier_image_quantized(const ei_impulse_t *impulse) {
  ...
  // Check if we have a quantized NN Input layer (input is always quantized for
  // DRP-AI)
...
/**
 * Special function to run the classifier on images, only works on TFLite models
 * (either interpreter, EON, tensaiflow, drpai or tidl) that allocates a lot
 * less memory by quantizing in place. This only works if
 * 'can_run_classifier_image_quantized' returns EI_IMPULSE_OK.
 */
extern "C" EI_IMPULSE_ERROR run_classifier_image_quantized(
    const ei_impulse_t *impulse, signal_t *signal, ei_impulse_result_t *result,
    bool debug = false) {
...
extern "C" void run_classifier_init() {
  ...
  if (calibration != NULL) {
    avg_scores = new RecognizeEvents(calibration, impulse.label_count,
                                     impulse.slice_size, impulse.interval_ms);
  }
#endif
}
...
extern "C" EI_IMPULSE_ERROR run_classifier_continuous(
    signal_t *signal, ei_impulse_result_t *result, bool debug = false,
...
/* These functions are being deprecated and possibly will be removed or moved in
future. Do not use these - if possible, change your code to reflect the upcoming
changes. */

#if EIDSP_SIGNAL_C_FN_POINTER == 0

/**
 * Run the impulse, if you provide an instance of sampler it will also persist
 * the data for you
...
__attribute__((unused)) EI_IMPULSE_ERROR run_impulse(
  ...
  // grab some data
  for (int i = 0; i < (int)impulse.dsp_input_frame_size;
       i += impulse.raw_samples_per_frame) {
