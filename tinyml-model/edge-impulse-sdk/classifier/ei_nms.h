...
// tensorflow/lite/kernels/internal/reference/non_max_suppression.h Copyright
// 2019 The TensorFlow Authors.  All rights reserved. Licensed under the Apache
// License, Version 2.0
#include <algorithm>
#include <cmath>
#include <deque>
...
static inline float ComputeIntersectionOverUnion(const float* boxes,
...
static inline void NonMaxSuppression(
    const float* boxes, const int num_boxes, const float* scores,
    const int max_output_size, const float iou_threshold,
    const float score_threshold, const float soft_nms_sigma,
    int* selected_indices, float* selected_scores, int* num_selected_indices) {
  struct Candidate {
    int index;
    float score;
...
EI_IMPULSE_ERROR ei_run_nms(
  ...
  NonMaxSuppression((const float*)boxes,   // boxes
                    bb_count,              // num_boxes
                    ...
                    0.0f,                  // soft_nms_sigma
                    selected_indices, selected_scores, &num_selected_indices);
...
#endif  // #if (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER ==
        // EI_CLASSIFIER_LAST_LAYER_YOLOV5) ||
        // (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER ==
        // EI_CLASSIFIER_LAST_LAYER_YOLOV5_V5_DRPAI) ||
        // (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER ==
