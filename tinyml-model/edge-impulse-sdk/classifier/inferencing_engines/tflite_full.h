 ...
 */
static EI_IMPULSE_ERROR get_interpreter(const ei_nn_tflite_full_t *config,
                                        tflite::Interpreter **interpreter) {
  // not in the map yet...
  if (!ei_tflite_instances.count(config->block_id)) {
    ei_tflite_state_t *new_state = new ei_tflite_state_t();

    auto new_model = tflite::FlatBufferModel::BuildFromBuffer(
        (const char *)config->model_arr, config->model_arr_size);
...
extern "C" EI_IMPULSE_ERROR run_nn_inference_from_dsp(
    const ei_nn_tflite_full_t *config, signal_t *signal,
...
extern "C" EI_IMPULSE_ERROR run_nn_inference(const ei_impulse_t *impulse,
                                             ei::matrix_t *fmatrix,
                                             ei_impulse_result_t *result,
                                             bool debug = false) {
  const ei_nn_tflite_full_t config = {
      0xFFFFFFFF,  // block ID (we don't pass this in yet, so just put it here)
      impulse->model_arr, impulse->model_arr_size};
  tflite::Interpreter *interpreter;
  auto interpreter_ret = get_interpreter(&config, &interpreter);
  if (interpreter_ret != EI_IMPULSE_OK) {
    return interpreter_ret;
  ...
  TfLiteTensor *input = interpreter->input_tensor(0);
  TfLiteTensor *output =
      interpreter->output_tensor(impulse->tflite_output_data_tensor);
