...
#if (EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_ONNX_TIDL) && \
 ...
 * @returns int status
 */
int printTensorInfo(Ort::Session *session,
...
#ifdef DEVICE_AM62
    ei_printf(
        "LOG_INFO: TIDL Delgate mode is not allowed on AM62 devices...\n");
    ei_printf(
...
static EI_IMPULSE_ERROR inference_onnx_setup(
    const ei_impulse_t *impulse, uint64_t *ctx_start_us,
    std::vector<Ort::Value> *input_tensors,
    std::vector<Ort::Value> *output_tensors, Ort::Session **session_ptr,
    Ort::RunOptions **run_options_ptr, Ort::IoBinding **binding_ptr) {
  ...
  std::string proj_artifacts_path = "/tmp/" +
                                    std::string(impulse->project_name) + "-" +
...
#pragma message("test onnx tidl: run_nn_inference")
  ...
  c_api_tidl_options *options =
      (c_api_tidl_options *)malloc(sizeof(c_api_tidl_options));
  ...
  /* ORT Session */
  Ort::Session *session =
      new Ort::Session(env, proj_model_path.c_str(), session_options);
  *session_ptr = session;
  ...
  if (EI_IMPULSE_ONNX_ERROR ==
      printTensorInfo(session, &input_node_names, &output_node_names)) {
  ...
  Ort::Value input_tensor =
      Ort::Value::CreateTensor(memory_info, inData, input_tensor_size_bytes,
                               input_node_dims.data(), 4, input_tensor_type);
  input_tensors->push_back(std::move(input_tensor));

  Ort::RunOptions *run_options = new Ort::RunOptions();
  *run_options_ptr = run_options;
  run_options->SetRunLogVerbosityLevel(2);
  auto output_tensors_warm_up =
      session->Run(*run_options, input_node_names.data(), input_tensors->data(),
                   1, output_node_names.data(), num_output_nodes);
  ...
  *binding_ptr = binding;
  ...
  for (int idx = 0; idx < num_output_nodes; idx++) {
    auto node_dims = output_tensors_warm_up[idx]
                         .GetTypeInfo()
    ...
    ONNXTensorElementDataType tensor_type = output_tensors_warm_up[idx]
                                                .GetTypeInfo()
                                                .GetTensorTypeAndShapeInfo()
                                                .GetElementType();
    ...
    auto output_tensor = Ort::Value::CreateTensor(
        memory_info, (void *)outData, tensor_size, node_dims.data(),
        node_dims.size(), tensor_type);
 ...
 * @return  EI_IMPULSE_OK if successful
 */
static EI_IMPULSE_ERROR inference_onnx_run(
    const ei_impulse_t *impulse, uint64_t ctx_start_us,
    std::vector<Ort::Value> *input_tensors,
    std::vector<Ort::Value> *output_tensors, Ort::Session *session,
    Ort::RunOptions *run_options, Ort::IoBinding *binding,
  ...
  if (impulse->object_detection) {
    switch (impulse->object_detection_last_layer) {
      case EI_CLASSIFIER_LAST_LAYER_YOLOX: {
        ...
        fill_res = fill_result_struct_f32_yolox_detect(
            impulse, result, (float *)out_data, output_tensor_features_count);
#endif
        break;
      }
      default: {
    ...
    switch (output_tensor_type) {
      case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT8: {
        fill_res = fill_result_struct_i8(impulse, result, (int8_t *)out_data,
    ...
    switch (output_tensor_type) {
      case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT: {
        fill_res =
            fill_result_struct_f32(impulse, result, (float *)out_data, debug);
        break;
      }
      default: {
        ei_printf("ERR: Cannot handle output type (%d)\n", output_tensor_type);
 ...
 *
 * @param      fmatrix  Processed matrix >> features [array of features] this is
 * input
 ...
 */
EI_IMPULSE_ERROR run_nn_inference(const ei_impulse_t *impulse,
...
#pragma message("test onnx tidl: run_nn_inference")
  ...
  EI_IMPULSE_ERROR init_res =
      inference_onnx_setup(impulse, &ctx_start_us, &input_tensors,
                           &output_tensors, &session, &run_options, &binding);
  ...
  */
  ...
  ei_dsp_config_image_t *config =
      (ei_dsp_config_image_t *)impulse->dsp_blocks[0].config;
  ...
  EI_IMPULSE_ERROR run_res =
      inference_onnx_run(impulse, ctx_start_us, &input_tensors, &output_tensors,
                         session, run_options, binding, result, debug);
 ...
 */
EI_IMPULSE_ERROR run_nn_inference_image_quantized(const ei_impulse_t *impulse,
                                                  signal_t *signal,
                                                  ei_impulse_result_t *result,
                                                  bool debug = false) {
  static std::vector<Ort::Value> input_tensors;
...
#pragma message("test onnx tidl: run_nn_inference_image_quantized")
  ...
  ONNXTensorElementDataType input_tensor_type = input_tensors.at(0)
                                                    .GetTypeInfo()
                                                    .GetTensorTypeAndShapeInfo()
                                                    .GetElementType();
  ...
  int ret = extract_image_features_quantized(
      impulse, signal, &a_features_matrix, impulse->dsp_blocks[0].config,
      impulse->frequency);
  if (ret != EIDSP_OK) {
  ...
  */
  ...
  if (debug) {
    ...
    for (size_t ix = 0; ix < features_matrix.cols; ix++) {
      ...
      ei_printf_float(static_cast<float>(((uint8_t)features_matrix.buffer[ix] -
                                          impulse->tflite_input_zeropoint) *
                                         impulse->tflite_input_scale));
      ei_printf(" ");
    }
    ei_printf("\n");
  }
...
#endif  // #if (EI_CLASSIFIER_INFERENCING_ENGINE == EI_CLASSIFIER_ONNX_TIDL) &&
        // (EI_CLASSIFIER_COMPILED != 1)
