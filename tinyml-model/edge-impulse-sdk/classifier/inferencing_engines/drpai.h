...
void get_udmabuf_memory_start_addr() { /* Obtain udmabuf memory area starting
                                          address */
...
uint8_t drpai_init_mem(uint32_t input_frame_size) {
  ...
  uint8_t *addr =
      (uint8_t *)mmap(NULL, input_frame_size, PROT_READ | PROT_WRITE,
                      MAP_SHARED, udmabuf_fd0, 0);
...
static int8_t read_addrmap_txt() {
  ...
  ifs.rdbuf()->pubsetbuf((char *)ei_ei_addrmap_intm_txt,
                         ei_ei_addrmap_intm_txt_len);
...
EI_IMPULSE_ERROR drpai_close(uint32_t input_frame_size) {
  ...
  return EI_IMPULSE_OK;
     ...
     (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER == \
      EI_CLASSIFIER_LAST_LAYER_YOLOV5_V5_DRPAI))
EI_IMPULSE_ERROR drpai_run_yolov5_postprocessing(const ei_impulse_t *impulse,
                                                 signal_t *signal,
  ...
  if (!model) {
    model = tflite::FlatBufferModel::BuildFromBuffer((const char *)yolov5_part2,
                                                     yolov5_part2_len);
    if (!model) {
  ...
  const size_t els_per_grid =
      drpai_features / ((NUM_GRID_1 * NUM_GRID_1) + (NUM_GRID_2 * NUM_GRID_2) +
                        (NUM_GRID_3 * NUM_GRID_3));
  ...
  for (size_t ix = 0; ix < 3; ix++) {
    ...
    if (tensor_size == grid_1_size) {
      memcpy(input, drpai_output_buf + grid_1_offset,
             grid_1_size * sizeof(float));
    } else if (tensor_size == grid_2_size) {
      memcpy(input, drpai_output_buf + grid_2_offset,
             grid_2_size * sizeof(float));
    } else if (tensor_size == grid_3_size) {
      memcpy(input, drpai_output_buf + grid_3_offset,
             grid_3_size * sizeof(float));
    } else {
      ei_printf(
          "ERR: Cannot determine which grid to use for input tensor %d with %d "
          "tensor size\n",
          (int)ix, (int)tensor_size);
      return EI_IMPULSE_TFLITE_ERROR;
    }
...
EI_IMPULSE_ERROR run_nn_inference_image_quantized(const ei_impulse_t *impulse,
  ...
  EI_LOGD(
      "fmatrix size == Bpp * signal.total_length ( %p == %p * %p = %p )\r\n",
  ...
  ret = extract_drpai_features_quantized(
      signal, &features_matrix, ei_dsp_blocks[0].config, impulse->frequency);
  if (ret != EIDSP_OK) {
    ...
    switch (impulse->object_detection_last_layer) {
      case EI_CLASSIFIER_LAST_LAYER_FOMO: {
        ...
        fill_res = fill_result_struct_f32_fomo(
            impulse, result, drpai_output_buf, impulse->fomo_output_size,
            impulse->fomo_output_size);
        break;
      }
      case EI_CLASSIFIER_LAST_LAYER_SSD: {
        ei_printf(
            "ERR: MobileNet SSD models are not implemented for DRP-AI (%d)\n",
      ...
      case EI_CLASSIFIER_LAST_LAYER_YOLOV5_V5_DRPAI: {
        ...
        if (debug) {
          ei_printf("DEBUG: raw drpai output");
          ei_printf("\n[");
          // impulse->tflite_output_features_count can't be used here as this is
          // not the final output so print only the first 10 values.
