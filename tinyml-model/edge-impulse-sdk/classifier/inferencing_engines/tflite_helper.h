...
static TfLiteRegistration post_process_op =
...
EI_IMPULSE_ERROR fill_input_tensor_from_matrix(matrix_t *fmatrix,
                                               TfLiteTensor *input) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
      if (input->bytes / 4 != matrix_els) {
        ei_printf(
            "ERR: input tensor has size %d, but input matrix has has size %d\n",
    ...
    case kTfLiteInt8: {
      ...
      for (size_t ix = 0; ix < fmatrix->rows * fmatrix->cols; ix++) {
        ...
        input->data.int8[ix] = static_cast<int8_t>(
            round(pixel / input->params.scale) + input->params.zero_point);
      }
      break;
    }
    case kTfLiteUInt8: {
      ...
      for (size_t ix = 0; ix < fmatrix->rows * fmatrix->cols; ix++) {
        ...
        input->data.uint8[ix] = static_cast<uint8_t>(
            (pixel / input->params.scale) + input->params.zero_point);
      }
      break;
    }
    default: {
...
EI_IMPULSE_ERROR fill_input_tensor_from_signal(signal_t *signal,
                                               TfLiteTensor *input) {
  switch (input->type) {
    case kTfLiteFloat32: {
    ...
    case kTfLiteUInt8: {
      ...
      // we don't understand the input very well; guess whether this is an RGB
      // input)
          ...
          if (is_rgb) {
            ...
            if (scale == 0.003921568859368563f && zero_point == -128) {
              ...
              if (input->type == kTfLiteInt8) {
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(r + zero_point);
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(g + zero_point);
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(b + zero_point);
              } else {
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(r + zero_point);
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(g + zero_point);
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(b + zero_point);
              }
            }
            // slow code path
            else {
              ...
              if (input->type == kTfLiteInt8) {
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(round(r / scale) + zero_point);
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(round(g / scale) + zero_point);
                input->data.int8[output_ix++] =
                    static_cast<int8_t>(round(b / scale) + zero_point);
              } else {
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(round(r / scale) + zero_point);
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(round(g / scale) + zero_point);
                input->data.uint8[output_ix++] =
                    static_cast<uint8_t>(round(b / scale) + zero_point);
              }
            }
          } else {
            ...
            if (input->type == kTfLiteInt8) {
              input->data.int8[output_ix++] =
                  static_cast<int8_t>(round(value / scale) + zero_point);
            } else {  // uint8
              input->data.uint8[output_ix++] =
                  static_cast<uint8_t>((value / scale) + zero_point);
            }
          }
        }
      }
...
EI_IMPULSE_ERROR fill_output_matrix_from_tensor(TfLiteTensor *output,
  ...
  switch (output->type) {
    case kTfLiteFloat32: {
      if (output->bytes / 4 != matrix_els) {
        ei_printf(
            "ERR: output tensor has size %d, but input matrix has has size "
            "%d\n",
    ...
    case kTfLiteInt8: {
      ...
      for (size_t ix = 0; ix < output->bytes; ix++) {
        float value = static_cast<float>(output->data.int8[ix] -
    ...
    case kTfLiteUInt8: {
      ...
      for (size_t ix = 0; ix < output->bytes; ix++) {
        float value = static_cast<float>(output->data.uint8[ix] -
                                         output->params.zero_point) *
                      output->params.scale;
        output_matrix->buffer[ix] = value;
      }
  ...
  if (impulse->object_detection) {
    switch (impulse->object_detection_last_layer) {
      case EI_CLASSIFIER_LAST_LAYER_FOMO: {
        ...
        if (int8_output) {
          fill_res = fill_result_struct_i8_fomo(
              impulse, result, output->data.int8, output->params.zero_point,
              output->params.scale, impulse->fomo_output_size,
              impulse->fomo_output_size);
        } else {
          fill_res = fill_result_struct_f32_fomo(
              impulse, result, output->data.f, impulse->fomo_output_size,
              impulse->fomo_output_size);
        }
        break;
      }
      ...
      case EI_CLASSIFIER_LAST_LAYER_SSD: {
        ...
        fill_res = fill_result_struct_f32_object_detection(
            impulse, result, tflite::post_process_boxes,
            tflite::post_process_scores, tflite::post_process_classes, debug);
#else
        ei_printf(
            "ERR: Cannot run SSD model, "
            "EI_CLASSIFIER_ENABLE_DETECTION_POSTPROCESS_OP is disabled\n");
        return EI_IMPULSE_UNSUPPORTED_INFERENCING_ENGINE;
#endif
    ...
    bool int8_output = output->type == TfLiteType::kTfLiteInt8;
    if (int8_output) {
      fill_res = fill_result_struct_i8(impulse, result, output->data.int8,
                                       output->params.zero_point,
