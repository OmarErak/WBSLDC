...
extern "C" {
static const ei_nn_tflite_micro_t get_nn_config_from_impulse(
    const ei_impulse_t* impulse) {
  const ei_nn_tflite_micro_t config = {
      .project_id = impulse->project_id,
      .block_id = 0xffffffff,  // we don't have one for learn blocks, so just
                               // set to max value here
...
static EI_IMPULSE_ERROR inference_tflite_setup(
    const ei_nn_tflite_micro_t* config, uint64_t* ctx_start_us,
    TfLiteTensor** input, TfLiteTensor** output, TfLiteTensor** output_labels,
    TfLiteTensor** output_scores, tflite::MicroInterpreter** micro_interpreter,
  ...
  *ctx_start_us = ei_read_timer_us();
  ...
  uint8_t* tensor_arena =
      (uint8_t*)ei_aligned_calloc(16, config->tflite_arena_size);
...
static EI_IMPULSE_ERROR inference_tflite_run(
    ...
    tflite::MicroInterpreter* interpreter, uint8_t* tensor_arena,
 ...
 */
EI_IMPULSE_ERROR run_nn_inference_from_dsp(const ei_nn_tflite_micro_t* config,
                                           signal_t* signal,
  ...
  EI_IMPULSE_ERROR init_res = inference_tflite_setup(
      config, &ctx_start_us, &input, &output, &output_labels, &output_scores,
...
EI_IMPULSE_ERROR run_nn_inference(const ei_impulse_t* impulse,
  ...
  EI_IMPULSE_ERROR run_res = inference_tflite_run(
      impulse, ctx_start_us, output, output_labels, output_scores, interpreter,
...
EI_IMPULSE_ERROR run_nn_inference_image_quantized(const ei_impulse_t* impulse,
  ...
  if (debug) {
    ...
    for (size_t ix = 0; ix < features_matrix.cols; ix++) {
      ei_printf_float(
          (features_matrix.buffer[ix] - impulse->tflite_input_zeropoint) *
...
__attribute__((unused)) int extract_tflite_features(signal_t* signal,
                                                    matrix_t* output_matrix,
                                                    void* config_ptr,
