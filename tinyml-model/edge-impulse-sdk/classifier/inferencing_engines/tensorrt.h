...
#error "TensorRT requires an unquantized network"
  ...
  snprintf(model_file_name, 128, "/tmp/%s-%d-%d.engine", impulse->project_name,
           impulse->project_id, impulse->deploy_version);
  ...
  libeitrt::infer(ei_trt_handle, fmatrix->buffer, out_data,
 ...
 */
EI_IMPULSE_ERROR run_nn_inference_image_quantized(ei_impulse_t *impulse,
                                                  signal_t *signal,
                                                  ei_impulse_result_t *result,
                                                  bool debug = false) {
  return EI_IMPULSE_UNSUPPORTED_INFERENCING_ENGINE;
