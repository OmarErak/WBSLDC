...
extern "C" {
static const ei_nn_tflite_eon_t get_nn_config_from_impulse(
...
static EI_IMPULSE_ERROR inference_tflite_setup(
    const ei_nn_tflite_eon_t* config, uint64_t* ctx_start_us,
    TfLiteTensor** input, TfLiteTensor** output, TfLiteTensor** output_labels,
    TfLiteTensor** output_scores, ei_unique_ptr_t& p_tensor_arena) {
...
static EI_IMPULSE_ERROR inference_tflite_run(
    const ei_impulse_t* impulse, uint64_t ctx_start_us, TfLiteTensor* output,
    TfLiteTensor* labels_tensor, TfLiteTensor* scores_tensor,
    uint8_t* tensor_arena, ei_impulse_result_t* result, bool debug) {
 ...
 */
EI_IMPULSE_ERROR run_nn_inference_from_dsp(const ei_nn_tflite_eon_t* config,
                                           signal_t* signal,
  ...
  EI_IMPULSE_ERROR init_res =
      inference_tflite_setup(config, &ctx_start_us, &input, &output,
                             &output_labels, &output_scores, p_tensor_arena);
  ...
  EI_IMPULSE_ERROR output_res =
      fill_output_matrix_from_tensor(output, output_matrix);
...
EI_IMPULSE_ERROR run_nn_inference(const ei_impulse_t* impulse,
  ...
  EI_IMPULSE_ERROR run_res =
      inference_tflite_run(impulse, ctx_start_us, output, output_labels,
                           output_scores, tensor_arena, result, debug);
...
EI_IMPULSE_ERROR run_nn_inference_image_quantized(const ei_impulse_t* impulse,
  ...
  int ret = extract_image_features_quantized(impulse, signal, &features_matrix,
  ...
  if (debug) {
    ...
    for (size_t ix = 0; ix < features_matrix.cols; ix++) {
      ei_printf_float((features_matrix.buffer[ix] - input->params.zero_point) *
                      input->params.scale);
      ei_printf(" ");
    }
    ei_printf("\n");
  ...
  EI_IMPULSE_ERROR run_res = inference_tflite_run(
      impulse, ctx_start_us, output, output_labels, output_scores,
      static_cast<uint8_t*>(p_tensor_arena.get()), result, debug);
...
__attribute__((unused)) int extract_tflite_eon_features(signal_t* signal,
                                                        matrix_t* output_matrix,
                                                        void* config_ptr,
                                                        const float frequency) {
  ei_dsp_config_tflite_eon_t config =
      *((ei_dsp_config_tflite_eon_t*)config_ptr);
  ...
  EI_IMPULSE_ERROR x =
      run_nn_inference_from_dsp(&nn_config, signal, output_matrix);
  if (x != EI_IMPULSE_OK) {
