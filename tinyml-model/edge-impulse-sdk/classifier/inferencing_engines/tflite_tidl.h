...
extern "C" EI_IMPULSE_ERROR run_nn_inference(const ei_impulse_t *impulse,
  ...
  if (!model) {
    ...
    typedef TfLiteDelegate *(*tflite_plugin_create_delegate)(
    ...
    tflite_plugin_dlg_create = (tflite_plugin_create_delegate)dlsym(
        lib, "tflite_plugin_create_delegate");
  ...
  for (uint32_t i = 0; i < inputs.size(); i++) {
    ...
    in_ptrs[i] =
        TIDLRT_allocSharedMem(tflite::kDefaultTensorAlignment, tensor->bytes);
    if (in_ptrs[i] == NULL) {
      ei_printf("Could not allocate Memory for input: %s\n", tensor->name);
    }
    interpreter->SetCustomAllocationForTensor(inputs[i],
                                              {in_ptrs[i], tensor->bytes});
  }
  for (uint32_t i = 0; i < outputs.size(); i++) {
    ...
    interpreter->SetCustomAllocationForTensor(outputs[i],
                                              {out_ptrs[i], tensor->bytes});
  }
  ...
  for (uint32_t ix = 0; ix < fmatrix->rows * fmatrix->cols; ix++) {
    if (impulse->object_detection) {
      ...
      input[ix] = static_cast<uint8_t>((pixel / impulse->tflite_input_scale) +
      ...
      input[ix] = static_cast<int8_t>(
          round(fmatrix->buffer[ix] / impulse->tflite_input_scale) +
          impulse->tflite_input_zeropoint);
#else
      input[ix] = fmatrix->buffer[ix];
#endif
