...
/* Private const types ----------------------------------------------------- */
#define MEM_ERROR \
  "ERR: Failed to allocate memory for performance calibration\r\n"
...
class RecognizeEvents {
 public:
  RecognizeEvents(const ei_model_performance_calibration_t *config,
                  uint32_t n_labels, uint32_t sample_length,
    ...
    float sample_length_ms =
        (static_cast<float>(sample_length) * sample_interval_ms);
    ...
    this->_average_window_duration_samples =
        (config->average_window_duration_ms <
         static_cast<uint32_t>(sample_length_ms))
            ? 1
    ...
    this->_suppression_samples =
        (config->suppression_ms < static_cast<uint32_t>(sample_length_ms))
            ? 0
            : static_cast<uint32_t>(static_cast<float>(config->suppression_ms) /
                                    sample_length_ms);

    /* Detection threshold should be high enough to only classifiy 1 possibly
     * output */
    ...
    this->_score_array =
        (float *)ei_malloc(this->_average_window_duration_samples *
                           this->_n_labels * sizeof(float));
    ...
    for (uint32_t i = 0;
         i < this->_average_window_duration_samples * this->_n_labels; i++) {
  ...
  bool should_boost() { return this->_should_boost; }
  ...
  int32_t trigger(ei_impulse_result_classification_t *scores) {
    ...
    /* Check pointers */
    if ((void *)this == NULL || this->_score_array == NULL ||
    ...
    for (uint32_t i = 0; i < this->_n_labels; i++) {
      this->_running_sum[i] -=
          this->_score_array[(this->_score_idx * this->_n_labels) + i];
  ...
  void operator delete(void *p) { ei_free(p); }
