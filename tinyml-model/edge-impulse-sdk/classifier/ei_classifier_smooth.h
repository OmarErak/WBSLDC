 ...
 * @param min_readings_same Minimum readings that need to be the same before
 * concluding (needs to be lower than n_readings)
 ...
 */
void ei_classifier_smooth_init(ei_classifier_smooth_t *smooth,
                               size_t n_readings, uint8_t min_readings_same,
                               float classifier_confidence = 0.8,
 ...
 * @returns Label, either 'uncertain', 'anomaly', or a label from the result
 * struct
 */
const char *ei_classifier_smooth_update(ei_classifier_smooth_t *smooth,
  ...
  uint8_t confidence_threshold =
      smooth->min_readings_same;  // XX% of windows should be the same
