...
#endif
#if (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER == \
     EI_CLASSIFIER_LAST_LAYER_YOLOV5) ||          \
    (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER == \
     EI_CLASSIFIER_LAST_LAYER_YOLOV5_V5_DRPAI)
#define EI_HAS_YOLOV5 1
#endif
#if (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER == \
     EI_CLASSIFIER_LAST_LAYER_YOLOX)
#define EI_HAS_YOLOX 1
#endif
#if (EI_CLASSIFIER_OBJECT_DETECTION_LAST_LAYER == \
     EI_CLASSIFIER_LAST_LAYER_YOLOV7)
 ...
 */
__attribute__((unused)) static bool ei_cube_check_overlap(
    ei_classifier_cube_t *c, int x, int y, int width, int height,
    float confidence) {
  bool is_overlapping = !(c->x + c->width < x || c->y + c->height < y ||
                          c->x > x + width || c->y > y + height);
  if (!is_overlapping) return false;

  // if we overlap, but the x of the new box is lower than the x of the current
  // box
  if (x < c->x) {
    // update x to match new box and make width larger (by the diff between the
    // boxes)
    c->x = x;
    c->width += c->x - x;
  }
  // if we overlap, but the y of the new box is lower than the y of the current
  // box
  if (y < c->y) {
    // update y to match new box and make height larger (by the diff between the
    // boxes)
    c->y = y;
    c->height += c->y - y;
  }
  // if we overlap, and x+width of the new box is higher than the x+width of the
  // current box
  if (x + width > c->x + c->width) {
    // just make the box wider
    c->width += (x + width) - (c->x + c->width);
  }
  // if we overlap, and y+height of the new box is higher than the y+height of
  // the current box
  if (y + height > c->y + c->height) {
    // just make the box higher
    c->height += (y + height) - (c->y + c->height);
  }
  // if the new box has higher confidence, then override confidence of the whole
  // box
...
__attribute__((unused)) static void ei_handle_cube(
    std::vector<ei_classifier_cube_t *> *cubes, int x, int y, float vf,
    const char *label, float detection_threshold) {
...
__attribute__((unused)) static void fill_result_struct_from_cubes(
    ei_impulse_result_t *result, std::vector<ei_classifier_cube_t *> *cubes,
    int out_width_factor, uint32_t object_detection_count) {
...
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_f32_fomo(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, float *data,
  ...
  for (size_t y = 0; y < out_width; y++) {
    ...
    for (size_t x = 0; x < out_height; x++) {
      ...
      for (size_t ix = 1; ix < impulse->label_count + 1; ix++) {
        ...
        ei_handle_cube(&cubes, x, y, vf, impulse->categories[ix - 1],
                       impulse->object_detection_threshold);
      }
    }
  }

  fill_result_struct_from_cubes(result, &cubes, out_width_factor,
                                impulse->object_detection_count);
...
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_i8_fomo(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, int8_t *data,
    float zero_point, float scale, int out_width, int out_height) {
#ifdef EI_HAS_FOMO
  std::vector<ei_classifier_cube_t *> cubes;
...
fill_result_struct_f32_object_detection(const ei_impulse_t *impulse,
                                        ei_impulse_result_t *result,
                                        float *data, float *scores,
                                        float *labels, bool debug) {
#ifdef EI_HAS_SSD
  static std::vector<ei_impulse_result_bounding_box_t> results;
  results.clear();
  ...
  for (size_t ix = 0; ix < impulse->object_detection_count; ix++) {
    ...
    if (score >= impulse->object_detection_threshold) {
      ...
      results[ix].x = static_cast<uint32_t>(
          xstart * static_cast<float>(impulse->input_width));
      results[ix].y = static_cast<uint32_t>(
          ystart * static_cast<float>(impulse->input_height));
      results[ix].width = static_cast<uint32_t>(
          (xend - xstart) * static_cast<float>(impulse->input_width));
      results[ix].height = static_cast<uint32_t>(
          (yend - ystart) * static_cast<float>(impulse->input_height));
 ...
 */
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_i8(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, int8_t *data,
    float zero_point, float scale, bool debug) {
 ...
 * Fill the result structure from an unquantized output tensor
 */
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_f32(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, float *data,
    bool debug) {
  for (uint32_t ix = 0; ix < impulse->label_count; ix++) {
    float value = data[ix];
 ...
 * Fill the result structure from an unquantized output tensor
 */
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_f32_yolov5(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, int version,
    float *data, size_t output_features_count) {
#ifdef EI_HAS_YOLOV5
  static std::vector<ei_impulse_result_bounding_box_t> results;
  results.clear();
...
template <typename T>
__attribute__((unused)) static EI_IMPULSE_ERROR
fill_result_struct_quantized_yolov5(const ei_impulse_t *impulse,
                                    ei_impulse_result_t *result, int version,
                                    T *data, float zero_point, float scale,
 ...
 * (we don't support quantized here a.t.m.)
 */
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_f32_yolox(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, float *data,
    size_t output_features_count) {
#ifdef EI_HAS_YOLOX
  static std::vector<ei_impulse_result_bounding_box_t> results;
  ...
  for (auto g : expanded_strides) {
    for (int row = 0; row < (int)g->rows; row++) {
      c_expanded_strides.buffer[c_expanded_strides_ix + 0] =
          g->buffer[(row * 1) + 0];
  ...
  for (int row = 0; row < (int)outputs.rows; row++) {
    boxes.buffer[(row * boxes.cols) + 0] =
        outputs.buffer[(row * outputs.cols) + 0];
    boxes.buffer[(row * boxes.cols) + 1] =
        outputs.buffer[(row * outputs.cols) + 1];
    boxes.buffer[(row * boxes.cols) + 2] =
        outputs.buffer[(row * outputs.cols) + 2];
    boxes.buffer[(row * boxes.cols) + 3] =
        outputs.buffer[(row * outputs.cols) + 3];
  }
  ...
  for (int row = 0; row < (int)outputs.rows; row++) {
    ...
    for (int cc = 0; cc < impulse->label_count; cc++) {
      scores.buffer[(row * scores.cols) + cc] =
          confidence * outputs.buffer[(row * outputs.cols) + (5 + cc)];
    }
  }
  ...
  for (int row = 0; row < (int)scores.rows; row++) {
    for (int col = 0; col < (int)scores.cols; col++) {
      ...
      if (confidence >= impulse->object_detection_threshold &&
          confidence <= 1.0f) {
        ei_impulse_result_bounding_box_t r;
 ...
 */
__attribute__((unused)) static EI_IMPULSE_ERROR
fill_result_struct_f32_yolox_detect(const ei_impulse_t *impulse,
                                    ei_impulse_result_t *result, float *data,
                                    size_t output_features_count) {
#ifdef EI_HAS_YOLOX
  static std::vector<ei_impulse_result_bounding_box_t> results;
  results.clear();
 ...
 * (we don't support quantized here a.t.m.)
 */
__attribute__((unused)) static EI_IMPULSE_ERROR fill_result_struct_f32_yolov7(
    const ei_impulse_t *impulse, ei_impulse_result_t *result, float *data,
    size_t output_features_count) {
#ifdef EI_HAS_YOLOV7
  static std::vector<ei_impulse_result_bounding_box_t> results;
