...
/**
  ...
  @param[in]     pSrc  points to input buffer (Source buffer is modified by this
  function)
  ...
  RIFFT"
  @par
                   If the input buffer is of length N, the output buffer must
  have length 2*N. The input buffer is modified by this function.
  @par
                   For the RIFFT, the source buffer must at least have length
                   fftLenReal + 2.
                   The last two elements must be equal to what would be
  generated by the RFFT: (pSrc[0] - pSrc[1]) >> 1 and 0
...
#else
    q31x4_t out = vhaddq_s32(MVE_CMPLX_MULT_FX_AxB(in1, coefA),
...
#if defined(__CMSIS_GCC_H)
    q31x4_t out =
        vhaddq_s32(MVE_CMPLX_MULT_FX_AxConjB_S32(in1, coefA),
                   vmulq_s32(conj, MVE_CMPLX_MULT_FX_AxB_S32(in2, coefB)));
#else
    q31x4_t out =
        vhaddq_s32(MVE_CMPLX_MULT_FX_AxConjB(in1, coefA),
