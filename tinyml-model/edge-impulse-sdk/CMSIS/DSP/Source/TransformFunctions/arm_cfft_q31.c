...
static void arm_bitreversal_32_inpl_mve(uint32_t *pSrc,
...
static void _arm_radix4_butterfly_q31_mve(const arm_cfft_instance_q31 *S,
                                          q31_t *pSrc, uint32_t fftLen) {
   ...
   * stages data is in 9.23(q23) format for the 256 point as there are 2 middle
   * stages data is in 7.25(q25) format for the 64 point as there are 1 middle
   * stage data is in 5.27(q27) format for the 16 point as there are no middle
   * stages
   */
...
static void arm_cfft_radix4by2_q31_mve(const arm_cfft_instance_q31 *S,
                                       q31_t *pSrc, uint32_t fftLen) {
  uint32_t n2;
  q31_t *pIn0;
  q31_t *pIn1;
...
static void _arm_radix4_butterfly_inverse_q31_mve(
    const arm_cfft_instance_q31 *S, q31_t *pSrc, uint32_t fftLen) {
  q31x4_t vecTmp0, vecTmp1;
  q31x4_t vecSum0, vecDiff0, vecSum1, vecDiff1;
  q31x4_t vecA, vecB, vecC, vecD;
  q31x4_t vecW;
...
static void arm_cfft_radix4by2_inverse_q31_mve(const arm_cfft_instance_q31 *S,
                                               q31_t *pSrc, uint32_t fftLen) {
  uint32_t n2;
  q31_t *pIn0;
  q31_t *pIn1;
...
void arm_cfft_q31(const arm_cfft_instance_q31 *S, q31_t *pSrc, uint8_t ifftFlag,
  ...
  if (bitReverseFlag) {
    arm_bitreversal_32_inpl_mve((uint32_t *)pSrc, S->bitRevLength,
                                S->pBitRevTable);
  }
}
#else
