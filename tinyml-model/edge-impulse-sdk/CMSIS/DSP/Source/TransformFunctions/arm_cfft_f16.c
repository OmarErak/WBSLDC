...
static void arm_bitreversal_f16_inpl_mve(uint16_t *pSrc,
...
static void _arm_radix4_butterfly_f16_mve(const arm_cfft_instance_f16 *S,
                                          float16_t *pSrc, uint32_t fftLen) {
  ...
  static const uint32_t strides[4] = {
      (0 - 16) * sizeof(float16_t *), (4 - 16) * sizeof(float16_t *),
      (8 - 16) * sizeof(float16_t *), (12 - 16) * sizeof(float16_t *)};
...
static void arm_cfft_radix4by2_f16_mve(const arm_cfft_instance_f16 *S,
                                       float16_t *pSrc, uint32_t fftLen) {
...
static void _arm_radix4_butterfly_inverse_f16_mve(
    const arm_cfft_instance_f16 *S, float16_t *pSrc, uint32_t fftLen,
  ...
  int32_t iter = 1;
  static const uint32_t strides[4] = {
      (0 - 16) * sizeof(q31_t *), (4 - 16) * sizeof(q31_t *),
      (8 - 16) * sizeof(q31_t *), (12 - 16) * sizeof(q31_t *)};
...
static void arm_cfft_radix4by2_inverse_f16_mve(const arm_cfft_instance_f16 *S,
                                               float16_t *pSrc,
                                               uint32_t fftLen) {
  float16_t const *pCoefVec;
  float16_t const *pCoef = S->pTwiddle;
...
void arm_cfft_f16(const arm_cfft_instance_f16 *S, float16_t *pSrc,
    ...
    switch (fftLen) {
      ...
      case 4096:
        _arm_radix4_butterfly_inverse_f16_mve(
            S, pSrc, fftLen, arm_inverse_fft_length_f16(S->fftLen));
        break;
  ...
  if (bitReverseFlag) {
    arm_bitreversal_f16_inpl_mve((uint16_t *)pSrc, S->bitRevLength,
                                 S->pBitRevTable);
  }
}
...
extern void arm_cfft_radix4by2_f16(float16_t* pSrc, uint32_t fftLen,
                                   const float16_t* pCoef);

extern void arm_radix4_butterfly_f16(float16_t* pSrc, uint16_t fftLen,
                                     const float16_t* pCoef,
...
/**
  ...
  inverse FFT.
  @par
                   For the MVE version, the new arm_cfft_init_f32 initialization
  function is <b>mandatory</b>. <b>Compilation flags are available to include
  only the required tables for the needed FFTs.</b> Other FFT versions can
  continue to be initialized as explained below.
  @par
                   For not MVE versions, pre-initialized data structures
  containing twiddle factors and bit reversal tables are provided and defined in
...
void arm_cfft_f16(const arm_cfft_instance_f16* S, float16_t* p1,
                  uint8_t ifftFlag, uint8_t bitReverseFlag) {
