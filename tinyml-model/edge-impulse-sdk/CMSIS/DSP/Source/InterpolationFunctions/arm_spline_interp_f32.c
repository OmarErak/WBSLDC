...
/**
 ...
 3/h(i)*[a(i+1)-a(i)]-3/h(i-1)*[a(i)-a(i-1)]    i=2, ..., n-1
  </pre>
 ...
 ...         ...    ||  ...   |=|                       ... | |  0        0 0
 ... h(n-2) 2[h(n-2)+h(n-1)] h(n-1) || c(n-1) | |
 3/h(n-1)*[a(n)-a(n-1)]-3/h(n-2)*[a(n-1)-a(n-2)] | |  0        0         0  ...
 0         0           1     ||  c(n)  | |                        0 |
  </pre>

  - Parabolic runout spline: S1''(x1)=2*c(1)=S2''(x2)=2*c(2) ;
 Sn-1''(xn-1)=2*c(n-1)=Sn''(xn)=2*c(n) In matrix form:
 ...
 0        -1           1     ||  c(n)  | |                        0 |
  ...
  A is a tridiagonal matrix (a band matrix of bandwidth 3) of size N=n+1. The
 factorization algorithms (A=LU) can be simplified considerably because a large
 number of zeros appear in regular patterns. The Crout method has been used: 1)
 Solve LZ=B
 ...
 @par Behaviour outside the given intervals
  ...
  input range (xq<x(1); xq>x(n)). The coefficients used to compute the y values
 for xq<x(1) are going to be the ones used for the first interval, while for
 xq>x(n) the coefficients used for the last interval.
...
void arm_spline_f32(arm_spline_instance_f32* S, const float32_t* xq,
  ...
  for (i = 0; i < n - 1; i++) {
    ...
    while (*pXq <= x[i + 1] && blkCnt > 0) {
      ...
      *pDst = y[i] + b[i] * (x_sc - x[i]) +
              c[i] * (x_sc - x[i]) * (x_sc - x[i]) +
