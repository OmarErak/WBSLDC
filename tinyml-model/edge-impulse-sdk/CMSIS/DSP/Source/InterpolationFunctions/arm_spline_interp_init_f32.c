...
/**
  ...
  The initialization function takes as input two arrays that the user has to
  allocate: <code>coeffs</code> will contain the b, c, and d coefficients for
  the (n-1) intervals (n is the number of known points), hence its size must be
  3*(n-1); <code>tempBuffer</code> is temporally used for internal computations
  and its size is n+n-1.
...
void arm_spline_init_f32(arm_spline_instance_f32* S, arm_spline_type type,
  ...
  float32_t* z =
      tempBuffer + (n - 1); /* n-long scratch buffer for z elements */
