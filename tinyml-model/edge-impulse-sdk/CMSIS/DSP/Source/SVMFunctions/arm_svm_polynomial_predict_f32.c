...
void arm_svm_polynomial_predict_f32(const arm_svm_polynomial_instance_f32 *S,
  ...
  while (row >= 4) {
    ...
    vSum = vfmaq_f32(
        ...
        arm_vec_exponent_f32(vaddq_n_f32(vmulq_n_f32(vtmp, S->gamma), S->coef0),
                             S->degree));
...
void arm_svm_polynomial_predict_f32(const arm_svm_polynomial_instance_f32 *S,
  ...
  while (vectorBlkCnt > 0U) {
    ...
    dotV = vsetq_lane_f32(vget_lane_f32(accum2, 0) + vget_lane_f32(accum2, 1),
                          dotV, 0);
    ...
    while (blkCnt > 0U) {
      dotV = vsetq_lane_f32(vgetq_lane_f32(dotV, 0) + *pIn * *pSupporta++, dotV,
                            0);
      dotV = vsetq_lane_f32(vgetq_lane_f32(dotV, 1) + *pIn * *pSupportb++, dotV,
                            1);
      dotV = vsetq_lane_f32(vgetq_lane_f32(dotV, 2) + *pIn * *pSupportc++, dotV,
                            2);
      dotV = vsetq_lane_f32(vgetq_lane_f32(dotV, 3) + *pIn * *pSupportd++, dotV,
                            3);
  ...
  while (vectorBlkCnt > 0U) {
    ...
    sum +=
        *pDualCoefs++ * arm_exponent_f32(S->gamma * dot + S->coef0, S->degree);
    vectorBlkCnt--;
  }
...
void arm_svm_polynomial_predict_f32(const arm_svm_polynomial_instance_f32 *S,
  ...
  for (i = 0; i < S->nbOfSupportVectors; i++) {
    ...
    sum += S->dualCoefficients[i] *
           arm_exponent_f32(S->gamma * dot + S->coef0, S->degree);
  }
