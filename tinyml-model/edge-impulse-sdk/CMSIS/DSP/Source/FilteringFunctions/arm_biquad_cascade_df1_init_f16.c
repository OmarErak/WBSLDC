...
/**
  ...
  @par             For MVE code, an additional buffer of modified coefficients
  is required. Its size is numStages and each element of this buffer has type
  arm_biquad_mod_coef_f16. So, its total size is 96*numStages float16_t
  elements.
...
static void generateCoefsFastBiquadF16(float16_t b0, float16_t b1, float16_t b2,
                                       float16_t a1, float16_t a2,
                                       arm_biquad_mod_coef_f16 *newCoef) {
  float32_t coeffs[8][12] = {{0, 0, 0, 0, 0, 0, 0, b0, b1, b2, a1, a2},
...
void arm_biquad_cascade_df1_mve_init_f16(arm_biquad_casd_df1_inst_f16 *S,
                                         ...
                                         float16_t *pState) {
  arm_biquad_cascade_df1_init_f16(S, numStages, (float16_t *)pCoeffsMod,
                                  pState);

  /* Generate SIMD friendly modified coefs */
  for (int i = 0; i < numStages; i++) {
    generateCoefsFastBiquadF16(pCoeffs[0], pCoeffs[1], pCoeffs[2], pCoeffs[3],
                               pCoeffs[4], pCoeffsMod);
    pCoeffs += 5;
    pCoeffsMod++;
  }
}
