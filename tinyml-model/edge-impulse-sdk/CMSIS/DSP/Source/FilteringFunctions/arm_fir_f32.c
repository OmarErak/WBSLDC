...
/**
  ...
  @par
  <pre>
      {x[n-numTaps+1], x[n-numTaps], x[n-numTaps-1],
  x[n-numTaps-2]....x[n](==pSrc[0]), x[n+1](==pSrc[1]), ...,
  ...
  FIR. The first A samples are temporary data. The remaining samples are the
  state of the FIR filter.
  @par
                 So the state buffer has size <code> numTaps + A + blockSize - 1
  </code> :
...
__STATIC_INLINE void arm_fir_f32_1_4_mve(const arm_fir_instance_f32 *S,
  ...
  /*
   * pState points to state array which contains previous frame (numTaps - 1)
   * samples pStateCur points to the location where the new input data should be
   * written
   */
  ...
  for (int i = 0; i < NB_TAPS; i++) c[i] = *pCoeffsCur++;
