  ...
  {                                                                          \
    ...
    for (int i = 0; i < nbVecTaps; i++)                                      \
      vecCoeffs[i] = vldrbq_s8(pCoeffs + 16 * i);                            \
                                                                             \
    /*                                                                       \
     * pState points to state array which contains previous frame (numTaps - \
     * 1) samples pStateCur points to the location where the new input data  \
     * should be written                                                     \
     */                                                                      \
...
void arm_fir_q7_1_16_mve(const arm_fir_instance_q7 *S,
                         const q7_t *__restrict pSrc, q7_t *__restrict pDst,
                         uint32_t blockSize) {
#define NBTAPS 16
  FIR_Q7_MAIN_CORE();
#undef NBTAPS
