...
/**
  ...
  arm_biquad_mod_coef_f32. So, its total size is 32*numStages float32_t
  elements.
...
static void generateCoefsFastBiquadF32(float32_t b0, float32_t b1, float32_t b2,
...
void arm_biquad_cascade_df1_mve_init_f32(arm_biquad_casd_df1_inst_f32 *S,
                                         ...
                                         float32_t *pState) {
  arm_biquad_cascade_df1_init_f32(S, numStages, (float32_t *)pCoeffsMod,
                                  pState);

  /* Generate SIMD friendly modified coefs */
  for (int i = 0; i < numStages; i++) {
    generateCoefsFastBiquadF32(pCoeffs[0], pCoeffs[1], pCoeffs[2], pCoeffs[3],
