...
uint32_t arm_gaussian_naive_bayes_predict_f16(
    const arm_gaussian_naive_bayes_instance_f16 *S, const float16_t *in,
    float16_t *pBuffer) {
  uint32_t nbClass;
  ...
  for (nbClass = 0; nbClass < S->numberOfClasses; nbClass++) {
    ...
    while (blkCnt > 0U) {
      ...
      vacc1 =
          vaddq(vacc1, vlogq_f16(vmulq_n_f16(vsigma, 2.0f16 * (_Float16)PI)));
    ...
    if (blkCnt > 0U) {
      ...
      vacc1 = vaddq_m_f16(vacc1, vacc1,
                          vlogq_f16(vmulq_n_f16(vsigma, 2.0f16 * (_Float16)PI)),
                          p0);
