...
arm_status arm_mat_cmplx_mult_q15(const arm_matrix_instance_q15 *pSrcA,
                                  ...
                                  q15_t *pScratch) {
  q15_t const *pInA =
      (q15_t const *)pSrcA->pData; /* input data matrix pointer A of Q15 type */
  q15_t const *pInB =
      (q15_t const *)pSrcB->pData; /* input data matrix pointer B of Q15 type */
  q15_t const *pInB2;
...

  {
    ...
    while (row > 0u) {
      ...
      while (col > 0u) {
        ...
        /*
         * Convert to 1.15, Store the results (1 x 2 block) in the destination
         * buffer
         */
