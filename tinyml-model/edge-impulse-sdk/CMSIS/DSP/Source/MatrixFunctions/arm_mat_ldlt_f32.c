...
/**
   ...
   * @param[in]  pSrc   points to the instance of the input floating-point
   matrix structure.
   * @param[out] pl   points to the instance of the output floating-point
   triangular matrix structure.
   * @param[out] pd   points to the instance of the output floating-point
   diagonal matrix structure.
   * @param[out] pp   points to the instance of the output floating-point
   permutation vector.
   ...
   * @return        execution status
                   ...
                   - \ref ARM_MATH_DECOMPOSITION_FAILURE      : Input matrix
   cannot be decomposed
   * @par
   *  Computes the LDL^t decomposition of a matrix A such that P A P^t = L D
   L^t.
   */
arm_status arm_mat_ldlt_f32(const arm_matrix_instance_f32 *pSrc,
  ...
  if ((pSrc->numRows != pSrc->numCols) || (pl->numRows != pl->numCols) ||
      (pd->numRows != pd->numCols) || (pp->numRows != pp->numCols) ||
    ...
    for (k = 0; k < n; k++) {
      ...
      for (w = k + 1; w < n; w += 4) {
        ...
        for (int x = k + 1; x < n; x += 4) {
          ...
          vecX =
              vldrwq_gather_shifted_offset_z_f32(&pA[x * n + k], vecOffs, p0);
