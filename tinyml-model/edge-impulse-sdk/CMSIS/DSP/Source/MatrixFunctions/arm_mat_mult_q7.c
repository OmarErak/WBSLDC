 ...
 *
 * \par
 * The function is implemented using a 32-bit internal accumulator saturated
 * to 1.7 format.
 *
 *
 */
...
__STATIC_FORCEINLINE arm_status arm_mat_mult_q7_2x2_mve(
    const arm_matrix_instance_q7 *pSrcA, const arm_matrix_instance_q7 *pSrcB,
    arm_matrix_instance_q7 *pDst) {
  const uint32_t MATRIX_DIM = 2;
  q7_t const *pInB =
      (q7_t const *)pSrcB->pData; /* input data matrix pointer B */
