...
float32_t arm_braycurtis_distance_f32(const float32_t *pA, const float32_t *pB,
  ...
  /*
     It is assumed that accumSum is not zero. Since it is the sum of several
     absolute values it would imply that all of them are zero. It is very
     unlikely for long vectors.
   */
...
float32_t arm_braycurtis_distance_f32(const float32_t *pA, const float32_t *pB,
  ...
  It is assumed that accumSum is not zero. Since it is the sum of several
  absolute values it would imply that all of them are zero. It is very unlikely
  for long vectors.
