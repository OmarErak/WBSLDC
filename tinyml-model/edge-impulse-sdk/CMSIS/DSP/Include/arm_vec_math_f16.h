...
__STATIC_INLINE float16x8_t vtaylor_polyq_f16(float16x8_t x,
...
__STATIC_INLINE float16x8_t vmant_exp_f16(float16x8_t x, int16x8_t* e) {
  any16x8_t r;
  int16x8_t n;
...
__STATIC_INLINE float16x8_t vexpq_f16(float16x8_t x) {
  ...
  float16x8_t val =
      vfmsq_f16(x, vcvtq_f16_s16(m), vdupq_n_f16(0.6931471805f16));
...
__STATIC_INLINE f16x8_t vrecip_f16(f16x8_t vecIn) {
  ...
  // v.f = v.f * (8 + w * (-28 + w * (56 + w * (-70 + w *(56 + w * (-28 + w * (8
  // - w)))))));
