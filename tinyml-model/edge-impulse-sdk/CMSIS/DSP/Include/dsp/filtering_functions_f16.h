...
typedef struct {
  ...
  float16_t *pState; /**< points to the state variable array. The array is of
                        length numTaps+blockSize-1. */
  const float16_t *pCoeffs; /**< points to the coefficient array. The array is
                               of length numTaps. */
 ...
 */
void arm_fir_f16(const arm_fir_instance_f16 *S, const float16_t *pSrc,
...
typedef struct {
  ...
  float16_t *pState;  /**< Points to the array of state coefficients.  The array
                         is of length 4*numStages. */
  const float16_t *pCoeffs; /**< Points to the array of coefficients.  The array
...
typedef struct {
  float16_t coeffs[12]
                  [8]; /**< Points to the array of modified coefficients.  The
...
void arm_biquad_cascade_stereo_df2T_f16(
    const arm_biquad_cascade_stereo_df2T_instance_f16 *S, const float16_t *pSrc,
    float16_t *pDst, uint32_t blockSize);
...
void arm_biquad_cascade_stereo_df2T_init_f16(
    arm_biquad_cascade_stereo_df2T_instance_f16 *S, uint8_t numStages,
    const float16_t *pCoeffs, float16_t *pState);
 ...
 */
void arm_correlate_f16(const float16_t *pSrcA, uint32_t srcALen,
                       const float16_t *pSrcB, uint32_t srcBLen,
                       float16_t *pDst);
