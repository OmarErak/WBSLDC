  ...
  if (num_elements_in_axis > 0) {
    for (size_t idx = 0; idx < num_outputs; ++idx) {
      if (compute_sum) {
        ...
        output_data[idx] = static_cast<T>(temp_sum[idx] /
                                          static_cast<U>(num_elements_in_axis));
      }
    }
  }
  return true;
...
inline void MeanOrSum(const tflite::MeanParams& op_params,
                      ...
                      T* output_data, bool compute_sum) {
...
inline void MeanOrSum(const tflite::MeanParams& op_params,
                      ...
                      float output_scale, bool compute_sum) {
  ...
  double real_scale =
      compute_sum ? static_cast<double>(input_scale / (1 * output_scale))
                  : static_cast<double>(input_scale /
                                        (num_elements_in_axis * output_scale));
