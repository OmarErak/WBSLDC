...
TfLiteStatus AverageEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data->supported == kMvp) {
    ...
    TFLITE_DCHECK_EQ(
        arm_avgpool_s8(&ctx, &pool_params, &input_dims, data->op_params.input,
                       &filter_dims, &output_dims, data->op_params.output),
        ARM_MATH_SUCCESS);
...
TfLiteStatus MaxEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data->supported == kMvp) {
    ...
    TFLITE_DCHECK_EQ(
        arm_max_pool_s8(&ctx, &pool_params, &input_dims, data->op_params.input,
                        &filter_dims, &output_dims, data->op_params.output),
        ARM_MATH_SUCCESS);
  } else if (data->supported == kTFLMrefF32) {
#if EI_TFLITE_DISABLE_MAX_POOL_2D_IN_F32
...
void AverageEvalQuantized(TfLiteContext* context, const TfLiteNode* node,
  ...
  if (depth % 4 == 0) {  // S3 version only supports channels multiple of 4
    for (int batch = 0; batch < batches; ++batch) {
      esp_nn_avg_pool_s8(input_data, input_width, input_height, output_data,
                         output_width, output_height, stride_width,
                         stride_height, filter_width, filter_height, pad_width,
                         pad_height, activation_min, activation_max, depth);
    ...
    for (int batch = 0; batch < batches; ++batch) {
      esp_nn_avg_pool_s8_ansi(input_data, input_width, input_height,
                              ...
                              stride_width, stride_height, filter_width,
                              filter_height, pad_width, pad_height,
...
void MaxEvalQuantized(TfLiteContext* context, TfLiteNode* node,
  ...
  if (depth % 4 == 0) {  // S3 version only supports channels multiple of 4
    for (int batch = 0; batch < batches; ++batch) {
      esp_nn_max_pool_s8(input_data, input_width, input_height, output_data,
                         output_width, output_height, stride_width,
                         stride_height, filter_width, filter_height, pad_width,
                         pad_height, activation_min, activation_max, depth);
      input_data += input_size;
