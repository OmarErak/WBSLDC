...
static int software_rfft(float* fft_input, TfLiteComplex64* output,
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  TF_LITE_ENSURE_STATUS(context->RequestScratchBufferInArena(
      context, output_els * sizeof(kiss_fft_cpx),
      &data->kiss_fft_output_buffer_index));
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (fft_height != 1) {
    context->ReportError(context, "Only supports fft_height 1",
                         TfLiteTypeGetName(output->type));
    return kTfLiteError;
  }

  kiss_fft_cpx* shift_buffer = (kiss_fft_cpx*)context->GetScratchBuffer(
      context, data->kiss_fft_output_buffer_index);
  ...
  for (size_t row = 0; row < input->dims->data[0]; row++) {
    ...
    if (x != 0) {
      context->ReportError(context, "software_rfft failed (%d)", x);
      return kTfLiteError;
    }
  }
