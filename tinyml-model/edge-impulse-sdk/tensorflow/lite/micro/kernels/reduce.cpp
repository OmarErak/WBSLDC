    ...
    case kTfLiteFloat32: {
      ...
      if (params->keep_dims && special_case_4d_axes_1_and_2) {
        reference_ops::MeanOrSum(
            ...
            tflite::micro::GetTensorData<float>(output), false);
      } else {
        TF_LITE_ENSURE(
            context,
            reference_ops::MeanOrSum(
    ...
    case kTfLiteUInt8: {
      ...
      if (params->keep_dims && special_case_4d_axes_1_and_2) {
        reference_ops::MeanOrSum(
            ...
            op_data->input_scale, tflite::micro::GetTensorShape(output),
            tflite::micro::GetTensorData<uint8_t>(output), op_data->output_zp,
            op_data->output_scale, false);
      } else if (op_data->input_zp == op_data->output_zp &&
                 op_data->input_scale == op_data->output_scale) {
    ...
    case kTfLiteFloat32: {
      ...
      if (params->keep_dims && special_case_4d_axes_1_and_2) {
        reference_ops::MeanOrSum(
            ...
            tflite::micro::GetTensorData<float>(input),
            tflite::micro::GetTensorShape(output),
            tflite::micro::GetTensorData<float>(output), true);
      } else {
        TF_LITE_ENSURE(
            context,
            reference_ops::MeanOrSum(
