...
// We use global memory to keep track of these... Why, you might ask... I'm not
// sure if this is a bug with our version of TFLite, and incompatibility with
// this op, but the output tensors are defined as using 4 bytes for each tensor.
// When these are filled they are thus writing in uninitialized memory or
// overwriting previous tensors. Perhaps this is fixed in TF2.4 or something,
// but for now we'll just allocate some global memory.
...
void Free(TfLiteContext* context, void* buffer) { /* noop */
}
...
void DequantizeBoxEncodings(const TfLiteTensor* input_box_encodings, int idx,
  ...
  const uint8_t* boxes = tflite::GetTensorData<uint8_t>(input_box_encodings) +
...
TfLiteStatus NonMaxSuppressionMultiClassRegularHelper(TfLiteContext* context,
  ...
  TfLiteTensor* detection_classes =
      tflite::GetOutput(context, node, kOutputTensorDetectionClasses);
  ...
  for (int output_box_index = 0; output_box_index < max_detections;
    ...
    if (output_box_index < size_of_sorted_indices) {
      ...
      // detection_classes
      tflite::GetTensorData<float>(detection_classes)[output_box_index] = 0.0f;
      // detection_scores
      tflite::GetTensorData<float>(detection_scores)[output_box_index] = 0.0f;
    }
  }
  tflite::GetTensorData<float>(num_detections)[0] = size_of_sorted_indices;
