...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    if (sli_mvp_ml_conv2d_s8_is_supported(&data->op_params)) {
      ...
      if (data->op_params.dilation_height == 1 &&
...
TfLiteStatus Invoke(TfLiteContext* context, TfLiteNode* node) {
  ...
  const auto bias = NumInputs(node) == 3 ? tflite::micro::GetEvalInput(
                                               context, node, kBiasTensor)
                                         : nullptr;
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteInt8) {
    ...
    data_dims_t filter_dims = {
        .width = filter_width, .height = filter_height, 0, 0};
  ...
  if (dilation_width_factor == 1 && dilation_height_factor == 1) {
    ...
    for (int i_batch = 0; i_batch < batch_size; i_batch++) {
      esp_nn_conv_s8(&input_dims, input_data + i_batch * input_size,
                     ...
                     tflite::micro::GetTensorData<int32_t>(bias), &output_dims,
                     output_data + i_batch * output_size, &conv_params,
                     &quant_data);
    }
  } else {
    reference_integer_ops::ConvPerChannel(
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {  // Already know in/out types are same.
    case kTfLiteFloat32: {
      ...
      return kTfLiteError;
#endif
      tflite::reference_ops::Conv(ConvParamsFloat(params, data.op_data),
                                  tflite::micro::GetTensorShape(input),
                                  tflite::micro::GetTensorData<float>(input),
                                  tflite::micro::GetTensorShape(filter),
                                  tflite::micro::GetTensorData<float>(filter),
